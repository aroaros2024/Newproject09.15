...
| EX | もっと不思議のダンジョン | 99F | **持ち込み不可・Lv1 スタート**（拾った物は持ち帰れる） |
...
`npm test` で 186 件が走ります。主なもの:

- **ダンジョン生成**: 300 シードで全マスが連結していること。
  水路・溶岩・大部屋・円形・迷路・狭いマップでも不通のフロアが出ないこと。
  迷路フロアで部屋が区画からはみ出さないこと
- **通しプレイ**: 6 ダンジョンをランダム操作で回し、毎ターン
  HP・満腹度・レベル・座標・アクターの重なり・床アイテムの重なりを検査
- **バランス**: 想定装備でボスを 13〜35 発で倒せること、
  雑魚を 10 発以内で倒せること、ボスに 3 ターン以上耐えられること
  （防御力を素直に伸ばすと、指数のダメージ式で討伐不可能になるため）
- **撃破の経路**: ボスを「実際に殴って」倒し、撃破が記録されて階段が
  開くこと（HP を直接 0 にするテストでは撃破時の処理を素通りしてしまう）
- **村の倉庫**: 冒険を繰り返しても uid が重複しないこと、
  狙った道具だけが売れること、ギタンが往復で増えないこと
- **アイテム**: 全 195 種を「使う」「投げる」— 例外も不整合も起きないこと。
  掘り進みの杖が「歩いて行けない床」を作らないこと
- **効果 id**: アイテム・モンスター・ワナ・腕輪の効果 id に、
  必ず実装が対応していること（綴り間違いで黙って無効になるのを防ぐ）
- **操作**: キーの衝突が無いこと、修飾キーを使っていないこと、
  テンキーだけで全操作が完結すること

このほか、45 万ターンのランダム操作による連続実行で不整合を探しています。
