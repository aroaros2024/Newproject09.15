 ...
 * 最初の入力で `unlock()` を呼ぶこと。
 */
const DEFAULT_SETTINGS = { master: 0.7, sfx: 0.8, bgm: 0.45, muted: false };
...
const seq = (scale, idx, oct = 0) => idx.map((i) => (i === null ? null : scale[i % scale.length] + 12 * (oct + Math.floor(i / scale.length))));
const BGM = {
    title: {
...
export class AudioEngine {
    ...
    play(name, volumeScale = 1) {
        ...
        for (const t of spec.tones ?? [])
            this.playTone(ctx, dest, t, now, volumeScale);
        for (const n of spec.noises ?? [])
            this.playNoise(ctx, dest, n, now, volumeScale);
    }
    playTone(ctx, dest, t, base, scale) {
        ...
        if (f2 !== t.freq) {
            if (t.exp)
                osc.frequency.exponentialRampToValueAtTime(Math.max(1, f2), start + t.dur);
            else
                osc.frequency.linearRampToValueAtTime(Math.max(1, f2), start + t.dur);
        }
    ...
    playNoise(ctx, dest, n, base, scale) {
        if (!this.noiseBuffer)
            return;
    ...
    scheduleBgm() {
        const tick = () => {
            ...
            if (!ctx || !track || !dest || this.settings.muted)
                return;
            if (ctx.state === 'suspended')
                return;
