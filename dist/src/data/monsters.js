...
const RAW = [
    ...
    m('armorBlack', '黒武者', 'armor', 2, 4, 110, 50, 20, 1680, '黒塗りの鎧を着た手練れ。二度斬りかかってくる。', { speed: 'doubleAttack', skills: ['doubleAttack'], rate: 30,
        drop: { itemId: 'steelShield', rate: 6 } }),
    m('armorTenrin', '天輪の衛士', 'armor', 3, 5, 200, 82, 28, 4800, '塔を守る衛士。隙がない。', { speed: 'doubleAttack', skills: ['doubleAttack', 'healSelf'], rate: 35,
    ...
    m('ogreKing', 'おにの大将', 'ogre', 2, 5, 190, 78, 22, 4400, '鬼たちを従える大将。', { crit: 1 / 10, skills: ['knockbackHit', 'summonAlly'], rate: 30,
    ...
    m('dragonFlame', 'ほのおドラゴン', 'dragon', 1, 4, 112, 52, 15, 1740, '広い範囲を焼き払う炎を吐く。', { move: 'lava', skills: ['breatheFire'], rate: 35, drop: { itemId: 'flameSword', rate: 4 } }),
    m('dragonInferno', 'しゃくねつドラゴン', 'dragon', 2, 5, 210, 86, 24, 5200, '灼熱の息で一直線に焼き尽くす。', { move: 'lava', speed: 'double', skills: ['breatheInferno'], rate: 40,
    ...
    // ===================================================== 特殊系統
    m('metalBug', 'はがねムシ', 'metal', 0, 3, 20, 14, 30, 700, '鋼のように硬い虫。ほとんどダメージが通らない。', { metal: true, ai: 'coward', evade: 0.2, drop: { itemId: 'ironShield', rate: 8 } }),
    m('metalKing', 'メタルムシの王', 'metal', 1, 5, 40, 30, 50, 8000, '倒せれば莫大な経験値になるが、まず当たらない。', { metal: true, ai: 'flee', speed: 'double', evade: 0.35,
    ...
    m('polterLord', 'おどりのぬし', 'poltergeist', 1, 5, 140, 62, 18, 3800, '持ち物を投げつけ、壺を割っていく。', { speed: 'double', skills: ['useItemOnPlayer'], rate: 45,
        drop: { itemId: 'unbreakablePot', rate: 8 } }),
    m('abyssWatcher', 'ふちのみはり', 'abyss', 0, 6, 160, 70, 22, 5600, 'もっと不思議のダンジョンの深層を徘徊する者。', { speed: 'double', skills: ['sealPlayer', 'levelDrain'], rate: 40 }),
    m('abyssDevourer', 'ふちのくらうもの', 'abyss', 1, 7, 260, 105, 30, 12000, '装備も持ち物も飲み込んでいく。', { speed: 'double', skills: ['swallowItem', 'curseEquip'], rate: 45 }),
    m('abyssSovereign', 'ふちのあるじ', 'abyss', 2, 8, 420, 150, 40, 26000, '底の底に棲むという存在。出会ったら逃げるほかない。', { speed: 'double', skills: ['breatheInferno', 'levelDrain', 'summonAlly'], rate: 50 }),
    // ===================================================== 店主・番犬
    m('shopkeeper', '店主', 'shop', 0, 10, 300, 90, 30, 0, '店を営む商人。怒らせると恐ろしい。', { ai: 'guard', speed: 'double', crit: 1 / 8, sprite: 'shopkeeper' }),
    m('shopGuard', '店の番犬', 'shop', 1, 8, 160, 70, 22, 0, '泥棒を追いかけてくる番犬。', { speed: 'double', sprite: 'shopGuard' }),
    // ===================================================== ボス
    m('bossForest', '森の主 オオヤマネコ', 'boss', 0, 6, 220, 40, 10, 2000, 'せせらぎの森の奥に棲む大山猫。素早い爪さばきで襲いかかる。', { ai: 'boss', speed: 'double', boss: true, skills: ['doubleAttack', 'leapAttack'],
        rate: 40, sprite: 'bossForest' }),
    m('bossVolcano', '火口の番人 イワオニ', 'boss', 1, 9, 420, 62, 16, 6000, '灼熱の火山の火口を守る岩の鬼。溶岩を投げつけてくる。', { ai: 'boss', move: 'lava', boss: true, skills: ['breatheFire', 'throwBoulder', 'summonAlly'],
        rate: 45, sprite: 'bossVolcano' }),
    m('bossWaterway', '水路の淀み ヌシ', 'boss', 2, 12, 640, 84, 22, 14000, '常闇の地下水路に沈む巨大な影。水と闇を操る。', { ai: 'boss', move: 'water', boss: true, speed: 'double',
        skills: ['waterJet', 'blindTouch', 'summonAlly'], rate: 50, sprite: 'bossWaterway' }),
    m('bossTowerFirst', '天輪の守り手', 'boss', 3, 15, 900, 105, 26, 30000, '天輪の塔の頂で待つ者。まだ本気ではない。', { ai: 'boss', boss: true, speed: 'double',
        skills: ['doubleAttack', 'magicBind', 'summonAlly'], rate: 50, sprite: 'bossTower' }),
    m('bossTowerFinal', '天輪の主 カゼノヌシ', 'boss', 4, 20, 1400, 135, 32, 80000, '風そのものが形をとった姿。すべての風来人の行き着く先。', { ai: 'boss', boss: true, speed: 'double',
        skills: ['breatheInferno', 'levelDrain', 'magicBind', 'summonAlly', 'healSelf'],
        rate: 60, sprite: 'bossTowerFinal' }),
    m('bossAbyss', 'もっと不思議の果て', 'boss', 5, 25, 2600, 190, 45, 200000, '99 階の最奥に在るもの。名前を持たない。', { ai: 'boss', boss: true, speed: 'double',
