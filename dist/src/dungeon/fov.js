 ...
 */
export function computeFov(map, from, opts = {}) {
    clearVisibility(map);
    if (opts.clairvoyant) {
    ...
    // 暗い部屋・通路: 周囲 8 マスのみ
    for (const n of neighbors8(map, from))
        reveal(map, n.x, n.y);
    return { room, visible: collectVisible(map) };
}
/**
 ...
 */
export function currentRoom(map, p) {
    ...
    for (const n of neighbors8(map, p)) {
        ...
        if (r && r.doors.some((d) => d.x === p.x && d.y === p.y))
            return r;
    }
    for (const n of neighbors8(map, p)) {
        const r = roomOf(map, n);
...
function revealRoom(map, room) {
    ...
    for (const d of room.doors) {
        reveal(map, d.x, d.y);
        for (const n of neighbors8(map, d))
            reveal(map, n.x, n.y);
    }
}
function reveal(map, x, y) {
    if (!inBounds(map, x, y))
        return;
    const t = at(map, x, y);
    if (!t)
...
function collectVisible(map) {
    ...
    for (let y = 0; y < map.height; y++) {
        for (let x = 0; x < map.width; x++) {
            if (map.tiles[y * map.width + x].visible)
                out.push({ x, y });
        }
    }
    return out;
 ...
 */
export function canSee(map, a, b, ignoreDark = false) {
    if (a.x === b.x && a.y === b.y)
        return true;
 ...
 */
export function hasLineOfFire(map, a, b) {
    const dx = b.x - a.x;
    const dy = b.y - a.y;
    if (dx === 0 && dy === 0)
        return false;
...
export function revealWholeFloor(map, includeTraps = false) {
    for (const t of map.tiles) {
        t.explored = true;
        if (includeTraps && t.trap)
