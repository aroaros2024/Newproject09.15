 ...
 */
import { chebyshev, idxOf, rectContains } from '../core/geom.js';
export function makeTile(kind = 'wall') {
    return {
        kind,
        hard: false,
        roomId: -1,
...
export function at(map, x, y) {
    if (!inBounds(map, x, y))
        return undefined;
    return map.tiles[idxOf(x, y, map.width)];
}
export const tileAt = (map, p) => at(map, p.x, p.y);
/** 範囲外なら「掘れない壁」を返す。境界チェックを省きたい所で使う */
const OUT_OF_BOUNDS = Object.freeze({
    kind: 'wall', hard: true, roomId: -1, isDoor: false,
    explored: false, visible: false, trap: null, shop: false,
});
export function tileOr(map, x, y) {
    return at(map, x, y) ?? OUT_OF_BOUNDS;
}
export function setKind(map, x, y, kind) {
    const t = at(map, x, y);
    if (t && !t.hard)
        t.kind = kind;
}
/** 壁でないか（床・水・溶岩・階段・落とし穴） */
export const isOpen = (t) => !!t && t.kind !== 'wall';
/** 通路か（部屋に属さない開けたマス） */
export const isCorridor = (t) => isOpen(t) && t.roomId < 0;
/** 視線を通すか。壁だけが遮る */
export const isTransparent = (t) => isOpen(t);
/**
 * その移動タイプで入れるマスか。
 * 罠や敵の有無は見ない（それは game 側の責務）。
 */
export function canEnter(map, x, y, move) {
    const t = at(map, x, y);
    if (!t)
        return false;
    switch (t.kind) {
 ...
 */
export function canMoveDiagonally(map, from, dx, dy, move) {
    if (dx === 0 || dy === 0)
        return true;
    if (move === 'phase' || move === 'fly')
        return true;
    const side1 = at(map, from.x + dx, from.y);
    const side2 = at(map, from.x, from.y + dy);
    return isOpen(side1) && isOpen(side2);
}
export function roomOf(map, p) {
    const t = tileAt(map, p);
    if (!t || t.roomId < 0)
        return null;
    return map.rooms.find((r) => r.id === t.roomId) ?? null;
}
export const roomById = (map, id) => map.rooms.find((r) => r.id === id) ?? null;
/** 矩形を指定の地形で塗る（外周の掘れない壁は避ける） */
export function fillRect(map, r, kind, roomId = -1) {
...
/** 部屋の内側（壁際を除く）の座標を列挙 */
export function roomInterior(room, inset = 0) {
    const out = [];
    const r = room.rect;
...
/** 部屋の全マス */
export const roomCells = (room) => roomInterior(room, 0);
export const isInRoom = (room, p) => rectContains(room.rect, p);
/** 8 近傍の座標（範囲内のみ） */
export function neighbors8(map, p) {
    const out = [];
...
/** 4 近傍の座標（範囲内のみ） */
export function neighbors4(map, p) {
    const out = [];
 ...
 */
export function connectivityReport(map) {
    const open = [];
 ...
 * 到達不能は -1。
 */
export function distanceField(map, from, move = 'ground') {
    const w = map.width;
    const dist = new Int32Array(w * map.height).fill(-1);
    if (!inBounds(map, from.x, from.y))
        return dist;
    const queue = [from];
    dist[idxOf(from.x, from.y, w)] = 0;
    let head = 0;
...
/** 開けたマスをすべて集める */
export function allOpenTiles(map, pred) {
    const out = [];
    for (let y = 0; y < map.height; y++) {
        for (let x = 0; x < map.width; x++) {
            const t = at(map, x, y);
...
export function allRoomFloors(map) {
    return allOpenTiles(map, (t) => t.roomId >= 0 && t.kind === 'floor');
}
/** p から半径 r 以内の開けたマス */
export function openTilesNear(map, p, r) {
    const out = [];
...
/** すべてのタイルの可視フラグを下ろす */
export function clearVisibility(map) {
    for (const t of map.tiles)
        t.visible = false;
}
/** デバッグ用に ASCII でフロアを出力する */
export function renderAscii(map, marks = []) {
    const grid = [];
    for (let y = 0; y < map.height; y++) {
        const row = [];
        for (let x = 0; x < map.width; x++) {
    ...
    for (const m of marks) {
        if (inBounds(map, m.p.x, m.p.y))
            grid[m.p.y][m.p.x] = m.ch;
    }
