...
const NOPE = (reason) => ({ tookTurn: false, reason });
/** 混乱していると方向が狂う */
export function applyConfusion(world, a, dir) {
    if (!world.hasStatus(a, 'confused'))
        return dir;
    if (!world.rng.chance(CONFUSE_MISDIRECT_RATE))
        return dir;
    return world.rng.int(8);
}
/**
 * そのアクターがその方向へ動けるか。
 * 壁・斜めのすり抜け・他のアクターをすべて見る。
 */
export function canStepTo(world, a, dir) {
    const v = DIR_VEC[dir];
    ...
    if (!canEnter(world.map, to.x, to.y, move))
        return false;
    if (!canMoveDiagonally(world.map, a.pos, v.x, v.y, move))
        return false;
...
export function moveActor(world, a, dir) {
    if (!canStepTo(world, a, dir))
        return false;
    const from = { ...a.pos };
...
/** プレイヤーの 1 行動を解決する */
export function performPlayerAction(world, action) {
...
function playerAttack(world, rawDir) {
    const p = world.player;
    if (!canAttack(p)) {
...
function playerPickup(world) {
    const p = world.player;
    const f = world.floorItemAt(p.pos);
    if (!f)
        return NOPE('足元には 何も無い');
...
function playerPlace(world, uid) {
    const p = world.player;
    const item = findItem(p, uid);
    if (!item)
        return NOPE('その道具は 持っていない');
    if (isEquipped(p, uid) && item.cursed) {
        world.log('呪われていて 手から 離れない！', 'bad');
        return OK;
    }
    if (world.floorItemAt(p.pos))
        return NOPE('足元に 物が 置いてある');
    if (at(world.map, p.pos.x, p.pos.y)?.kind !== 'floor')
        return NOPE('ここには 置けない');
...
function playerSwap(world, uid) {
    ...
    if (!floor)
        return NOPE('足元に 何も 無い');
    if (floor.item.shopPrice > 0)
        return NOPE('店の 商品は 交換できない');
    if (at(world.map, p.pos.x, p.pos.y)?.kind !== 'floor')
        return NOPE('ここでは 交換できない');
    removeFromInventory(p, uid);
    ...
    world.log(`${itemName(mine, world.run.identify)}を 置いて `
        + `${itemName(floor.item, world.run.identify)}を 拾った。`, 'item');
    world.sfx('pickup');
    return OK;
}
function playerStairs(world) {
    const p = world.player;
    if (!samePoint(p.pos, world.map.stairs))
        return NOPE('ここに 階段は 無い');
    ...
    if (a.kind === 'player') {
        ...
        if (f) {
            ...
            if (f.item.shopPrice > 0) {
                ...
                if (!picked)
                    world.log(`${name}が 落ちている。持ち物が いっぱいだ。`, 'warning');
            }
        }
...
export function triggerTrap(world, a, trapId) {
    ...
    if (tile.trap.revealed && !world.rng.percent(def.rate)) {
        if (a.kind === 'player')
            world.log(`${def.name}を うまく かわした。`, 'good');
        return false;
    }
    tile.trap.revealed = true;
    if (def.oneShot)
...
/** 隣接したワナを見つける */
export function detectAdjacentTraps(world, p) {
...
/** ダッシュを止めるべき状況か */
export function shouldStopDash(world, p, prevRoomId) {
    ...
    for (const m of world.run.monsters) {
        ...
        if (mt?.visible && chebyshev(m.pos, p.pos) <= 4)
            return true;
    }
    // 通路で分岐に来たら止まる
    if (tile.roomId < 0) {
        ...
        for (const v of DIR_VEC) {
            if (isOpen(at(world.map, p.pos.x + v.x, p.pos.y + v.y)))
                open++;
        }
        if (open > 2)
            return true;
...
export function throwPath(world, from, dir, range) {
    const path = [];
    let hit = null;
    let cur = { ...from };
    for (let i = 0; i < range; i++) {
        const next = step(cur, dir);
        if (!isOpen(at(world.map, next.x, next.y)))
            break;
        path.push(next);
        cur = next;
