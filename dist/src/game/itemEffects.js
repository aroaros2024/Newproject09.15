...
function enemiesInRoom(world, from) {
    ...
    if (room) {
        return world.run.monsters.filter((m) => m.alive && roomCells(room).some((c) => c.x === m.pos.x && c.y === m.pos.y));
    }
    return world.run.monsters.filter((m) => m.alive && chebyshev(m.pos, from) <= 2);
}
/** 杖の弾道に沿って最初に当たる相手を探す */
export function staffTarget(world, from, dir, range = 10) {
    let cur = { ...from };
    for (let i = 0; i < range; i++) {
        const next = step(cur, dir);
        if (!at(world.map, next.x, next.y) || at(world.map, next.x, next.y).kind === 'wall')
            return null;
        cur = next;
...
/** 状態異常を与えるだけの効果 */
const inflictSelf = (id, msg, good) => ({ world, user, victim }) => {
    const t = victim ?? user;
...
export const ITEM_EFFECTS = {
    ...
    cureConfuse: ({ world, user }) => {
        ...
        if (a || b)
            world.log('頭が すっきりした。', 'good');
        return a || b;
    },
    cureAll: ({ world, user }) => {
        const n = cureAilments(world, user);
        if (n > 0)
            world.log('体の 異常が すべて 治った。', 'good');
        return n > 0;
    },
    revive: ({ world }) => {
    ...
    warp: ({ world, user }) => {
        const spot = world.randomSpawnTile(6) ?? world.randomOpenTile((p) => at(world.map, p.x, p.y)?.kind === 'floor' && !world.actorAt(p));
        if (!spot)
            return false;
        const from = { ...user.pos };
        user.pos = spot;
    ...
    levelUp: ({ world, user }) => {
        if (user.kind !== 'player') {
            if (user.kind === 'monster')
                return evolveMonster(world, user);
            return false;
        }
        if (user.level >= 99) {
    ...
    identifyOne: ({ world }) => {
        const p = world.player;
        const unknown = p.inventory.filter((i) => !world.run.identify.known[i.defId] || !i.plusKnown);
        if (unknown.length === 0)
            return false;
    ...
    sleepRoom: ({ world, user }) => {
        ...
        for (const m of targets)
            applyStatus(world, m, 'deepAsleep');
    ...
    vacuum: ({ world, user }) => {
        ...
        world.sfx('zap');
        for (const m of targets)
            dealDamage(world, user, m, dmg, 'magic');
        return true;
    },
    blast: ({ world, user }) => {
        world.log('爆発した！', 'bad');
    ...
    fireLine: ({ world, user, dir }) => ITEM_EFFECTS.breatheFire({
        world, user, dir: dir ?? user.dir, item: {},
    }),
    sanctuary: ({ world, user }) => {
        if (at(world.map, user.pos.x, user.pos.y)?.kind !== 'floor')
            return false;
    ...
    clone: ({ world, user }) => {
        ...
        for (const p of neighbors8(world.map, user.pos)) {
            if (n >= 2)
                break;
            if (world.actorAt(p) || at(world.map, p.x, p.y)?.kind !== 'floor')
                continue;
            const m = factory(p);
            if (m) {
                // 分身は味方として振る舞う
    ...
    blank: ({ world }) => {
        ...
        const handler = ITEM_EFFECTS[pick.effect];
        if (!handler)
            return true;
        return handler({
            world, user: world.player, item: {}, target: null,
        });
    },
    synthesize: ({ world }) => {
        const p = world.player;
    ...
    makeTrap: ({ world, user }) => {
        ...
        if (!tile || tile.kind !== 'floor' || tile.trap)
            return false;
    ...
    curseItems: ({ world }) => {
        ...
        for (const it of world.rng.sample(pool, n))
            it.cursed = true;
    ...
    invisibleTarget: ({ world, user, dir }) => staffStatus(world, user, dir, 'invisible', '透明になった'),
    ...
    transform: ({ world, user, dir }) => staffHit(world, user, dir, (t) => {
        if (t.kind === 'player')
            return false;
        const pool = world.dungeon.monsters.filter((e) => world.run.depth >= e.from && world.run.depth <= e.to);
        if (pool.length === 0)
            return false;
    ...
    changePot: ({ world, target }) => {
        if (!target)
            return false;
        const pool = world.dungeon.items.filter((e) => world.run.depth >= e.from && world.run.depth <= e.to);
        if (pool.length === 0)
            return false;
        ...
        if (def && def.kind === 'staff')
            target.charges = world.rng.range(def.charges[0], def.charges[1]);
    ...
    blindOnHit: ({ world, victim, user }) => {
        const center = victim ? victim.pos : user.pos;
        for (const m of world.run.monsters) {
            if (chebyshev(m.pos, center) <= 2)
                applyStatus(world, m, 'blind');
        }
...
/** 杖の弾で当たった相手に処理を適用する */
function staffHit(world, user, dir, fn) {
    const d = dir ?? user.dir;
...
function staffStatus(world, user, dir, status, verb) {
 ...
 */
export function mergeInto(world, base, material) {
...
export function throwDamage(world, item, thrower) {
    ...
    if (def.kind === 'weapon')
        return def.atk + item.plus;
    if (def.kind === 'shield')
        return def.def + item.plus;
...
//# sourceMappingURL=itemEffects.js.map