 ...
 */
import { DIR_VEC, chebyshev, dirTo, isOnRay, oppositeDir, samePoint, step, } from '../core/geom.js';
...
/** そのモンスターがその方向に進めるか */
function canStep(world, m, dir) {
    const v = DIR_VEC[dir];
    const to = { x: m.pos.x + v.x, y: m.pos.y + v.y };
    const move = world.defOf(m).moveType;
    if (!canEnter(world.map, to.x, to.y, move))
        return false;
    if (!canMoveDiagonally(world.map, m.pos, v.x, v.y, move))
        return false;
...
/** 目標から遠ざかる方向へ 1 歩 */
function stepAwayFrom(world, m, target) {
    const away = dirTo(target, m.pos);
    const candidates = away === null
        ? (world.rng.shuffled([0, 1, 2, 3, 4, 5, 6, 7]))
        : [away, ((away + 1) % 8), ((away + 7) % 8),
            ((away + 2) % 8), ((away + 6) % 8)];
    for (const d of candidates) {
        if (canStep(world, m, d)) {
            const to = step(m.pos, d);
            if (chebyshev(to, target) >= chebyshev(m.pos, target))
                return stepTo(world, m, d);
        }
    }
    // どこにも逃げられない
    for (const d of world.rng.shuffled([0, 1, 2, 3, 4, 5, 6, 7])) {
        if (canStep(world, m, d))
            return stepTo(world, m, d);
    }
    return false;
...
function wanderStep(world, m) {
    if (world.rng.percent(65) && canStep(world, m, m.dir))
        return stepTo(world, m, m.dir);
    const dirs = world.rng.shuffled([0, 1, 2, 3, 4, 5, 6, 7])
...
/** 階段へ向かう（盗んだ敵の逃走） */
function stepTowardStairs(world, m) {
...
/** 目標を見つけているか */
function perceives(world, m, target) {
    if (world.hasStatus(target, 'invisible'))
        return false;
    const def = world.defOf(m);
    if (chebyshev(m.pos, target.pos) <= 1)
        return true;
    if (def.keenSense && chebyshev(m.pos, target.pos) <= 8)
        return true;
 ...
 */
export function takeMonsterTurn(world, m, ctx) {
    if (!m.alive)
        return;
    // 眠り・まひなどで動けない
    if (isIncapacitated(m))
        return;
    const def = world.defOf(m);
    // 眠っている敵は、近くに来られると起きる
    if (m.asleep) {
    ...
    switch (def.ai) {
        ...
        case 'ranged': {
            ...
            if (sees && dist <= 1) {
                // 近すぎるので離れる
                if (stepAwayFrom(world, m, target.pos))
                    return;
                attackIfPossible(world, m, target);
                return;
            }
...
function attackIfPossible(world, m, target) {
    ...
    if (d === null || chebyshev(m.pos, target.pos) > 1)
        return false;
...
function chaseAndAttack(world, m, target, ctx) {
    if (chebyshev(m.pos, target.pos) <= 1) {
        if (attackIfPossible(world, m, target))
            return;
    }
    if (!canMove(m))
        return;
    ...
    if (target.kind === 'player') {
        ...
        if (stepDownField(world, m, field))
            return;
    }
    // 仲間狙いや距離場で進めない場合は素朴に寄る
    const goal = m.lastSeen ?? target.pos;
    const d = dirTo(m.pos, goal);
    if (d !== null && stepTo(world, m, d))
        return;
    if (samePoint(m.pos, goal))
        m.lastSeen = null;
    wanderStep(world, m);
}
/** ボスの行動。HP が減るほど攻勢に出る */
function bossTurn(world, m, target, ctx) {
    const ratio = m.hp / m.maxHp;
    const phase = ratio > 0.6 ? 0 : ratio > 0.3 ? 1 : 2;
    if (phase !== m.bossPhase) {
        m.bossPhase = phase;
        if (phase === 1)
            world.log(`${world.nameOf(m)}は 本気を 出してきた！`, 'bad');
        if (phase === 2)
            world.log(`${world.nameOf(m)}の 力が 膨れ上がる！`, 'bad');
    }
    // 追い詰められると特技の頻度が上がる
    if (phase >= 1 && world.rng.percent(30 + phase * 20)) {
        if (useSkill(world, m, target))
            return;
    }
    chaseAndAttack(world, m, target, ctx);
}
/** 仲間の作戦に応じた行動 */
export function takeAllyTurn(world, a, ctx) {
    if (!a.alive || isIncapacitated(a))
        return;
    ...
    if (chebyshev(a.pos, player.pos) > 1) {
        const field = distanceField(world.map, player.pos, world.defOf(a).moveType);
        if (stepDownField(world, a, field))
            return;
    }
    // プレイヤーのそばで待機
    if (roomOf(world.map, a.pos))
        wanderStep(world, a);
}
//# sourceMappingURL=monsterAI.js.map