 ...
 */
const dividedAtk = (world, m) => (world.dungeon.splitWeakens ? Math.max(1, Math.floor(m.atk / 2)) : m.atk);
/** 盾の「盗」印・盗賊よけの腕輪で盗みを防げるか */
function blocksTheft(world, t) {
    if (t.kind !== 'player')
        return false;
    if (shieldRune(world, world.player, 'antiSteal') > 0)
        return true;
    ...
    if (b && !b.cursed && !world.hasStatus(t, 'sealed')) {
        ...
        if (def.kind === 'bracelet' && def.effect === 'wardSteal')
            return true;
    }
    return false;
}
/** 状態異常を与える特技の共通処理 */
const inflict = (id, message, needAdjacent = true) => (world, m, t) => {
    if (needAdjacent && !adjacent(m, t))
        return false;
    if (world.hasStatus(t, id))
        return false;
...
/** 視線が通っていれば離れていても効く特技 */
const gaze = (id, message) => (world, m, t) => {
    if (chebyshev(m.pos, t.pos) > 5)
        return false;
    if (!isOnRay(m.pos, t.pos) || !hasLineOfFire(world.map, m.pos, t.pos))
        return false;
    if (world.hasStatus(t, id))
        return false;
...
/** 直線上に飛ぶ魔法弾 */
const magicBolt = (id, message, color) => (world, m, t) => {
...
/** 空いている隣接マスを探す */
function freeNeighbor(world, from) {
    const spots = [];
    for (let dy = -1; dy <= 1; dy++) {
        for (let dx = -1; dx <= 1; dx++) {
            ...
            if (!canEnter(world.map, p.x, p.y, 'ground'))
                continue;
            if (world.actorAt(p))
                continue;
            spots.push(p);
...
export const MONSTER_SKILLS = {
    ...
    drainHp: (world, m, t) => {
        ...
        if (r.hit && r.damage > 0) {
            ...
            if (heal > 0)
                world.log(`${world.nameOf(m)}は 血を 吸った！`, 'bad');
        }
        return true;
    },
    healSelf: (world, m) => {
    ...
    sealPlayer: (world, m, t) => {
        if (chebyshev(m.pos, t.pos) > 3)
            return false;
        if (world.hasStatus(t, 'sealed'))
            return false;
    ...
    stealEquip: (world, m, t) => {
        ...
        if (blocksTheft(world, t))
            return false;
        const p = t;
        const equips = [equippedWeapon(p), equippedShield(p)].filter((e) => !!e && !e.cursed);
        if (equips.length === 0)
            return false;
    ...
    curseEquip: (world, m, t) => {
        if (!adjacent(m, t) || t.kind !== 'player')
            return false;
        const equips = [equippedWeapon(t), equippedShield(t)].filter((e) => !!e && !e.cursed);
        if (equips.length === 0)
            return false;
        const item = world.rng.pick(equips);
        item.cursed = true;
    ...
    useItemOnPlayer: (world, m, t) => {
        if (chebyshev(m.pos, t.pos) > 6 || t.kind !== 'player')
            return false;
        const pool = losableItems(t);
        if (pool.length === 0)
            return false;
...
export function useSkill(world, m, target) {
    const def = world.defOf(m);
...
/** 爆発する敵が倒れた時の処理 */
export function explodeOnDeath(world, m) {
    ...
    for (const a of world.livingActors()) {
        if (a === m)
            continue;
        if (chebyshev(a.pos, m.pos) > radius)
            continue;
...
//# sourceMappingURL=monsterSkills.js.map