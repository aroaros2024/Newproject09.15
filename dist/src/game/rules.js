...
/**
 * 10 連の値段。単発 10 回とちょうど同じ。
 *
 * 安くしていないのは、10 連の取り柄を値段ではなく
 * 「SR 以上が 1 つ確定する」ことに寄せてあるため。
 */
export const GACHA_COST_10 = 100;
/**
 * 出るものが尽きたあと、1 回引くごとに出るギタン。
 *
 * 護石は在庫が尽きないので、ここに来るのは
 * 「護石の箱が満杯で、かつ他の景品を集めきった」ときだけ。
 */
export const GACHA_EMPTY_GITAN = 300;
 ...
 * 深く潜る理由が無くなる。
 */
export function depthValue(depth) {
    let v = 0;
 ...
 */
export function stonesForRun(a) {
...
export const hpGainForLevel = (level) => 4 + Math.floor(level / 10);
/** 累積経験値からレベルを求める（ドレイン後の整合取りに使う） */
export function levelForExp(exp) {
    let level = 1;
    while (level < MAX_LEVEL && exp >= EXP_TABLE[level + 1])
        level++;
    return level;
}
/** そのレベルでの最大 HP（初期値から積み上げた理論値） */
export function baseMaxHpAt(level) {
    let hp = START_HP;
    for (let l = 1; l < level; l++)
        hp = Math.min(MAX_HP_CAP, hp + hpGainForLevel(l));
    return hp;
}
/** 次のレベルまでの必要経験値。最大レベルなら 0 */
export function expToNext(level, exp) {
    if (level >= MAX_LEVEL)
        return 0;
 ...
 * 最低 1 ダメージは保証する。
 */
export function calcDamage(atk, def, rng) {
    const base = atk * defenseMultiplier(def);
    const r = rng.range(112, 143) / 128;
    return Math.max(1, Math.floor(base * r));
}
/** 乱数を使わない期待値（表示・デバッグ用） */
export const expectedDamage = (atk, def) => Math.max(1, Math.floor(atk * defenseMultiplier(def)));
/** 基本命中率 */
...
export function hitRate(baseAcc, accBonus, evade) {
 ...
 */
export function regenPerTurnX1000(maxHp, level, mul = 1) {
...
export function monsterHouseRate(base, depth, maxDepth) {
    const progress = maxDepth > 1 ? (depth - 1) / (maxDepth - 1) : 0;
    return Math.min(35, base + progress * base);
}
/** ギタンを投げた時のダメージ */
export const gitanThrowDamage = (amount) => Math.max(1, Math.floor(amount / 10));
