...
export function startRun(dungeonId, town, opts = {}) {
    ...
    if (town.shortcuts) {
        for (let i = 0; i < SHORTCUT_SLOTS; i++)
            player.shortcutIds[i] = town.shortcuts[i] ?? null;
    }
    ...
    if (dungeon.allowBring && opts.bring) {
        ...
        for (const item of opts.bring.slice(0, boosts.bagLimit)) {
            ...
            if (kept.includes(item.uid))
                addKept(player, copy.uid, boosts.keepSlots);
    ...
    for (const id of boosts.knownIds)
        run.identify.known[id] = true;
    ...
    if (dungeon.allowBring && boosts.gitan > 0)
        player.gitan += boosts.gitan;
    enterFloor(world, 1);
    // 相棒は地形ができてから置く
    if (boosts.partner)
        spawnPartner(world, boosts.partner);
    return world;
}
/**
