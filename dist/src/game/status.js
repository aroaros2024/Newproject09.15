...
function isWarded(world, a, id) {
    ...
    if (!world.hasStatus(a, 'sealed')) {
        ...
        if (bracelet && !bracelet.cursed) {
            ...
            if (effect === 'wardConfuse' && id === 'confused')
                return true;
            if (effect === 'wardSleep' && (id === 'asleep' || id === 'deepAsleep'))
                return true;
            if (effect === 'wardPoison' && (id === 'poisoned' || id === 'deadlyPoisoned'))
                return true;
        }
    }
...
/** 状態異常の持続ターンを決める */
export function rollDuration(world, id) {
 ...
 */
export function applyStatus(world, a, id, turns, power = 0) {
    if (!a.alive)
        return false;
    if (isWarded(world, a, id)) {
        if (a.kind === 'player')
            world.log(`しかし ${STATUS_NAME[id]}は 防がれた！`, 'good');
        return false;
    }
    // 聖なる印で持続を短くする
    ...
    for (const other of CANCELS[id] ?? [])
        removeStatus(world, a, other, true);
...
export function removeStatus(world, a, id, silent = false) {
    const i = a.statuses.findIndex((s) => s.id === id);
    if (i < 0)
        return false;
    a.statuses.splice(i, 1);
    if (!silent)
        world.emit({ t: 'status', actorId: a.id, status: id, applied: false });
    return true;
}
/** 悪い状態異常をすべて解除する */
export function cureAilments(world, a) {
    let n = 0;
...
/** ダメージを受けた時に解除される状態 */
export function wakeOnDamage(world, a) {
 ...
 * 戻り値は「このアクターが死んだか」。
 */
export function tickStatuses(world, a) {
    if (!a.alive)
        return false;
...
/** 表示用: 掛かっている状態異常の一覧 */
export function activeStatuses(a) {
