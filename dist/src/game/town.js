...
import { EMBED_PRICE, REFORGE_PRICE, addCharmRune, meltValue, reforgeCharm, rollAddableRune, } from './charm.js';
...
export const storageLimit = (town) => STORAGE_LIMIT + townBoosts(town).storage;
export const storageFull = (town) => town.storage.length >= storageLimit(town);
/**
 * 村での行動を数える。
 *
...
export function inventoryLimitFor(town, d) {
...
export function shopStock(town) {
    ...
    if (town.cleared.length >= 1)
        pool.push('ironSword', 'ironShield', 'sleepStaff', 'bentou');
    // 分身の巻物はダンジョンでは 1 周 0.04〜0.20 個しか出ない。
    // 仲間を連れる遊びが運任せにならないよう、村でも買えるようにする
    if (town.cleared.length >= 2)
        pool.push('steelSword', 'steelShield', 'greatIdentify', 'cloneScroll');
 ...
 */
export function smithEmbed(town, rng, charmUid) {
