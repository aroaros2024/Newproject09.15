...
const TRAP_EFFECTS = {
    ...
    trapBigMine: (world, a) => {
        ...
        for (const victim of world.livingActors()) {
            if (victim === a || victim.kind === 'player')
                continue;
            if (chebyshev(victim.pos, a.pos) > 2)
                continue;
            dealDamage(world, null, victim, victim.maxHp, 'fire');
        }
    },
    ...
    trapItemLoss: (world, a) => {
        ...
        const lost = world.rng.sample(pool, n);
...
export function applyTrapEffect(world, a, trapId) {
