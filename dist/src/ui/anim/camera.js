...
export class Camera {
     ...
     */
    setTarget(centerX, centerY, mapTilesW, mapTilesH) {
        this.cx = centerX;
        this.cy = centerY;
        this.clampTarget(mapTilesW, mapTilesH);
    }
    /** マス（小数あり）の中心を画面の中央に置く目標にする */
    centerOnTile(tileX, tileY, mapTilesW, mapTilesH) {
    ...
    clampTarget(mapTilesW, mapTilesH) {
    ...
    approach() {
        ...
        if (Math.abs(this.tx - this.x) < Camera.SETTLE)
            this.x = this.tx;
        if (Math.abs(this.ty - this.y) < Camera.SETTLE)
            this.y = this.ty;
        this.round();
    }
    round() {
