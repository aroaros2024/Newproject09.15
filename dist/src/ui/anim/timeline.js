...
const tileKey = (x, y) => (y & 0xffff) * 65536 + (x & 0xffff);
...
export class Timeline {
    ...
    flush(h) {
        while (this.head < this.count)
            h.fireCue(this.q[this.head++]);
        this.head = 0;
        this.count = 0;
        this.now = 0;
    }
    ...
    fireDue(h) {
        while (this.head < this.count && this.q[this.head].time <= this.now)
            h.fireCue(this.q[this.head++]);
    ...
    pendingDamage(actorId) {
        ...
        for (let i = this.head; i < this.count; i++) {
            const c = this.q[i];
            if (c.kind === CUE.DAMAGE && c.actor === actorId)
                sum += c.effective;
        }
        return sum;
    }
    ...
    pendingHeal(actorId) {
        ...
        for (let i = this.head; i < this.count; i++) {
            const c = this.q[i];
            if (c.kind === CUE.HEAL && c.actor === actorId)
                sum += c.amount;
        }
        return sum;
    }
    /** この id に撃破の合図が待っているか */
    pendingDefeat(actorId) {
        for (let i = this.head; i < this.count; i++) {
            const c = this.q[i];
            if (c.kind === CUE.DEFEAT && c.actor === actorId)
                return true;
        }
        return false;
    }
                ...
                case 'sfx': {
                    ...
                    if (!launch && this.nextOpensAttack(events, i + 1, to)) {
                        ...
                        if (this.deferredCount < this.deferred.length)
                            this.deferred[this.deferredCount] = e.name;
                        else
                            this.deferred.push(e.name);
                        this.deferredCount++;
                        break;
                    }
    ...
    finish() {
        ...
        if (k < 1) {
            for (let i = this.head; i < this.count; i++) {
                const c = this.q[i];
                c.time *= k;
                if (c.kind === CUE.THROW || c.kind === CUE.BEAM)
                    c.ms *= k;
            }
        }
        this.compression = k;
    ...
    // ------------------------------------------------------------ 内側
    openBeat(kind, src, srcTile, impact, gap) {
        const b = this.b;
    ...
    hpLeft(ctx, id) {
        ...
        for (let i = this.head; i < this.count; i++) {
            ...
            if (c.kind === CUE.DAMAGE)
                hp -= c.effective;
            else if (c.kind === CUE.HEAL)
                hp += c.amount;
        }
        return hp > 0 ? hp : 0;
    }
    /** 台帳の位置を合図へ写す（second なら x2, y2） */
    locate(ctx, id, c, second) {
        if (id < 0 || !ctx.locate(id, this.pt))
            return;
        if (second) {
            c.x2 = this.pt.x;
            c.y2 = this.pt.y;
