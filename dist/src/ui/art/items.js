...
import { disc, ellipse, hline, line, rect, runs, vline } from './pixbuf.js';
 ...
 */
export function iconKeyOf(defId, known) {
    const def = tryGetItem(defId);
    if (!def)
        return 'weapon';
    if (CATEGORY_ONLY.has(def.kind))
        return def.kind;
    if (def.kind === 'pot')
        return known && POT_SPEC[defId] ? defId : 'pot';
    if (ICONS.has(defId))
        return defId;
 ...
 * 輪郭は外周 0 / 15 まで広がってよい）
 */
export function buildIcon(key, out, finish = true) {
...
export const hasIcon = (key) => ICONS.has(key);
export const allIconKeys = () => [...ICONS.keys()];
const ICONS = new Map();
/** 左下から右上へ斜めの線（刃・柄） */
function diag(b, x, y, len, c) {
    for (let i = 0; i < len; i++)
        b.set(x + i, y - i, c);
}
const WEAPON_SPEC = {
...
function drawWeapon(b, s) {
    const B = (k) => ci(s.blade, k);
    const H = (k) => ci(s.grip, k);
    const G = (k) => ci(s.guard ?? s.grip, k);
    const len = s.len ?? 0;
    switch (s.shape) {
        case 'sword':
        case 'great':
        ...
        case 'fang': {
            ...
            if (s.shape === 'cleaver') {
                // 鉈：刃の背を四角く
                for (let i = 2; i < blade; i++)
                    b.set(x0 + i + 2, y0 - i + 1, B(2));
            }
            break;
        }
        case 'twin': {
            // 双刃：2 本の短剣を交差させる
            for (const [x, y, flip] of [[3, 12, false], [12, 12, true]]) {
                for (let i = 0; i < 9; i++) {
...
function shieldRows(shape) {
    ...
    for (let y = 1; y <= 14; y++) {
        ...
        if (hw >= 0)
            rows.push([y, 8 - hw, 8 + hw - 1]);
    }
    return rows;
}
function drawShield(b, s) {
    const rows = shieldRows(s.shape);
    // 縁（外側 1 ドット）と面
    for (const [y, x0, x1] of rows)
        hline(b, x0, y, x1 - x0 + 1, ci(s.rim, 3));
    for (let i = 1; i < rows.length - 1; i++) {
        const [y, x0, x1] = rows[i];
        if (x1 - x0 >= 2)
            hline(b, x0 + 1, y, x1 - x0 - 1, ci(s.face, 3));
    }
    // 面の明暗：左上を明るく、右下を暗く
    for (let i = 1; i < rows.length - 1; i++) {
        const [y, x0, x1] = rows[i];
        ...
        if (i === 1)
            hline(b, x0 + 1, y, x1 - x0 - 2, ci(s.face, 4));
    }
    const E = (k) => ci(s.emblemRamp ?? s.rim, k);
 ...
 */
function riceBall(b, size, rice, specks) {
...
const MISC = {
    ...
    gitan: (b) => {
        // 積んだ小判
        for (const [x, y] of [[4, 11], [9, 11], [6, 8], [11, 8], [8, 5]]) {
...
const CATEGORY = {
    ...
    scroll: (b) => {
        ...
        for (const y of [6, 8, 10])
            hline(b, 6, y, y === 10 ? 3 : 4, ci('stone', 2));
        for (const y of [2, 12]) {
...
function potShape(b, body, mark, markRamp) {
    // 胴
    ...
    b.set(6, 7, ci(body, 5));
    for (let y = 8; y <= 12; y++)
        b.set(12, y, ci(body, 2));
    hline(b, 5, 13, 6, ci(body, 2));
    const M = (k) => ci(markRamp ?? body, k);
    switch (mark) {
        case 'swirl':
...
// 登録
// ---------------------------------------------------------------------------
for (const [id, spec] of Object.entries(WEAPON_SPEC))
    ICONS.set(id, (b) => drawWeapon(b, spec));
for (const [id, spec] of Object.entries(SHIELD_SPEC))
    ICONS.set(id, (b) => drawShield(b, spec));
for (const [id, draw] of Object.entries(FOOD))
    ICONS.set(id, draw);
for (const [id, draw] of Object.entries(MISC))
    ICONS.set(id, draw);
for (const [id, spec] of Object.entries(POT_SPEC)) {
    ICONS.set(id, (b) => potShape(b, spec.body, spec.mark, spec.markRamp));
}
for (const [id, draw] of Object.entries(CATEGORY))
    ICONS.set(id, draw);
/** 図鑑などで道具の種類を問わず絵を出したいとき（正体が分かっている前提） */
export function iconKeyForCatalog(defId) {
