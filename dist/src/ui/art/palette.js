...
/** 階調の番号 */
export const rampIndex = (name) => RAMPS.indexOf(name);
/** 階調と段から色番号。段は 0〜5 に切り詰める */
export function ci(ramp, step) {
    const r = typeof ramp === 'number' ? ramp : RAMPS.indexOf(ramp);
    const s = step < 0 ? 0 : step > STEPS - 1 ? STEPS - 1 : Math.round(step);
    return 1 + r * STEPS + s;
}
/** 色番号 → 階調の番号（透明と白は -1） */
export const rampOf = (i) => i <= 0 || i >= WHITE ? -1 : Math.floor((i - 1) / STEPS);
/** 色番号 → 段（透明は -1、白は 6） */
export const stepOf = (i) => i <= 0 ? -1 : i >= WHITE ? STEPS : (i - 1) % STEPS;
/** 同じ階調の中で段をずらす。白は明るくも暗くもしない */
export function shiftStep(i, d) {
    const r = rampOf(i);
    if (r < 0)
        return i;
 ...
 */
export function outlineOf(i, lightSide = false) {
    const r = rampOf(i);
    if (r < 0)
        return ci('ink', lightSide ? 2 : 1);
    return ci(r, lightSide ? 1 : 0);
}
const hexList = ['#000000'];
for (const name of RAMPS)
    for (const h of RAMP_HEX[name])
        hexList.push(h);
hexList.push('#ffffff');
/** 色番号 → '#rrggbb'（0 番の透明には '#000000' が入っているが描かない） */
export const PAL_HEX = hexList;
/** fillStyle にそのまま渡せる文字列（毎フレーム文字列を作らないため） */
export const PAL_CSS = hexList;
/**
 * 色番号 → ImageData 用の 32bit 値（リトルエンディアンの ABGR）。
 * 0 番は完全な透明。
 */
export const PAL_RGBA = (() => {
