...
import { clamp, flipT, over, rnd, shades } from './rat.js';
// ---------------------------------------------------------------------------
// 色
// ---------------------------------------------------------------------------
/** 飾りのビット */
...
const DARKER = { W: 'w', w: 'v', v: 'u', u: 'u', h: 'a', a: 'b', b: 'c', c: 'c' };
function shadeRight(t) {
    return flipT(t).map((row) => [...row].map((ch) => DARKER[ch] ?? ch).join(''));
}
const EMPTY = { t: [], x: 0, y: 0 };
// ---------------------------------------------------------------------------
// S の大きさ
// ---------------------------------------------------------------------------
const WINGS_S_FRONT = {
...
function shiftWings(w, dx, dy) {
    const sh = (p) => ({ t: p.t, x: p.x + dx, y: p.y + dy });
...
function buildBat(K, b, p, dir, v) {
    ...
    if ((flags & F_FANG) && !open)
        over(b, bx + V.fangs.x, by + V.fangs.y, V.fangs.t, map);
    // 手前の翼（横向き）・体を包んだ翼（眠り）
    if (dir === 'E')
        runs(b, bx + w.x, by + w.y, w.t, map);
    else if (fold)
