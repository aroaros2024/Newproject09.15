...
import { clamp, heighten, rnd, shades } from './rat.js';
// ---------------------------------------------------------------------------
// 色
// ---------------------------------------------------------------------------
function palOf(v) {
...
const AX = 16;
const AY = 37;
function put(b, x, y, t, map) {
    runs(b, x, y, t, map);
}
function eye(b, x, y, eyes, map) {
    const e = rnd(eyes);
    if (e <= 0)
 ...
 */
function leg(b, x, t, dx, lift, reach, map) {
    let rows = lift > 0 ? heighten(t, 2, -lift) : t;
    if (reach > 0)
        rows = heighten(rows, 0, reach);
    runs(b, x + dx, AY - rows.length + 1 - lift, rows, map);
}
function buildDog(b, p, dir, v) {
    const map = palOf(v);
    const bob = rnd(p.bob);
    ...
    const sL = p.legL;
    const sR = p.legR;
    const swing = (s) => clamp(rnd(s * 1.5), -2, 2);
    const lift = (s) => (s > 0.5 ? 1 : 0);
    const drop = lie ? 8 : 0;
    ...
    if (dir === 'S') {
        ...
        if (lie) {
            ...
            for (const [ex, ey] of S_EYES)
                eye(b, cx + S_HEAD.x + ex, hy + ey, p.eyes, map);
