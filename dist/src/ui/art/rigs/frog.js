 ...
 */
function orb(b, cx, cy, rx, ry, v, m, shift = 0, clipY = 999) {
...
function limb(b, x0, y0, x1, y1, v, m) {
    if (Math.abs(x1 - x0) >= Math.abs(y1 - y0))
        line(b, x0, y0 + 1, x1, y1 + 1, mat(v, m, 2));
    else
        line(b, x0 + 1, y0, x1 + 1, y1, mat(v, m, 2));
...
function eye(b, x, y, v, big, state, flip) {
...
function drawFront(b, p, v, c, back) {
    ...
    if (p.mouth >= 0.5) {
        ...
        for (let dy = 0; dy <= open * c.k; dy++) {
            ...
            for (let x = -w; x <= w; x++)
                b.set(cx + x, my + dy, dy === 0 ? m : mat(v, 'mouth', 1));
        }
        const tw = Math.max(1, (ox >> 1));
        for (let x = -tw; x <= tw; x++)
            b.set(cx + x, my + open * c.k - (c.k > 1 ? 1 : 0), mat(v, 'tongue', 3));
...
function drawSide(b, p, v, c) {
    ...
    // 6. いぼ
    if (c.warts) {
        for (const [wx, wy] of WARTS_SIDE)
            wart(b, backX + R(wx * c.backRx), backY + R(wy * c.backRy), v);
    }
    // 7. 口と舌
...
function drawFrog(b, p, dir, v, c) {
    if (dir === 'E')
        drawSide(b, p, v, c);
    else
        drawFront(b, p, v, c, dir === 'N');
}
// ---------------------------------------------------------------------------
// 動き
// ---------------------------------------------------------------------------
