...
const inside = (o, x, y) => y >= o.top && y <= o.base && x >= o.x0[y] && x <= o.x1[y];
function shapeBody(o, s) {
    o.x0.fill(1);
    o.x1.fill(0);
    ...
    const shear = (y) => R((s.lean * (s.base - y)) / span);
...
const LY = -0.62;
const LZ = 0.6;
function shadeBody(b, o, v, s) {
    const body = (step) => mat(v, 'body', Math.max(1, step - s.dark));
    for (let y = o.top; y <= o.base; y++) {
        ...
        for (let x = x0; x <= x1; x++) {
            ...
            if (s.ditherEdge && ((x + y) & 1) === 0) {
                if (step === 3 && d < s.mid + 0.05)
                    step = 2;
                else if (step === 4 && d < s.hi + 0.04)
                    step = 3;
            }
            b.set(x, y, body(step));
        }
    ...
    // 底：床に触れる行は一番暗い
    for (let x = o.x0[o.base]; x <= o.x1[o.base]; x++)
        b.set(x, o.base, body(1));
    ...
    for (let y = o.dome + 1; y < o.base - 1; y++) {
        ...
        for (let x = x0 + 1; x < x1; x++) {
            ...
            if (edge && !deep)
                b.set(x, y, mat(v, 'inner', nx + ny > 0.95 ? s.innerHi : s.innerStep));
        }
    }
}
const EYES_S = {
 ...
 */
function drawFace(b, v, cx, eyeY, gap, eyes, mouth, mouthY, mirrorRight = true) {
    const map = {
        e: mat(v, 'eye', 0),
        f: mat(v, 'eye', 1),
        g: mat(v, 'glint', 5),
...
const eyeKey = (p) => p.eyes >= 1.5 ? (p.mouth > 0.5 ? 'hurt' : 'sleep') : p.eyes >= 0.5 ? 'squint' : 'open';
const mouthKey = (p, mean) => p.mouth >= 1.5 ? 'wide' : p.mouth >= 0.5 ? 'open' : mean ? 'mean' : 'shut';
const CROWN_H = 12;
const body = makeBody();
function drawSlime(b, p, v, z) {
    ...
    if (flags & F_BUBBLE) {
        ...
        if (flags & F_MEAN)
            b.set(bx + 3, by - 1, mat(v, 'inner', innerHi));
    }
    ...
    if (z.large) {
        ...
        drawFace(b, v, cx, eyeY, 6, eyes, mouth, Math.min(eyeY + 7, base - 1 - mouth.length), eyes !== EYES_L.open);
    }
    else {
        ...
        drawFace(b, v, cx, eyeY, mean ? 1 : 2, eyes, mouth, Math.min(eyeY + 4, base - 1 - mouth.length), eyes !== EYES_S.open);
    }
    if (flags & F_CROWN)
        drawCrown(b, v, z.cx + R(p.lean * k), body.dome, p.glow);
...
function drawCrown(b, v, cx, dome, glow) {
    const lit = glow > 0.5;
...
export function table(frames, fps, loop, strike) {
    return {
...
const SPARK_BIG = ['a.a.a', '.aWa.', 'aWWWa', '.aWa.', 'a.a.a'];
/** 導火線（下が口金の上。右へ曲がって上る） */
const FUSE_S = ['.f', 'f.', 'f.'];
const FUSE_M = ['..f', '.f.', '.f.', 'f..', 'f..'];
/** 怒った目（左目。右目は反転） */
const BOMB_EYES_S = {
    open: ['e..', '.ee', '.ge'],
    squint: ['e..', '.ee', '...'],
...
function drawBomb(b, p, v, z) {
    ...
    if (z.large)
        runs(b, hx, hy, ['..hhh', '.hww.', 'hw...', 'h....', 'h....'], hl);
    else
        runs(b, hx, hy, ['.hh', 'hw.', 'h..'], hl);
    // 怒った目と、への字の口
    const key = eyeKey(p);
    ...
    if (z.large)
        runs(b, capX - 3, capTop, ['.cbba.', 'cbbbad', 'dddddd'], cap);
    else
        runs(b, capX - 2, capTop, ['cbba', 'dddd'], cap);
...
const BOMB_S = { cx: 12, ay: 21, rx: 7, h: 12, large: false, ceil: 2 };
const BOMB_M = { cx: 16, ay: 37, rx: 10, h: 20, large: true, ceil: 3 };
const bomb = {
