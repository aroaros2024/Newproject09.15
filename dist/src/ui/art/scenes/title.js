...
function far(b, lights) {
    ...
    for (let y = top; y < foot; y++) {
        ...
        // 階の区切り
        if ((y - top) % 14 === 0)
            hline(b, tx - half - 1, y, half * 2 + 3, ci('steel', 2));
    }
    ...
    for (let y = top + 7; y < foot - 6; y += 14) {
        ...
        if (lit)
            lights.push({ x: tx, y, radius: 10, color: '#ffcf7a', intensity: 0.5, layer: 1, flicker: true });
    }
    // 尖塔と天輪（頂を巡る金の輪。光る）
...
function house(b, x, ground, w, lights, seed) {
    const wallH = 16;
    ...
    for (let r = 2; r < roofH; r += 3)
        hline(b, peak - Math.round(((r + 1) / roofH) * (w / 2 + 4)) + 1, ground - wallH - roofH + r, 3, ci('gold', 2));
    // 棟
    hline(b, peak - 3, ground - wallH - roofH - 1, 7, ci('earth', 0));
}
/** 灯籠 */
function lantern(b, x, ground, lights) {
...
function village(b, lights) {
    ...
    for (let y = wg - 44; y < wg; y++) {
        ...
        if (y % 5 === 0)
            hline(b, wx - half + 1, y, half * 2 - 1, ci('stone', 1));
    }
    for (let r = 0; r < 10; r++)
        hline(b, wx - r, wg - 54 + r, r * 2 + 1, r < 5 ? ci('crimson', 2) : ci('crimson', 1));
