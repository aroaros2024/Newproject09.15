...
import { DIORAMA_H, DIORAMA_W, DUSK, house, lantern, noise1, ridge, tower, trees, windmillBody, } from './common.js';
/** 夕風に舞う綿毛（金のドット） */
const FLUFF = {
...
function clouds(b) {
    const bank = (cx, cy, w, seed) => {
        ...
        for (let i = 0; i * 7 < w; i++) {
            ...
            if (hash2(i, 3, seed) % 3 !== 0)
                ellipse(b, x + 3, cy - r - 1, r, r - 1, 1);
        }
...
function far(b, lights) {
    ridge(b, (x) => Math.round(142 + 30 * noise1(x / 80, 111) - 16 * noise1(x / 25, 112)), ci('violet', 1), ci('rose', 2));
    tower(b, 356, 26, 176, lights, 1, DUSK, 1.3);
}
function mid(b, lights) {
    const hill = (x) => Math.round(180 + 8 * noise1(x / 50, 121) - 5 * noise1(x / 17, 122));
    for (let x = 0; x < DIORAMA_W; x++)
        vline(b, x, hill(x), DIORAMA_H - hill(x), ci('moss', 1));
    trees(b, 36, 131, hill, ci('moss', 1), ci('leaf', 2), (x) => x > 338 && x < 374);
    // 遠くの屋根
    for (const x of [40, 110, 300]) {
        const y = hill(x) + 1;
        for (let r = 0; r < 5; r++)
            hline(b, x - 4 - r, y - 5 + r, 9 + r * 2, ci('earth', 2));
        b.set(x, y - 1, ci('gold', 5));
        lights.push({ x, y: y - 1, radius: 7, color: '#ffc46a', intensity: 0.3, layer: 2, flicker: true });
    }
}
/** 蔵（白壁・黒瓦・家紋） */
function storehouse(b, x, ground, w) {
    const wallH = 22;
...
/** 広場（石畳・建物） */
function plaza(b, lights) {
    const ground = (x) => Math.round(198 + 3 * noise1(x / 45, 141));
    for (let x = 0; x < DIORAMA_W; x++) {
        const g = ground(x);
    ...
    // 花の鉢
    for (const [x, c] of [[200, 'rose'], [352, 'gold']]) {
...
function near(b) {
    const top = (x) => Math.round(228 + 7 * noise1(x / 26, 161));
    for (let x = 0; x < DIORAMA_W; x++) {
        if (x > 200 && x < 330)
            continue; // 広場の手前は空ける
        const t = top(x);
        ...
        if (hash2(x, 0, 162) % 3 === 0) {
            ...
            if (hash2(x, 2, 162) % 5 === 0)
                b.set(x, t - h - 1, hash2(x, 3, 162) % 2 ? ci('rose', 4) : WHITE);
        }
    }
}
/**
 ...
 */
export function buildTownDiorama(time = 'dusk') {
