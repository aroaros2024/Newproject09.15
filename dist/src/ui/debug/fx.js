...
import { PAL_CSS, RAMP_EMBER, RAMP_HEAL, RAMP_MAGIC, RAMP_DUST, ci, } from '../art/palette.js';
...
import { KIND_HD, KIND_PIXEL, ParticlePool, } from '../gfx-core/particles.js';
...
/** 松明（火の粉の出どころ） */
const TORCHES = [[60, 14], [230, 14], [372, 14], [152, 84]];
/** 光の筋（光の粒の出どころ）：左上から右下へ斜めの帯 */
const SHAFT = { x0: 190, y0: 20, dx: 70, w: 44, h: 200 };
const DUST = {
...
function particlesScene(params) {
    ...
    const gg = ctx2d(glow);
    const glowXf = { scale: ART_SCALE / 4, offX: 0, offY: 0 };
    const screenXf = { scale: ART_SCALE, offX: 0, offY: 0 };
    ...
    const tick = () => {
        ...
        if (stress)
            pool.burst(STRESS_PX, 60 + rng.float() * 300, 40 + rng.float() * 180);
        field.tick();
    ...
    const draw = () => {
        ...
        ag.drawImage(bg, 0, 0);
        if (spriteAlive)
            ag.drawImage(spriteCanvas, spriteX, spriteY);
        if (trail) {
        ...
        if (showField)
            drawFieldOverlay(g, field, flowOut);
        let sum = 0;
        ...
        label(g, `粒 ${pool.count} / ${pool.capacity}（環境 ${pool.ambientCount}）  `
            + `1 刻み ${(n ? sum / n : 0).toFixed(3)} ms（直近 ${n} 刻みの平均）  t=${field.time.toFixed(1)}s`, 12, H - 28);
    };
    const frame = (now) => {
        if (lastT < 0)
            lastT = now;
...
function paintBackground(g) {
    ...
    for (let x = 0; x < ART_W; x += 16)
        g.fillRect(x, 0, 1, ART_H);
    for (let y = 0; y < ART_H; y += 16)
        g.fillRect(0, y, ART_W, 1);
...
function drawFieldOverlay(g, field, out) {
    g.save();
    g.lineWidth = 1.5;
...
function curlScene() {
    ...
    times.forEach((t, n) => {
        ...
        label(g, `t = ${t} 秒  ${field.cols}×${field.rows} マス（格子 ${field.nx}×${field.ny} 点・${field.cellSize} ドット間隔）`, ox, 14);
        label(g, `平均の速さ ${meanSpeed(field, worldW, worldH).toFixed(1)} ドット/秒`, ox, top + worldH * k + 12, '#e8d6a0', 13);
    });
    label(g, '背景 = ψ（藍 < 0 < 橙）  細い線 = 流線（ψ の等高線に沿う）  矢印 = 2 マスごとの速度（1.2 秒ぶんの長さ）', gap, top + worldH * k + 40, '#a9a391', 13);
}
function meanSpeed(field, w, h) {
...
function drawCurlPanel(g, field, ox, oy, k) {
