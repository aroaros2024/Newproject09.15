...
export class FxSystem {
        ...
        for (const e of events) {
            switch (e.t) {
                ...
                case 'damage': {
                    ...
                    if (p)
                        this.addPopup(String(e.amount), 'damage', p);
                    break;
                }
                case 'heal': {
                    const p = posOf(e.actorId);
                    if (p)
                        this.addPopup(`+${e.amount}`, 'heal', p);
                    break;
                }
                case 'defeat': {
                    const v = this.views.get(e.actorId);
    ...
    update(dt, positions) {
        if (this.holdMs > 0)
            this.holdMs = Math.max(0, this.holdMs - dt);
        ...
        for (const [id, target] of positions) {
            ...
            if (Math.abs(v.x - target.x) > 3 || Math.abs(v.y - target.y) > 3) {
                ...
                v.x += (target.x - v.x) * k;
                v.y += (target.y - v.y) * k;
                if (Math.abs(v.x - target.x) < 0.01)
                    v.x = target.x;
                if (Math.abs(v.y - target.y) < 0.01)
                    v.y = target.y;
            }
            ...
            if (v.fade > 0)
                v.fade = Math.min(1, v.fade + dt / Math.max(1, ANIM.defeat * this.speedScale));
        }
        ...
        if (this.banner) {
            this.banner.life += dt;
            if (this.banner.life >= this.banner.maxLife)
                this.banner = null;
        }
        if (this.shake > 0) {
...
/** タイル座標 → 画面のピクセル座標 */
export const tileToPx = (t) => t * TILE;
