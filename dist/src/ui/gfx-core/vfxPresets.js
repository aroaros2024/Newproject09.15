 ...
 */
import { RAMP_DUST, RAMP_EMBER, RAMP_GOLD, RAMP_HEAL, RAMP_ICE, RAMP_MAGIC, RAMP_POISON, RAMP_SMOKE, RAMP_THUNDER, RAMP_WATER, } from '../art/palette.js';
import { KIND_HD, KIND_PIXEL, TONE_EMBER, TONE_GOLD, TONE_HEAL, TONE_MAGIC, TONE_WATER, TONE_WHITE, } from './particles.js';
const TAU = Math.PI * 2;
...
const CRIT_PX = { ...SPARKS, count: 30, speed: [90, 220], spread: 1.2, life: [0.3, 0.7] };
const CRIT_HD = { ...HIT_GLOW, count: 5, size: 11, life: [0.15, 0.3] };
const HEAL_PX = {
...
export const statusRamp = (id) => STATUS_RAMP[id] ?? RAMP_MAGIC;
/** 8 方向の向き（0 = 北から時計回り） */
const DX = [0, 1, 1, 1, 0, -1, -1, -1];
const DY = [-1, -1, 0, 1, 1, 1, 0, -1];
/** 強さや色を変える時の使い回しの入れ物（書き写してから吹き出す） */
const scratchSpeed = [0, 0];
const scratch = { ...SPARKS, speed: scratchSpeed };
 ...
 */
export function fireVfx(pool, preset, ax, ay, opts) {
    const r = VFX[preset];
    const cx = ax + 8;
    ...
    for (const b of r.bursts) {
        ...
        if (p !== 1 || recolor) {
            ...
            if (preset === 'statusApplied')
                scratch.ramp = statusRamp(opts.tag);
            if (preset === 'trapGas' && opts.tag !== '')
                scratch.ramp = statusRamp(opts.tag);
            spec = scratch;
        }
...
//# sourceMappingURL=vfxPresets.js.map