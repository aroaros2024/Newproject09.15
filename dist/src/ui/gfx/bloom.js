...
export class Bloom {
    E = new Surface(EMISSIVE_W, EMISSIVE_H, true, true);
    b1 = new Surface(EMISSIVE_W / 2, EMISSIVE_H / 2, true, true);
    b2 = new Surface(EMISSIVE_W / 4, EMISSIVE_H / 4, true, true);
    /** 1/4 の加算の紙（ブルームの段と光の筋を重ねる） */
    Q = new Surface(EMISSIVE_W, EMISSIVE_H, true, true);
    /** 画面の半分の大きさの加算の紙（Q を滑らかに広げたもの。画面へは最近傍の 2 倍で貼る） */
    H = new Surface(SCREEN_W / 2, SCREEN_H / 2, true, true);
    /**
