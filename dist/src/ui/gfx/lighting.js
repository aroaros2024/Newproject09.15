...
import { LightField, bakeLightSprite, fillTileImages, levelsFromLook, quantIndex, } from '../gfx-core/lightModel.js';
...
export class Surface {
     ...
     */
    constructor(w, h, opaque = false, smooth = false) {
...
export class LightRenderer {
    ...
    div = 0;
    L = new Surface(1, 1, true, true);
    LA = new Surface(1, 1, true, true);
    /** L を画用紙の大きさにしたもの（地形・床の物に乗算する） */
    La = new Surface(ART_W, ART_H, true, true);
    tileBase = null;
    tileMask = null;
    unexp = null;
    imgBase = null;
    ...
    u32Base = new Uint32Array(0);
    u32Mask = new Uint32Array(0);
    u32Unexp = new Uint32Array(0);
    mapW = 0;
    mapH = 0;
    version = Number.NaN;
    tilesDirty = true;
    ...
    syncTiles(scene) {
        ...
        if (!this.tilesDirty && scene.tileVersion === this.version)
            return false;
        this.version = scene.tileVersion;
        this.tilesDirty = false;
        scene.writeTileStates(this.field.states);
        fillTileImages(this.field.states, w * h, this.field.levels, this.u32Base, this.u32Mask, this.u32Unexp);
        this.tileBase.g.putImageData(this.imgBase, 0, 0);
        this.tileMask.g.putImageData(this.imgMask, 0, 0);
        this.unexp.g.putImageData(this.imgUnexp, 0, 0);
        return true;
    }
    /**
    ...
    build(camX, camY) {
        ...
        for (let i = 0; i < list.count; i++) {
            ...
            if (s <= 0.004 || x + r < camX || y + r < camY || x - r > x1 || y - r > y1)
                continue;
    ...
    sprite(r, g, b) {
        ...
        if (!this.spriteImg)
            this.spriteImg = new ImageData(LIGHT_SPRITE, LIGHT_SPRITE);
        bakeLightSprite(this.spriteImg.data, LIGHT_SPRITE, ((key >> 8) & 15) / 15, ((key >> 4) & 15) / 15, (key & 15) / 15);
    ...
    debugBuffers(out) {
        if (this.tileBase)
            out.push({ name: 'tileBase', c: this.tileBase.c });
        if (this.tileMask)
            out.push({ name: 'tileMask', c: this.tileMask.c });
        if (this.unexp)
            out.push({ name: 'unexplored', c: this.unexp.c });
...
function blackOutside(g, camX, camY, mw, mh) {
    const x1 = camX + ART_W;
    const y1 = camY + ART_H;
    if (camX >= 0 && camY >= 0 && x1 <= mw && y1 <= mh)
        return;
    g.fillStyle = '#000';
    if (camX < 0)
        g.fillRect(camX, camY, -camX, ART_H);
    if (x1 > mw)
        g.fillRect(mw, camY, x1 - mw, ART_H);
    if (camY < 0)
        g.fillRect(camX, camY, ART_W, -camY);
    if (y1 > mh)
        g.fillRect(camX, mh, ART_W, y1 - mh);
}
// ---------------------------------------------------------------------------
// キャラの色付け
// ---------------------------------------------------------------------------
 ...
 */
export class SceneActorLighter {
    field;
    /** 画質「高」のときだけ true（compositor が決める） */
    rim = false;
    b = null;
    scratch = new Surface(ACTOR_SCRATCH, ACTOR_SCRATCH);
    rimBuf = new Surface(ACTOR_SCRATCH, ACTOR_SCRATCH);
    tmp = new Float32Array(3);
    rimTmp = new Float32Array(6);
    constructor(field) {
        this.field = field;
    }
    ...
    drawFrame(f, footX, footY, alpha = 1, lit = true) {
        this.drawImage(f.src, f.sx, f.sy, f.w, f.h, footX - f.ax, footY - f.ay, footX, footY, alpha, lit);
    }
    drawImage(src, sx, sy, w, h, dx, dy, footX, footY, alpha = 1, lit = true) {
        const b = this.b;
        if (!b || w <= 0 || h <= 0 || alpha <= 0)
            return;
        ...
        if (this.rim)
            this.drawRim(b, src, sx, sy, w, h, dx, dy, footX, footY, alpha);
        b.globalAlpha = 1;
    }
    /** 光の来る側の縁だけを光の色で加算する */
    drawRim(b, src, sx, sy, w, h, dx, dy, footX, footY, alpha) {
        const t = this.rimTmp;
