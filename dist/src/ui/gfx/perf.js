...
export class PerfMeter {
    ...
    np;
    data;
    totals = new Float32Array(RING);
    intervals = new Float32Array(RING);
    cur;
    scratch = new Float32Array(RING);
    head = 0;
    count = 0;
    t0 = 0;
    last = 0;
    ...
    mark(pass, flush) {
        ...
        if (this.sync >= 1 && g) {
            ...
            if (this.np > 0)
                this.cur[this.np - 1] += t - this.last;
            this.last = t;
        }
        const h = this.head;
    ...
    stat(read) {
        ...
        view.sort();
        const at = (p) => (n === 0 ? 0 : view[Math.min(n - 1, Math.floor(p * n))]);
