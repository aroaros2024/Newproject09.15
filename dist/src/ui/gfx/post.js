...
import { GRADE_FULL, GRADE_TINT } from './quality.js';
...
export class PostFx {
    shaft = new Surface(SHAFT_W, SHAFT_H, false, true);
    fog = new Surface(FOG_W, FOG_H, false, true);
    vignette = new Surface(VIG_W, VIG_H, false, true);
    vignetteHalf = new Surface(HALF_W, HALF_H, false, true);
    /** 霧 2 枚を重ねる紙（画面の半分の大きさ、上の部分だけ） */
    fogBuf = new Surface(HALF_W, FOG_BUF_H, false, true);
    look = null;
    fadeCss = '#000';
     ...
     */
    drawShafts(g, f, sx, sy) {
        const look = this.look;
        if (!look)
            return;
        const n = Math.min(3, look.shafts.count | 0);
        if (n <= 0 || look.shafts.alpha <= 0)
            return;
        const t = f.time;
    ...
    drawFog(g) {
        if (!this.look || this.look.fog.alpha <= 0)
            return;
    ...
    desaturateArt(a, w, h) {
        const look = this.look;
        if (!look || look.grade.desaturate <= 0)
            return;
    ...
    drawGrade(g, q, w, h) {
        const look = this.look;
        if (!look || q.grade < GRADE_TINT || look.grade.tintAlpha <= 0)
            return;
...
function tiledNoise(x, y, cell, period, seed) {
    const gx = x / cell;
    const gy = y / cell;
    ...
    const fy = gy - iy;
    const n = period / cell;
    const h = (cx, cy) => hash2(((cx % n) + n) % n, cy, seed) / 4294967296;
...
function bakeVignette(s, strength) {
    put(s, (x, y) => {
