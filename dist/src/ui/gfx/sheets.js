 ...
 */
import { ANIM_IDS, TIER_BOX, animOf, facing, getRig, getSpecies, renderFrame, } from '../art/rig.js';
 ...
 */
export function packKey(species, anim, dir8, frame, fx) {
...
export const animIndex = (id) => ANIM_IDS.indexOf(id);
// ---------------------------------------------------------------------------
// 画素の仕上げ（DOM 無し）
// ---------------------------------------------------------------------------
 ...
 * 描かれている画素の数を返す。
 */
export function applyFx(rgba, stride, w, h, fx, emissive) {
    let painted = 0;
    for (let y = 0; y < h; y++) {
        const row = y * stride;
        for (let x = 0; x < w; x++) {
            ...
            if (fx === FX_FLASH) {
                ...
                if (BAYER4[(y & 3) * 4 + (x & 3)] < DISSOLVE_CUT[fx - FX_DISSOLVE_25])
                    c = 0;
            }
            else if (fx === FX_EMISSIVE) {
                if (!emissive || emissive[i] !== 1)
                    c = 0;
            }
 ...
 */
export function legacyToRGBA(sprite, scale) {
    ...
    for (const [ch, css] of Object.entries(sprite.palette))
        colors.set(ch, cssToRGBA(css));
...
export class CanvasSheetBackend {
    img = null;
    ...
    upload(page, rgba, stride, w, h, x, y) {
        if (!this.img) {
        ...
        for (let yy = 0; yy < h; yy++) {
            ...
            for (let xx = 0; xx < w; xx++)
                u[dst + xx] = rgba[src + xx];
        }
        page.g.putImageData(this.img, x, y, 0, 0, w, h);
    }
    release(page) {
        const c = page.src;
        c.width = 1;
...
export class SheetCache {
    ...
    byId = new Map();
    ref = {
        src: null, sx: 0, sy: 0, w: 0, h: 0, ax: 0, ay: 0,
    };
    clock = 0;
    work = new Uint32Array(WORK * WORK);
    emis = new Uint8Array(WORK * WORK);
    ...
    index(id, legacyBoss = false) {
        ...
        let rec;
        const base = {
            id, index: this.recs.length, slots: new Map(), pages: [],
        ...
        if (def && rig) {
            ...
            for (let a = 0; a < ANIM_IDS.length; a++) {
                ...
                const use = own ? a : 0;
                rec.animMap[a] = use;
                const d = animOf(rig, ANIM_IDS[use]);
    ...
    frameCount(species, anim) {
        const rec = this.recs[species];
        if (!rec || anim < 0 || anim >= ANIM_IDS.length)
            return 1;
        return rec.animFrames[anim];
    }
    /** 動きの定義（速さ・当たりのコマ）。旧来の絵は null */
    animDef(species, anim) {
        const rec = this.recs[species];
        if (!rec || !rec.rig || anim < 0 || anim >= ANIM_IDS.length)
            return null;
     ...
     */
    frame(species, anim, dir8, frame, fx = FX_NORMAL) {
        const rec = this.recs[species];
        if (!rec)
            return null;
        ...
        if (!slot)
            slot = this.bake(rec, key, a, d, fr, f);
        if (slot.empty)
            return null;
        const ref = this.ref;
    ...
    bake(rec, key, a, d, fr, fx) {
        ...
        if (painted === 0 && fx === FX_EMISSIVE) {
            const empty = { page: EMPTY_PAGE, sx: 0, sy: 0, empty: true };
            rec.slots.set(key, empty);
            return empty;
        }
        if (rec.pages.length === 0 || rec.used >= CELLS)
            this.addPage(rec);
        const cell = rec.used++;
        ...
        const slot = { page, sx, sy, empty: false };
    ...
    enforceCap(current) {
        while (this.bytes > this.capBytes) {
            let victim = null;
            for (const rec of this.recs) {
                if (rec === current || rec.bytes === 0)
                    continue;
                if (!victim || rec.lastUse < victim.lastUse)
                    victim = rec;
            }
            if (!victim)
                return;
