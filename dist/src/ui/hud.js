...
import { drawBar, drawPanel, drawText, ellipsize, roundRect } from './draw.js';
...
export class Hud {
    ...
    drawStatus(g, p, frame) {
        ...
        drawText(g, toNext > 0 ? `次まで ${toNext}` : '最大レベル', r.x + r.w - 20, r.y + 30, { size: 13, align: 'right', color: UI.textDim });
        // HP
    ...
    drawStatusIcons(g, p) {
        const active = p.statuses.filter((s) => s.turns !== 0);
        if (active.length === 0)
            return;
