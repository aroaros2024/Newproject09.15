...
import { drawPanel, drawText, wrapText } from './draw.js';
...
export class MessageLog {
    ...
    skipTyping() {
        const last = this.entries[this.entries.length - 1];
        if (last)
            this.revealed = this.displayText(last).length;
    }
    displayText(e) {
    ...
    draw(g, now) {
        ...
        for (let i = this.entries.length - 1; i >= 0 && lines.length < VISIBLE_LINES + 2; i--) {
            ...
            if (i === this.entries.length - 1)
                text = text.slice(0, Math.floor(this.revealed));
