 ...
 */
import { chebyshev } from '../core/geom.js';
import { at } from '../dungeon/tilemap.js';
import { drawText, roundRect } from './draw.js';
...
/** 地形をまるごと描く（キャッシュ用） */
function paintTerrain(g, map, theme) {
...
function paintTile(g, t, x, y, map, theme) {
    const px = x * TILE;
    const py = y * TILE;
    if (t.kind === 'wall') {
...
export class DungeonRenderer {
    ...
    drawTerrain(g, ctx) {
        ...
        g.drawImage(canvas, -camera.x, -camera.y);
    ...
    drawGround(g, ctx, floorItems) {
        ...
        for (let y = y0; y <= y1; y++) {
            for (let x = x0; x <= x1; x++) {
                const t = at(map, x, y);
                if (!t?.trap || !t.trap.revealed || t.trap.used)
                    continue;
                if (!ctx.revealAll && !t.explored)
                    continue;
                const p = { x, y };
        ...
        for (const item of floorItems) {
            ...
            if (item.shopPrice > 0 && t.visible) {
                drawText(g, `${item.shopPrice}G`, item.pos.x * TILE - camera.x + TILE / 2, item.pos.y * TILE - camera.y - 2, {
    ...
    /** アクター 1 体 */
    drawActor(g, ctx, view, spriteId, opts = {}) {
        const { camera } = ctx;
...
function drawSpriteAt(g, id, pos, camera, fallback, yOffset = 0) {
