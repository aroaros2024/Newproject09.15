...
import { PAL_CSS } from '../art/palette.js';
...
export function elementOfHex(hex) {
    ...
    if (r > 200 && g < 150 && b < 120)
        return 'fire';
    if (r > 200 && g > 190 && b < 140)
        return 'thunder';
    if (b > 200 && r < 150)
        return g > 180 ? 'ice' : 'water';
    if (g > 180 && r < 170 && b < 160)
        return 'poison';
    if (r > 150 && b > 180)
        return 'magic';
...
export class VfxLayer {
    ...
    onSfx;
    onDeath;
    popups = new PopupLayer();
    // --- 飛び道具（SoA）
    pFx = new Float32Array(PROJ_CAP);
    pFy = new Float32Array(PROJ_CAP);
    pTx = new Float32Array(PROJ_CAP);
    pTy = new Float32Array(PROJ_CAP);
    pT = new Float32Array(PROJ_CAP);
    pMs = new Float32Array(PROJ_CAP);
    pKey = new Array(PROJ_CAP).fill('');
    pKind = new Array(PROJ_CAP).fill('item');
    pN = 0;
    // --- ビーム
    bFx = new Float32Array(BEAM_CAP);
    bFy = new Float32Array(BEAM_CAP);
    bTx = new Float32Array(BEAM_CAP);
    bTy = new Float32Array(BEAM_CAP);
    bT = new Float32Array(BEAM_CAP);
    bMs = new Float32Array(BEAM_CAP);
    bColor = new Array(BEAM_CAP).fill('#ffffff');
    bEl = new Array(BEAM_CAP).fill('magic');
    bN = 0;
    // --- 一瞬の光（世界のドット）
    lX = new Float32Array(LIGHT_CAP);
    lY = new Float32Array(LIGHT_CAP);
    lR = new Float32Array(LIGHT_CAP);
    lI = new Float32Array(LIGHT_CAP);
    lT = new Float32Array(LIGHT_CAP);
    lMs = new Float32Array(LIGHT_CAP);
    lColor = new Array(LIGHT_CAP).fill('#ffffff');
    lN = 0;
    // --- 画面の状態
    shakePx = 0;
    shakeMs = 0;
    ...
    setFloorCard(dungeonName, depthLabel) {
    ...
    burst(preset, tileX, tileY, opts) {
        ...
        const r = VFX[preset];
        if (r.light)
            this.light(tileX, tileY, opts.color || r.light.color, r.light.radius, r.light.intensity, r.light.ms);
        if (r.shake)
            this.shake(r.shake[0] * Math.min(2, opts.power || 1), r.shake[1]);
        if (preset === 'death' && opts.tag)
            this.onDeath(tileX, tileY, opts.tag);
    }
    ...
    light(tileX, tileY, color, radius, intensity, durationMs) {
        ...
        if (i < 0) {
            // 一番弱っている光を差し替える
            let best = 0;
            for (let k = 1; k < LIGHT_CAP; k++)
                if (this.lT[k] / this.lMs[k] > this.lT[best] / this.lMs[best])
                    best = k;
            i = best;
        }
    ...
    tick(stepMs) {
        ...
        if (this.hitstopLeft > 0)
            this.hitstopLeft = Math.max(0, this.hitstopLeft - stepMs);
        // 飛び道具とビーム
        for (let i = 0; i < this.pN;) {
            this.pT[i] += stepMs;
            if (this.pT[i] >= this.pMs[i])
        ...
        if (this.fadeDir > 0)
            this.fade = Math.min(1, this.fade + stepMs / 220);
    ...
    oldestBeam() {
        let best = 0;
        for (let k = 1; k < BEAM_CAP; k++)
            if (this.bT[k] > this.bT[best])
                best = k;
        return best;
    }
    removeProjectile(i) {
        ...
        if (i === last)
            return;
        this.pFx[i] = this.pFx[last];
        this.pFy[i] = this.pFy[last];
        this.pTx[i] = this.pTx[last];
        this.pTy[i] = this.pTy[last];
        this.pT[i] = this.pT[last];
        this.pMs[i] = this.pMs[last];
        this.pKey[i] = this.pKey[last];
        this.pKind[i] = this.pKind[last];
    }
    removeBeam(i) {
        const last = --this.bN;
        if (i === last)
            return;
        this.bFx[i] = this.bFx[last];
        this.bFy[i] = this.bFy[last];
        this.bTx[i] = this.bTx[last];
        this.bTy[i] = this.bTy[last];
        this.bT[i] = this.bT[last];
        this.bMs[i] = this.bMs[last];
        this.bColor[i] = this.bColor[last];
        this.bEl[i] = this.bEl[last];
    }
    removeLight(i) {
        const last = --this.lN;
        if (i === last)
        ...
        this.lR[i] = this.lR[last];
        this.lI[i] = this.lI[last];
        this.lT[i] = this.lT[last];
        this.lMs[i] = this.lMs[last];
    ...
    paintHD(g, camX, camY, tilePx = 16 * ART_SCALE) {
        ...
        this.popups.draw(g, ox, oy, tilePx);
    }
...
export { ELEMENT_ORB };
//# sourceMappingURL=vfxLayer.js.map