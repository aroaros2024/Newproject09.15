...
import { getRig, getSpecies, renderFrame } from '../src/ui/art/rig.js';
...
/** 種の 1 コマを描いた画用紙 */
function frame(id, anim = 'idle', dir8 = 4, f = 0) {
    const def = getSpecies(id);
    const rig = getRig(def.rig);
    const b = new PixBuf(rig.tier === 'S' ? 24 : rig.tier === 'M' ? 32 : 64, rig.tier === 'S' ? 24 : rig.tier === 'M' ? 40 : 64);
    renderFrame(b, rig, def.variant, anim, dir8, f);
    return b;
}
const emissiveCount = (b) => {
    let n = 0;
    for (const c of b.px)
        if (EMISSIVE[c])
            n++;
...
test('プルンは光らない。ボムプルンは導火線の火花だけが光る', () => {
    for (const id of SLIMES)
        assert.equal(emissiveCount(frame(id)), 0, `${id} に光る色がある`);
    for (const id of BOMBS) {
        ...
        for (let y = r.y + 6; y < b.h; y++) {
            for (let x = 0; x < b.w; x++)
                assert.ok(!EMISSIVE[b.get(x, y)], `${id} の体 (${x},${y}) が光っている`);
        }
...
test('デカガエルはアマガエルより大きく、舌を伸ばす（横・手前・奥）', () => {
    ...
    const tongue = (b) => b.px.filter((c) => c >= ci('rose', 0) && c <= ci('rose', 5)).length;
...
//# sourceMappingURL=art-slime.test.js.map