...
import { PAL_HEX, PAL_RGBA, PAL_SIZE, RAMPS, STEPS, WHITE, ci, isEmissive, luminance, outlineOf, rampOf, shiftStep, stepOf, } from '../src/ui/art/palette.js';
import { PixBuf, bbox, countPainted, distinctColors, ellipse, line, mirrorX, rect, runs, } from '../src/ui/art/pixbuf.js';
...
test('どの階調も暗い段から明るい段へ単調に明るくなる', () => {
    for (let r = 0; r < RAMPS.length; r++) {
        for (let s = 1; s < STEPS; s++) {
            assert.ok(luminance(ci(r, s)) > luminance(ci(r, s - 1)), `${RAMPS[r]} の ${s - 1} → ${s} が暗くなっている`);
        }
    }
});
...
test('ImageData 用の値は 0 番だけが透明', () => {
    ...
    for (let i = 1; i < PAL_SIZE; i++)
        assert.equal(PAL_RGBA[i] >>> 24, 255);
});
...
test('楕円は左右対称', () => {
    ...
    for (let y = 0; y < 15; y++) {
        for (let x = 0; x < 21; x++)
            assert.equal(b.get(x, y), b.get(20 - x, y), `(${x},${y})`);
    }
...
//# sourceMappingURL=art-toolkit.test.js.map