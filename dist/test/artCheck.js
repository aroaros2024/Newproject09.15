...
import { ANIM_IDS, TIER_BOX, animOf, getRig, getSpecies, isRamp, renderFrame, } from '../src/ui/art/rig.js';
...
export function checkSpecies(id, o = {}) {
        ...
        for (let dir8 = 0; dir8 < 8; dir8++) {
            ...
            for (let f = 0; f < a.frames; f++) {
                ...
                for (let i = 0; i < b.px.length; i++)
                    assert.ok(b.px[i] < PAL_SIZE, `${where}：パレットの外の色`);
                ...
                renderFrame(again, rig, def.variant, anim, dir8, f);
...
export function checkDistinct(ids) {
    const seen = new Map();
    for (const id of ids) {
        const def = getSpecies(id);
        const rig = getRig(def.rig);
...
//# sourceMappingURL=artCheck.js.map