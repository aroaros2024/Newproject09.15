...
/** 乱数の期待値でのダメージ */
const typicalDamage = (atk, def) => expectedDamage(atk, def);
test('ボスは想定装備で殴り切れる', () => {
    for (const d of allDungeons()) {
        ...
        for (const entry of d.bosses) {
            ...
            assert.ok(hits <= 40, `${d.id} の ${m.name} は想定装備(攻撃力 ${atk})で ${hits} 発必要。硬すぎる`);
            assert.ok(hits >= 6, `${d.id} の ${m.name} は想定装備(攻撃力 ${atk})で ${hits} 発で沈む。柔らかすぎる`);
        }
    }
});
test('ボスに殴り殺されるまでに、立て直す余地がある', () => {
    for (const d of allDungeons()) {
        ...
        for (const entry of d.bosses) {
            ...
            assert.ok(turns >= 3, `${d.id} の ${m.name} は想定装備(守り ${pdef}/HP ${php})を ${turns.toFixed(1)} ターンで倒す。即死すぎる`);
            assert.ok(turns <= 30, `${d.id} の ${m.name} は想定装備(守り ${pdef}/HP ${php})相手に ${turns.toFixed(1)} ターンかかる。緊張感が無い`);
        }
    }
});
test('雑魚は想定装備で 10 発以内に倒せる', () => {
    ...
    for (const d of allDungeons()) {
        ...
        for (const e of d.monsters) {
            ...
            assert.ok(hits <= 10, `${d.id} ${e.from}F の ${m.name} は想定装備(攻撃力 ${atk})で ${hits} 発必要`);
        }
    }
});
    ...
    for (const d of allDungeons()) {
        for (const b of d.bosses) {
            for (let seed = 0; seed < 30; seed++) {
                ...
                world.drainEvents();
                assert.equal(world.map.rooms.some((r) => r.monsterHouse !== null), false, `${d.id} ${b.depth}F seed${seed}: ボスの階にモンスターハウスがある`);
                // 取り巻きは少数。ボスを含めて 6 体を超えない
                assert.ok(world.run.monsters.length <= 6, `${d.id} ${b.depth}F seed${seed}: ボスの階に ${world.run.monsters.length} 体いる`);
                // 装備を奪う敵は、ボス戦の事故でしかない
                for (const m of world.run.monsters) {
                    const def = world.defOf(m);
                    if (def.isBoss)
                        continue;
                    assert.notEqual(def.ai, 'thief', `${d.id} ${b.depth}F seed${seed}: ボスの階に ${def.name}（盗み）がいる`);
                }
            }
        }
    }
