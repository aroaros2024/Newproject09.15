...
import { SHORTCUT_SLOTS, addKept, assignShortcut, keptItems, makeItem, shortcutOf, shortcutSlots, } from '../src/game/inventory.js';
...
test('正体を知らない杖は、持っていても枠に入らない', () => {
    ...
    assert.equal(shortcutOf(w.player, 'sleepStaff', w.run.identify.known), -1, '未識別なのに枠の番号が分かってしまう');
    // 正体が分かった瞬間に入る
...
test('保持した道具は持ち込みで選ばれ、そのまま持っていけば保持も続く', () => {
    ...
    assert.ok(w.player.inventory.some((i) => i.uid === kept[0].uid), '持ち物に無い uid を保持している');
});
