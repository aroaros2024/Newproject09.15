...
function interior(rng, cols, rows, margin) {
...
test('値ノイズ：0〜1、なめらか、種で別の揺らぎ', () => {
    ...
    for (let n = 0; n < 1000; n++) {
        ...
        if (Math.abs(valueNoise1(t, 4) - v) > 0.05)
            differ++;
    }
...
test('流れ場の発散はマスの中でちょうど 0（500 点・格子の間の位相）', () => {
    ...
    for (let k = 0; k < CURL_KEY_TICKS * 3 + 7; k++)
        field.tick();
    ...
    for (let n = 0; n < 500; n++) {
        ...
        assert.ok(Math.abs(dvx + dvy) < limit, `(${x.toFixed(3)}, ${y.toFixed(3)}) で発散 ${dvx + dvy}（上限 ${limit}）`);
...
test('同じ時刻なら同じ場（刻みで進めても、その時刻へ飛んでも）', () => {
    ...
    for (let n = 0; n < 300; n++) {
        ...
        if (Math.abs(va[0] - vd[0]) + Math.abs(va[1] - vd[1]) > 0.5)
            differ++;
    }
    // 時刻が違えば場も違う（止まっていない）
