...
import { ALL_ITEMS, validateData, getDungeon, allDungeons, allMonsters, itemsOfKind, UNIDENTIFIED_KINDS, getItem, getMonster, } from '../src/data/registry.js';
...
test('アイテムが各カテゴリに十分な数ある', () => {
    ...
    for (const [kind, n] of Object.entries(want)) {
        const got = itemsOfKind(kind).length;
...
test('装備の初期印がスロット数に収まっている', () => {
    for (const d of ALL_ITEMS) {
        if (d.kind !== 'weapon' && d.kind !== 'shield')
            continue;
...
test('クリア報酬のアイテムが存在する', () => {
    for (const d of allDungeons()) {
        if (d.reward.itemId)
            assert.doesNotThrow(() => getItem(d.reward.itemId));
...
test('序盤の階は、想定装備で 3 発は耐えられる強さに収まっている', async () => {
    ...
    const EXPECTED_SHIELD = {
        d1: 2, d2: 4, d3: 8, d4: 11, dl: 15, ex: 0,
    };
    // 潜るにつれてレベルが上がるので、想定 HP も階層で伸ばす
    const expectedHp = (depth) => START_HP + (depth - 1) * 4;
    const problems = [];
    for (const d of allDungeons()) {
        ...
        for (const depth of [1, 2, 3]) {
            ...
            for (const e of d.monsters) {
                ...
                if (hits < 3) {
                    problems.push(`${d.id} ${depth}F: ${m.name}（攻撃${m.atk}）が ${dmg} ダメージ`
                        + `＝HP${hp} を ${hits}発で削り切る`);
                }
            }
        }
...
test('ダンジョンの難易度が階層とともに上がっていく', () => {
    for (const d of allDungeons()) {
        ...
        assert.ok(late > early * 1.5, `${d.id}: 最深部(${late}) が序盤(${early}) と比べて強くなっていない`);
    }
});
