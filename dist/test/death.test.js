 ...
 * ここでは必ず攻撃で倒す。
 */
function slay(world, m, maxTurns = 400) {
    ...
    for (let i = 0; i < maxTurns && m.alive; i++) {
        ...
        for (const other of [...world.run.monsters]) {
            if (other !== m && chebyshev(other.pos, m.pos) <= 1)
                world.removeActor(other);
        }
        // 相手の真隣に立つ。立てる隣マスが無ければ倒せない
        const beside = neighbors8(world.map, m.pos).find((q) => canEnter(world.map, q.x, q.y, 'ground')
...
test('ボスを普通に殴り倒すと撃破が記録され、階段が開く', () => {
    ...
    assert.ok(world.run.defeatedBosses.includes(boss.defId), '殴り倒したのに撃破が記録されていない');
...
test('HP0 の状態で行動できてしまわない', () => {
    ...
    assert.ok(world.finished || (p.pos.x === before.x && p.pos.y === before.y), 'HP0 で歩けてしまっている');
});
test('最下層で落とし穴に落ちても、ボス未撃破ならクリアにならない', () => {
    ...
    world.drainEvents();
    assert.notEqual(world.finished?.kind, 'clear', 'ボスを倒さずに落とし穴でクリアできてしまった');
});
...
//# sourceMappingURL=death.test.js.map