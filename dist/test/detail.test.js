...
test('倉庫の並びは持ち物の整理と同じ順番になる', () => {
    ...
    assert.deepEqual(town.storage.map((i) => i.defId), player.inventory.map((i) => i.defId), '倉庫と持ち物で順番が違う');
});
...
test('換金の壺は容量の回数しか使えず、使い切ると割れる', () => {
    const cap = getItem('cashPot').kind === 'pot' ? getItem('cashPot').capacity : 0;
