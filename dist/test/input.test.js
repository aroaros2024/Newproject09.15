...
test('テンキーだけで全操作が完結する', () => {
    const needsNumpad = [Cmd.A, Cmd.B, Cmd.X, Cmd.Y, Cmd.L, Cmd.R, Cmd.Dash, Cmd.Wait];
    for (const cmd of needsNumpad) {
        const keys = InputManager.keysFor(cmd);
        assert.ok(keys.some((k) => k.startsWith('Numpad')), `${cmd} にテンキーが割り当てられていない（${keys.join(', ')}）`);
    }
});
