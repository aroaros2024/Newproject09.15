...
import { ICON_SIZE, allIconKeys, buildIcon, hasIcon, iconKeyForCatalog, iconKeyOf, } from '../src/ui/art/items.js';
...
test('草・巻物・杖・腕輪は種類の絵だけ（道具ごとの絵を 1 つも持たない）', () => {
    for (const d of ALL_ITEMS) {
        if (!CATEGORY_ONLY.includes(d.kind))
            continue;
    ...
    for (const key of allIconKeys()) {
        ...
        if (def)
            assert.ok(!CATEGORY_ONLY.includes(def.kind), `${key} は種類だけのはず`);
    }
});
...
test('未識別の種類はどれも、知らない状態では種類の絵になる', () => {
    for (const d of ALL_ITEMS) {
        if (!UNIDENTIFIED_KINDS.includes(d.kind))
            continue;
        assert.equal(iconKeyOf(d.id, false), d.kind, d.id);
    }
});
...
//# sourceMappingURL=items-art.test.js.map