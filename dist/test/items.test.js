...
import { needsDirection, needsItemTarget, equipItem, throwItem, useItem, } from '../src/game/itemActions.js';
...
test('すべてのアイテムを「使う」ことができ、例外も不整合も起きない', () => {
    ...
    for (const def of ALL_ITEMS) {
        ...
        const targetUid = needsItemTarget(def)
            ...
            : undefined;
        const dir = needsDirection(def) ? rng.int(8) : undefined;
        assert.doesNotThrow(() => { useItem(world, item.uid, targetUid, dir); }, `${def.id} (${def.name}) を使うと例外が出た`);
        world.drainEvents();
...
test('すべてのアイテムを「投げる」ことができる', () => {
    ...
    for (const def of ALL_ITEMS) {
        ...
        const item = give(world, def.id);
        assert.doesNotThrow(() => { throwItem(world, item.uid, rng.int(8)); }, `${def.id} (${def.name}) を投げると例外が出た`);
        world.drainEvents();
...
test('腕輪の効果 id はすべて実装済みのものだけ', () => {
    ...
    for (const def of itemsOfKind('bracelet')) {
        ...
        if (!isBraceletEffect(effect))
            unknown.push(`${def.id} → ${effect}`);
    }
...
test('効果 id を持つアイテムには必ずハンドラがある', async () => {
    ...
    for (const def of ALL_ITEMS) {
        ...
        if (def.kind === 'bracelet')
            continue;
        const effect = def.effect;
        if (effect && !ITEM_EFFECTS[effect])
            missing.push(`${def.id} → ${effect}`);
        const thrown = def.throwEffect;
        if (thrown && !ITEM_EFFECTS[thrown])
            missing.push(`${def.id} → throw:${thrown}`);
    }
...
test('ワナの効果 id には必ずハンドラがある', async () => {
    ...
    for (const t of allTraps()) {
        assert.doesNotThrow(() => { mod.applyTrapEffect(world, world.player, t.id); }, `${t.id} (${t.name}) の効果で例外が出た`);
...
test('合成の壺は満杯になると合成される', () => {
    ...
    const capacity = getItem('synthesisPot').capacity;
...
test('掘り進みの杖は、歩いて行ける道しか作らない', () => {
    ...
    for (let seed = 0; seed < 60; seed++) {
        ...
        for (const dir of DIRS) {
            useItem(world, staff.uid, undefined, dir);
            world.drainEvents();
        }
