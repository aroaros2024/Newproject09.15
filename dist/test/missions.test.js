...
test('act: を条件にした操作は、本当にターンを使う', () => {
    ...
    for (const type of used) {
        ...
        // どの操作も成立するように持ち物を整える
        const mk = (id) => makeItem(id, world.rng, { plusKnown: true }, () => world.nextUid());
        ...
        const action = type === 'wait' ? { type: 'wait' }
        ...
        if (!action) {
            assert.ok(['sell', 'swap'].includes(type), `act:${type} の確かめ方が用意されていない`);
            continue;
        }
        ...
        assert.ok(after > before, `act:${type} が数えられていない。ターンを使わない操作を条件にしている`);
    }
});
test('ミッションが使うキーは、どこかで数えられている', () => {
    ...
    for (const def of MISSIONS) {
        for (const key of tallyKeysOf(def.cond)) {
            ...
            assert.ok(known.has(key) || known.has(base), `${def.id}: ${key} を数えている場所が無い`);
        }
    }
});
test('ちょうど 100 個ある', () => {
