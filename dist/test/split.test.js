...
function useSkill(world, defId, skill) {
...
test('何度分裂しても攻撃力は 1 を下回らない', () => {
    ...
    world.addMonster(m);
    for (let i = 0; i < 10; i++)
        MONSTER_SKILLS.split(world, m, p);
...
test('分裂で増えても、攻撃力の合計はほとんど増えない', () => {
    ...
    const total = () => world.run.monsters.reduce((a, x) => a + x.atk, 0);
    const before = total();
    ...
    assert.ok(total() <= before + added, `合計が ${before} から ${total()} に増えている（増えた敵 ${added} 体）`);
});
