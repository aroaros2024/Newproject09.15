...
test('すべてのドット絵が 16x16 で、パレットに無い文字を使っていない', () => {
    ...
    for (const [id, sprite] of Object.entries(ALL)) {
        ...
        if (sprite.rows.length !== 16)
            errors.push(`${id}: 行数が ${sprite.rows.length}（16 が必要）`);
        for (const [i, row] of sprite.rows.entries()) {
            if (row.length !== 16)
                errors.push(`${id}: 行 ${i} の長さが ${row.length}（16 が必要）`);
        }
    }
...
test('すべてのアイテムに、少なくともカテゴリ共通の絵がある', () => {
    ...
    for (const def of ALL_ITEMS) {
        if (!ALL[def.id] && !ALL[def.sprite])
            missing.push(`${def.id}（${def.name}）→ ${def.sprite}`);
    }
...
test('未識別のカテゴリは、種類ごとに絵を変えていない', () => {
    ...
    const leaked = [];
    for (const def of ALL_ITEMS) {
        if (!['herb', 'scroll', 'staff', 'bracelet'].includes(def.kind))
            continue;
        if (ALL[def.id])
            leaked.push(`${def.id}（${def.name}）に個別の絵がある`);
    }
