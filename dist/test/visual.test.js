...
class RecordingSink {
    ...
    beam(from, to, _color, element, ms) {
...
function spawnBeside(world, defId, prefer = DIRS) {
...
test('本物の戦闘で倒した敵は、取り除かれた後も亡骸が残り、数字は当たりの瞬間に出る', () => {
    ...
    const tile = { ...m.pos };
    const d = dirTo(world.player.pos, m.pos);
    let killed = false;
    ...
    assert.ok(dmg.at >= BEAT_TIMES.attackImpact - 1 && dmg.at < BEAT_TIMES.attackImpact + TICK + 1, `数字の時刻が当たりと合わない: ${dmg.at}`);
...
test('見えないマスで倒れた敵は、亡骸も数字も粒も出さない', () => {
    ...
    if (!m) {
        ...
        for (let i = 0; i < world.map.tiles.length && !m; i++) {
            ...
            if (t.kind === 'floor' && !t.visible)
                m = world.spawnAt?.('ratField', { x, y }) ?? undefined;
        }
    }
...
function scriptedBatch(world, m1, m2) {
...
test('1 手のできごとは拍に分かれ、当たりの時刻に出る（決まった時刻・毎回同じ）', () => {
    const run = () => {
        ...
        for (let i = 0; i < vw.timeline.pending; i++) {
            ...
            if (c.kind === CUE.DAMAGE || c.kind === CUE.MISS)
                times.push(Math.round(c.time * 100) / 100);
        }
        runTicks(vw, world, 1000);
    ...
    assert.deepEqual(pops.map((r) => `${r.what}:${r.tag}`), ['12:crit', 'MISS:miss', '3:damageToPlayer', '5:damageToPlayer']);
    for (let i = 0; i < pops.length; i++) {
        assert.ok(pops[i].at >= a.times[i] && pops[i].at < a.times[i] + TICK + 0.01, `${pops[i].what} が ${pops[i].at}ms に出た（予定 ${a.times[i]}）`);
    }
    // 会心の手応え：閃き・揺れ・止め
...
test('階の移動：前の階の残りは粒を出さず、暗転して台帳を空にし、明けに階の札', () => {
    ...
    assert.ok(before > 1);
    const P = world.player.id;
    const events = [{ t: 'damage', actorId: P, amount: 4, kind: 'physical' }];
    enterFloor(world, 2);
...
test('300 手の再生：見た目を通しても通さなくても、ゲームの乱数と結果は同じ', () => {
    ...
    const script = (w, r) => {
        ...
        if (near && nearD === 1 && roll < 85)
            return { type: 'attack', dir: dirTo(p, near.pos) };
        if (near && nearD <= 10 && roll < 70) {
            const d = dirTo(p, near.pos);
            if (canEnter(w.map, p.x + DIR_VEC[d].x, p.y + DIR_VEC[d].y, 'ground'))
                return { type: 'move', dir: d };
        }
        if (roll < 60)
            return { type: 'move', dir: r.pick(DIRS), dash: roll < 10 };
        if (roll < 78)
            return { type: 'attack', dir: r.pick(DIRS) };
        if (roll < 86)
            return { type: 'wait' };
        if (roll < 92)
            return { type: 'pickup' };
    ...
    const onDraw = (d) => {
        ...
        if (d.kind === 'player')
            return;
        if (!at(seen.map, d.tileX, d.tileY)?.visible)
            leaks++;
    };
    Math.random = forbid;
    try {
    ...
    let damages = 0;
    for (let i = 0; i < 300; i++) {
        if (plain.finished || seen.finished)
            break;
    ...
    let waiting = 0;
    for (let i = 0; i < vw.timeline.pending; i++)
        if (vw.timeline.pendingAt(i).kind === CUE.DAMAGE)
            waiting++;
...
test('ビームの色から属性を決め、演出の名前はそろっている', () => {
    ...
    for (const id of ['hitPhysical', 'hitFire', 'hitMagic', 'crit', 'heal', 'death', 'levelUp', 'explosion',
        ...
        assert.ok(VFX_PRESET_IDS.includes(id), id);
    }
});
...
const CUE_NAMES = Object.fromEntries(Object.entries(CUE).map(([k, v]) => [v, k]));
/** まだ出していない合図を「名前@時刻」で */
function planOf(vw, kinds) {
    const out = [];
    for (let i = 0; i < vw.timeline.pending; i++) {
        const c = vw.timeline.pendingAt(i);
        if (kinds && !kinds.includes(c.kind))
            continue;
