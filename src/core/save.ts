...
import type {
  Charm, PartnerRecord, Replay, RunPartner, RunState, Settings, TownState,
} from './types.js';
...
function validateTown(t: Partial<TownState>): TownState {
  ...
  const out: TownState = {
    ...
    activeCharm: Number.isFinite(t.activeCharm) ? Math.floor(t.activeCharm as number) : null,
    shortcuts: sanitizeShortcuts(t.shortcuts),
    // uid は下の renumberStorage で振り直されるので、ここでは形だけ整える
    kept: Array.isArray(t.kept)
      ? t.kept.filter((x): x is number => Number.isFinite(x)).map((x) => Math.floor(x))
      : [],
  ...
  // 持っていない護石を指していたら外す
  if (!(out.charms ?? []).some((c) => c.uid === out.activeCharm)) out.activeCharm = null;
  renumberStorage(out);
  return out;
}
 ...
 */
function sanitizeCharms(raw: unknown): Charm[] {
  if (!Array.isArray(raw)) return [];
  const out: Charm[] = [];
  const usedUid = new Set<number>();
  for (const v of raw.slice(0, CHARM_BOX_LIMIT)) {
    if (typeof v !== 'object' || v === null) continue;
    const c = v as Partial<Charm>;
    if (!Number.isFinite(c.uid) || !Array.isArray(c.runes)) continue;
    const uid = Math.max(1, Math.floor(c.uid as number));
    ...
    for (const r of c.runes) {
      ...
      if (n > rule.maxLevel) continue;
      level.set(r, n);
    }
    const runes: string[] = [];
    for (const [id, n] of level) for (let i = 0; i < n; i++) runes.push(id);
    ...
    out.push({
      ...
      slots: Number.isFinite(c.slots)
        ? Math.max(0, Math.min(CHARM_MAX_SLOTS, Math.floor(c.slots as number))) : 0,
    });
  }
  return out;
}
...
function sanitizeShortcuts(v: unknown): (string | null)[] {
  const out: (string | null)[] = new Array(SHORTCUT_SLOTS).fill(null);
...
function renumberStorage(town: TownState): void {
  ...
  for (const item of town.storage) {
    ...
    item.uid = next++;
    if (wasKept) keptAfter.push(item.uid);
...
export const saveReplay = (replay: Replay): boolean => write(KEY_REPLAY, replay);
...
export function loadReplay(): Replay | null {
  const data = read<Replay>(KEY_REPLAY);
  if (!data) return null;
  if (data.version !== 1) return null;
  if (typeof data.dungeonId !== 'string' || !Number.isFinite(data.seed)) return null;
  if (!Array.isArray(data.actions) || !Array.isArray(data.lines)) return null;
  if (typeof data.town !== 'object' || data.town === null) return null;
  return data;
}

export const clearReplay = (): void => remove(KEY_REPLAY);
