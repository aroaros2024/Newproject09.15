...
import type { RuneId } from '../core/types.js';
...
export interface CharmRuneRule {
  /** そのレア度の中での出やすさ */
  weight: number;
  /** 護石での上限レベル（装備での maxLevel とは別） */
  maxLevel: number;
}
 ...
 */
export const CHARM_RUNES: Readonly<Record<RuneId, CharmRuneRule>> = {
  // --- 武器の印
...
/** 護石に乗りうる印の一覧（表の順） */
export const CHARM_RUNE_IDS: readonly RuneId[] = Object.keys(CHARM_RUNES);

/** その印が護石に乗るか */
export const charmRuneRule = (id: RuneId): CharmRuneRule | undefined => CHARM_RUNES[id];

/**
 * 質ごとの作り方。
 *
 * 護石が出るのは N と R だけ。SSR と SR は相棒と加護のままにしてある。
 * 10 連の「SR 以上が 1 つ確定」を護石が食うと、確定枠の意味が消えるため。
 */
export interface CharmQuality {
  /** 印の種類数 [最小, 最大] */
  runes: [number, number];
  /** 空きスロット [最小, 最大] */
  slots: [number, number];
  /**
   * レベルが上振れする確率。
   * 1 段上げるたびにこの確率で判定するので、上限まで伸びるのはまれ。
   */
  levelUp: number;
}

export const CHARM_QUALITY: Readonly<Record<'n' | 'r', CharmQuality>> = {
  n: { runes: [1, 1], slots: [0, 1], levelUp: 0.15 },
  r: { runes: [1, 3], slots: [0, 3], levelUp: 0.4 },
};

/** 表の印がすべて実在することを起動時に確かめる */
export function validateCharmRunes(): string[] {
  const known = new Set(RUNES.map((r) => r.id));
  const bad: string[] = [];
  for (const id of CHARM_RUNE_IDS) {
    if (!known.has(id)) bad.push(`${id}: そんな印は無い`);
    const rule = CHARM_RUNES[id];
    if (rule.maxLevel < 1) bad.push(`${id}: maxLevel が ${rule.maxLevel}`);
    if (rule.weight <= 0) bad.push(`${id}: weight が ${rule.weight}`);
