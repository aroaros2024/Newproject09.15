...
import type {
  DungeonDef, DungeonTheme, FloorGenParams, ItemKind, SpawnEntry,
} from '../core/types.js';
...
const mob = (id: string, from: number, to: number, weight = 100): SpawnEntry =>
  ({ id, from, to, weight });

interface ItemTableOptions {
  /** 最深部 */
  depth: number;
  /** この値段ごとに 1 階ぶん出現が遅くなる */
 ...
 */
function itemTable(o: ItemTableOptions): SpawnEntry[] {
  const exclude = new Set(o.exclude ?? []);
  const early = new Set(o.earlyIds ?? []);
  const kinds = o.kinds ? new Set<string>(o.kinds) : null;
...
const trapTable = (depth: number, from = 1, exclude: string[] = []): SpawnEntry[] => {
...
const gen = (o: Partial<FloorGenParams>): FloorGenParams => ({
  width: 48, height: 32,
 ...
 * 倉庫を育てる意味は失われない。
 */
export const LOANER_GEAR: Record<string, { weapon: string; shield: string } | null> = {
