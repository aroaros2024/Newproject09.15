...
export type BoostKind =
  /** 保持枠が増える */
  | { t: 'keepSlot'; amount: number }
  /** その道具が、冒険の最初から識別済みになる */
  | { t: 'knownItem'; itemId: string }
  /**
   * その種類の道具が、まとめて識別済みになる。
   *
   * 今は引けない。1 種類ずつに分ける前のセーブを読めるように残してある。
   */
  ...
  | { t: 'bag'; amount: number }
  /** 倉庫の上限が増える（村） */
  | { t: 'storage'; amount: number }
  /** 村の道具屋の品揃えが増える（村） */
  | { t: 'shopSlot'; amount: number };
...
export interface GachaPrize {
   ...
   * 引けなくなった景品。
   *
   * 抽選には出ないが、持っている人のためにここへ残す。
   * 消すとセーブの読み込み（sanitizeOwned）が知らない id として捨ててしまい、
   * 既に引いた加護が黙って失われる。
   */
  retired?: boolean;
}
 ...
 */
const KNOWLEDGE: GachaPrize[] = KNOWABLE_ITEMS.map((d) => ({
  id: `k:${d.id}`,
  name: `${d.name}の 知識`,
  rarity: (d.price >= KNOWLEDGE_R_PRICE || ALWAYS_CURSED_BRACELETS.includes(d.id)
    ? 'r' : 'n') as Rarity,
  weight: 1,
  boost: { t: 'knownItem', itemId: d.id } as BoostKind,
}));

export const PRIZES: readonly GachaPrize[] = [...SSR, ...SR, ...KNOWLEDGE];
 ...
 */
const byRarity = new Map<Rarity, GachaPrize[]>();
export function prizesOf(r: Rarity): GachaPrize[] {
