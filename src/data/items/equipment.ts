...
import type { ShieldDef, WeaponDef } from '../../core/types.js';

const w = (
  id: string, name: string, atk: number, slots: number, price: number,
  weight: number, desc: string, innate: string[] = [], sprite = 'weapon',
): WeaponDef => ({
...
const s = (
  id: string, name: string, def: number, slots: number, price: number,
  weight: number, desc: string, innate: string[] = [], sprite = 'shield',
): ShieldDef => ({
...
export const WEAPONS: readonly WeaponDef[] = [
  ...
  w('ironSword', '鉄の剣', 9, 3, 1800, 70, '癖の無い剣。印を 3 つ埋められる。'),
