...
import { type Point, type Rect, clamp, rectCenter } from '../core/geom.js';
import type { FloorGenParams, FloorMap, Room } from '../core/types.js';
import {
  at, connectivityReport, createMap, fillRect, inBounds, isOpen, setKind,
} from './tilemap.js';
...
export function generateFloor(
  ...
  let lastMap: FloorMap | null = null;
  for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
    ...
    if (report.connected && map.rooms.length > 0) return map;
    lastMap = map;
  }
...
function buildPartitionedFloor(
  params: FloorGenParams, rng: Rng, opts: GenerateOptions,
): FloorMap {
  ...
  for (let i = 0; i < order.length; i++) {
    ...
    if (!mustPlace && rng.percent(params.emptyCellRate)) continue;
  ...
  for (const cell of cells) {
    if (!cell.room && cell.degree > 0) setKind(map, cell.junction.x, cell.junction.y, 'floor');
  }
 ...
 */
function splitAxis(total: number, n: number, rng: Rng): number[] {
  const cuts: number[] = [0];
  const base = total / n;
  for (let i = 1; i < n; i++) {
...
function placeRoom(
  map: FloorMap, cell: Cell, params: FloorGenParams, rng: Rng,
  id: number, opts: GenerateOptions,
): Room | null {
  const b = cell.bounds;
  ...
  if (availW < minW || availH < minH) return null;
 ...
 */
function buildSpanningEdges(cells: Cell[], cols: number, rows: number, rng: Rng): Edge[] {
  const candidates: Edge[] = [];
  const idx = (cx: number, cy: number): number => cy * cols + cx;
  for (let cy = 0; cy < rows; cy++) {
    for (let cx = 0; cx < cols; cx++) {
      if (cx + 1 < cols) candidates.push({ a: idx(cx, cy), b: idx(cx + 1, cy), axis: 'h' });
      if (cy + 1 < rows) candidates.push({ a: idx(cx, cy), b: idx(cx, cy + 1), axis: 'v' });
    }
  }
  ...
  for (const e of extras) {
    if (rng.percent(25)) chosen.push(e);
  }
 ...
 */
function pruneDeadEnds(cells: Cell[], edges: Edge[]): void {
  let changed = true;
  while (changed) {
    changed = false;
    for (let i = edges.length - 1; i >= 0; i--) {
      const e = edges[i];
      for (const side of [e.a, e.b] as const) {
...
function carveConnection(
  map: FloorMap, a: Cell, b: Cell, axis: 'h' | 'v',
  xs: number[], ys: number[], rng: Rng,
): void {
  if (axis === 'h') {
 ...
 */
function connectorOf(map: FloorMap, cell: Cell, side: Side, rng: Rng): Point {
  if (!cell.room) return cell.junction;
  ...
  if (tile && !tile.hard) {
    ...
    tile.roomId = -1;
    tile.isDoor = true;
    if (!cell.room.doors.some((d) => d.x === p.x && d.y === p.y)) cell.room.doors.push(p);
  }
  return p;
}

/** 水平に掘る（両端を含む）。部屋の中は塗り替えない */
function carveH(map: FloorMap, x1: number, x2: number, y: number): void {
  const [lo, hi] = x1 <= x2 ? [x1, x2] : [x2, x1];
  for (let x = lo; x <= hi; x++) carveCorridorTile(map, x, y);
}

function carveV(map: FloorMap, y1: number, y2: number, x: number): void {
  const [lo, hi] = y1 <= y2 ? [y1, y2] : [y2, y1];
  for (let y = lo; y <= hi; y++) carveCorridorTile(map, x, y);
}

function carveCorridorTile(map: FloorMap, x: number, y: number): void {
  const t = at(map, x, y);
  if (!t || t.hard) return;
  if (t.kind === 'wall') {
    t.kind = 'floor';
...
function buildBigRoomFloor(params: FloorGenParams, rng: Rng, opts: GenerateOptions): FloorMap {
  const map = createMap(params.width, params.height);
  const margin = 2;
  const rect: Rect = {
    x: margin,
 ...
 */
function addLiquid(map: FloorMap, params: FloorGenParams, rng: Rng): void {
  if (params.liquid === 'none' || params.waterRate <= 0) return;
  const kind = params.liquid;
  for (const room of map.rooms) {
    if (!rng.percent(params.waterRate)) continue;
    const r = room.rect;
    ...
    for (let i = 0; i < blobs; i++) {
      ...
      for (let y = cy - ry; y <= cy + ry; y++) {
        for (let x = cx - rx; x <= cx + rx; x++) {
          if (x < inner.x || y < inner.y) continue;
          if (x >= inner.x + inner.w || y >= inner.y + inner.h) continue;
          // 楕円の内側だけ、しかも縁は確率で欠けさせて自然な形にする
          const nx = (x - cx) / (rx + 0.5);
          const ny = (y - cy) / (ry + 0.5);
          const d = nx * nx + ny * ny;
          if (d > 1) continue;
          if (d > 0.6 && rng.percent(40)) continue;
          const t = at(map, x, y);
          if (t && t.kind === 'floor' && t.roomId === room.id) t.kind = kind;
        }
      }
    }
  }
 ...
 * 生成が全滅した場合のみ呼ばれる。
 */
function sealUnreachable(map: FloorMap): void {
  const report = connectivityReport(map);
  ...
  map.rooms = map.rooms.filter((room) => {
    for (let y = room.rect.y; y < room.rect.y + room.rect.h; y++) {
      for (let x = room.rect.x; x < room.rect.x + room.rect.w; x++) {
        if (isOpen(at(map, x, y))) return true;
      }
    }
    return false;
  });
