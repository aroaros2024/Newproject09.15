...
import type { Charm, RuneId } from '../core/types.js';
import type { Rng } from '../core/rng.js';
import {
  CHARM_MAX_RUNES, CHARM_QUALITY, CHARM_RUNES, CHARM_RUNE_IDS, type CharmQuality,
} from '../data/charms.js';
...
export function charmRuneLevel(charm: Charm | null, runeId: RuneId): number {
  if (!charm) return 0;
  let n = 0;
  for (const r of charm.runes) if (r === runeId) n++;
  return n;
}

/** 付いている印の一覧（id とレベル）。表示順は付けた順 */
export function charmRuneList(charm: Charm): Array<{ id: RuneId; level: number }> {
  const seen: RuneId[] = [];
  for (const r of charm.runes) if (!seen.includes(r)) seen.push(r);
  return seen.map((id) => ({ id, level: charmRuneLevel(charm, id) }));
}

/** 使っている印の種類数 */
export const charmUsedSlots = (charm: Charm): number => new Set(charm.runes).size;
 ...
 */
export function charmName(charm: Charm): string {
 ...
 */
export function charmScore(charm: Charm): number {
...
/** 溶かしたときのギタン */
export function meltValue(charm: Charm): number {
  return MELT_BASE + charm.runes.length * MELT_PER_LEVEL + charm.slots * MELT_PER_SLOT;
}

/** その印と同時に付けられない印 */
function exclusiveOf(id: RuneId): RuneId | null {
...
/** 在庫から重みで 1 つ選ぶ */
function pickRune(rng: Rng, pool: readonly RuneId[]): RuneId {
 ...
 */
function rollLevel(rng: Rng, max: number, levelUp: number): number {
  let level = 1;
  while (level < max && rng.chance(levelUp)) level++;
  return level;
}
 ...
 */
export function rollCharm(rng: Rng, rarity: 'n' | 'r', uid: number): Charm {
  const q: CharmQuality = CHARM_QUALITY[rarity];
  const want = rng.range(q.runes[0], q.runes[1]);

  const runes: RuneId[] = [];
  const taken = new Set<RuneId>();
  ...
  for (let i = 0; i < want && pool.length > 0; i++) {
    ...
    for (let n = 0; n < level; n++) runes.push(id);
  }
 ...
 */
export function addCharmRune(charm: Charm, runeId: RuneId): boolean {
  const rule = CHARM_RUNES[runeId];
  if (!rule) return false;
  if (charm.slots <= 0) return false;

  const level = charmRuneLevel(charm, runeId);
  if (level >= rule.maxLevel) return false;
  if (level === 0 && charmUsedSlots(charm) >= CHARM_MAX_RUNES) return false;
...
export function canAddCharmRune(charm: Charm, runeId: RuneId): boolean {
  const rule = CHARM_RUNES[runeId];
  if (!rule || charm.slots <= 0) return false;
  const level = charmRuneLevel(charm, runeId);
  if (level >= rule.maxLevel) return false;
 ...
 */
export function rollAddableRune(rng: Rng, charm: Charm): RuneId | null {
  const pool = (Object.keys(CHARM_RUNES) as RuneId[])
 ...
 */
export function reforgeCharm(rng: Rng, charm: Charm, keep: RuneId | null): Charm {
  const fresh = rollCharm(rng, 'r', charm.uid);
  if (keep === null || charmRuneLevel(charm, keep) === 0) return fresh;

  const level = charmRuneLevel(charm, keep);
  const other = exclusiveOf(keep);
  const rest = fresh.runes.filter((r) => r !== keep && r !== other);
  const kept: RuneId[] = [];
  for (let i = 0; i < level; i++) kept.push(keep);

  // 残した印を先頭に置く。種類数が上限を超えないように後ろを削る
  const out: RuneId[] = [...kept];
  const seen = new Set<RuneId>([keep]);
  for (const r of rest) {
    if (!seen.has(r) && seen.size >= new Set(fresh.runes).size) continue;
    seen.add(r);
    out.push(r);
  }
