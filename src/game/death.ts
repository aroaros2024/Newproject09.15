 ...
 */
export function handlePlayerDeath(world: World, cause: string): boolean {
  ...
  const herb = p.inventory.find(
    (i) => i.defId === 'reviveHerb' && !i.cursed,
  );
  if (herb) {
    if (herb.count > 1) herb.count--;
    else removeFromInventory(p, herb.uid);
    revive(world, 0.5);
...
function revive(world: World, ratio: number): void {
  const p = world.player;
 ...
 */
export function losesItemsOnDeath(dungeonId: string): boolean {
  return dungeonId !== 'd1';
}
