...
import {
  addToInventory, allCarriedItems, findItem, isEquipped, isInventoryFull,
...
const NO = (reason: string): ActionResult => ({ tookTurn: false, reason });

/** 使うときに対象アイテムを選ぶ必要があるか */
export function needsItemTarget(def: ItemDef): boolean {
  if (def.kind === 'scroll') {
...
/** 使うときに方向を選ぶ必要があるか */
export const needsDirection = (def: ItemDef): boolean =>
  def.kind === 'staff' && def.ballistics !== 'self';

/** 壺に物を入れられるか（＝「入れる」コマンドを出すか） */
export function isContainer(def: ItemDef): boolean {
...
export function useItem(
  world: World, uid: number, targetUid?: number, dir?: Dir,
): ActionResult {
  const p = world.player;
  const item = findItem(p, uid);
  if (!item) return NO('その道具は 持っていない');
  ...
  if (def.kind === 'food') {
    ...
    if (def.effect) runEffect(world, def.effect, item, p, target, dir);
    consume(world, item);
    return OK;
  }
  ...
  if (!happened) world.log('しかし 何も 起こらなかった。');
  else identifyOnUse(world, item);

  // 草と巻物は使うと無くなる。杖は残る
  if (def.kind === 'herb' || def.kind === 'scroll') consume(world, item);
  return OK;
}

function runEffect(
  world: World, effectId: string, item: ItemInstance, user: Actor,
  target: ItemInstance | null, dir?: Dir,
): boolean {
...
/** 使ったら種類が分かる */
function identifyOnUse(world: World, item: ItemInstance): void {
  world.run.identify.known[item.defId] = true;
}

/** 1 個消費する */
function consume(world: World, item: ItemInstance): void {
  const p = world.player;
  if (item.count > 1) item.count--;
  else removeFromInventory(p, item.uid);
}
...
function usePot(world: World, pot: ItemInstance, target: ItemInstance | null): ActionResult {
  ...
  if (!needsItemTarget(def)) {
    ...
    if (happened) identifyOnUse(world, pot);
    world.log(`${itemName(pot, world.run.identify)}を 使った。`, 'item');
    if (def.effect === 'sealPot' || def.effect === 'monsterPot') consume(world, pot);
    return OK;
  }

  if (!target) return NO('入れる物を 選んでいない');
  if (target.uid === pot.uid) return NO('自分自身は 入れられない');
...
/** 壺から取り出す */
export function takeOutOfPot(world: World, potUid: number, index: number): ActionResult {
  const p = world.player;
  const pot = findItem(p, potUid);
  if (!pot) return NO('その壺は 持っていない');
  const item = pot.contents[index];
  if (!item) return NO('その中身は 無い');
...
export function equipItem(world: World, uid: number): ActionResult {
  const p = world.player;
  const item = findItem(p, uid);
  if (!item) return NO('その道具は 持っていない');
  const def = getItem(item.defId);
...
export function unequipItem(world: World, uid: number): ActionResult {
  const p = world.player;
  const item = findItem(p, uid);
  if (!item) return NO('その道具は 持っていない');
  if (item.cursed) {
...
export function throwItem(world: World, uid: number, dir: Dir): ActionResult {
  const p = world.player;
  const item = findItem(p, uid);
  if (!item) return NO('その道具は 持っていない');
  ...
  if (victim) {
    ...
    if (world.rng.chance(hitRate(THROW_HIT, 0, evade))) {
      ...
      if (effectId && ITEM_EFFECTS[effectId]) {
        ...
        if (hit) identifyOnUse(world, flying);
      }
  ...
  if (def.kind === 'pot' && def.effect !== 'unbreakable') {
    ...
    world.sfx('break');
    for (const inner of flying.contents) world.dropItem(inner, landing);
    return OK;
  }
...
/** 足元の商品を買う */
export function buyItem(world: World): ActionResult {
  const p = world.player;
  const f = world.floorItemAt(p.pos);
  if (!f || f.item.shopPrice <= 0) return NO('ここには 売り物が 無い');
...
/** 店主に売る */
export function sellItem(world: World, uid: number): ActionResult {
  const p = world.player;
  const item = findItem(p, uid);
  if (!item) return NO('その道具は 持っていない');
  if (item.shopPrice > 0) return NO('それは 店の 商品だ');
  if (isEquipped(p, uid) && item.cursed) {
    world.log('呪われていて 手から 離れない！', 'bad');
    return OK;
  }
...
/** 借金（持ち出した商品の合計） */
export function shopDebt(world: World): number {
...
/** 借金を払う */
export function payDebt(world: World): ActionResult {
  const debt = shopDebt(world);
  if (debt <= 0) return NO('払う物が 無い');
  const p = world.player;
  if (p.gitan < debt) {
...
/** 合成（村の鍛冶屋から呼ぶ） */
export function synthesizeItems(
  world: World, baseUid: number, materialUids: number[],
): boolean {
  const p = world.player;
