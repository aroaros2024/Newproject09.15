...
import type { MonsterActor, PartnerRecord, PlayerActor, RunPartner } from '../core/types.js';
import { type PartnerDef, tryGetPartner } from '../data/partners.js';
...
/** 村で伸ばせる上限 */
export function partnerLevelCap(rec: PartnerRecord): number {
  return Math.min(
    PARTNER_MAX_LEVEL,
    PARTNER_BASE_CAP + Math.max(0, Math.floor(rec.dupes)) * PARTNER_CAP_PER_DUPE,
  );
}

/** 次のレベルまでに要る経験値 */
export function partnerExpToNext(level: number): number {
 ...
 */
export function effectivePartnerLevel(rec: PartnerRecord, playerLevel: number): number {
  return Math.max(1, Math.min(
    Math.floor(rec.level),
    Math.max(1, Math.floor(playerLevel)) + PARTNER_LEAD,
  ));
}

/** そのレベルでの能力値 */
export function partnerStats(def: PartnerDef, level: number): {
...
export const partnerName = (rec: PartnerRecord): string =>
  rec.nickname || tryGetPartner(rec.id)?.name || rec.id;

/** プレイヤーの近くで、味方を置けるマスを探す */
function spotNearPlayer(world: World): { x: number; y: number } | null {
  ...
  for (let r = 1; r <= 5; r++) {
    ...
    for (let dy = -r; dy <= r; dy++) {
      for (let dx = -r; dx <= r; dx++) {
        ...
        if (!world.canPlaceItem(p)) continue;
        candidates.push(p);
      }
    }
    if (candidates.length > 0) return world.rng.pick(candidates);
  }
  return null;
}
 ...
 */
export function spawnPartner(world: World, rec: PartnerRecord): MonsterActor | null {
 ...
 */
export function gainPartnerExp(world: World, amount: number): void {
 ...
 */
export function mergePartnerExp(rec: PartnerRecord, gained: number): number {
  ...
  if (rec.level >= cap) rec.exp = 0;
  return levels;
}

/** 冒険中の相棒のアクター（居なければ null） */
export function partnerActor(world: World, rp: RunPartner | undefined): MonsterActor | null {
  if (!rp) return null;
  return world.run.allies.find((a) => a.id === rp.actorId) ?? null;
}

/** プレイヤーのレベルが上がったら、相棒も付いて行く（村で育てた上限まで） */
export function refreshPartnerLevel(world: World, p: PlayerActor): void {
  const rp = world.run.partner;
  if (!rp) return;
