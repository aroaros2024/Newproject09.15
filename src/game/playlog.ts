...
import type {
  ItemInstance, LogLine, LogStyle, PlayerActor, Replay, TownState,
} from '../core/types.js';
...
const STYLES: readonly LogStyle[] = [
...
async function gzip(bytes: Uint8Array): Promise<Uint8Array | null> {
  const C = (globalThis as { CompressionStream?: new (f: string) => unknown }).CompressionStream;
  if (!C) return null;
  try {
    const stream = new Blob([bytes as unknown as BlobPart]).stream()
      .pipeThrough(new C('gzip') as ReadableWritablePair<Uint8Array, Uint8Array>);
...
async function gunzip(bytes: Uint8Array): Promise<Uint8Array | null> {
  const D = (globalThis as { DecompressionStream?: new (f: string) => unknown }).DecompressionStream;
  if (!D) return null;
  try {
    const stream = new Blob([bytes as unknown as BlobPart]).stream()
      .pipeThrough(new D('gzip') as ReadableWritablePair<Uint8Array, Uint8Array>);
    return new Uint8Array(await new Response(stream).arrayBuffer());
  } catch {
    return null;
  }
...
/** 運ぶ形。鍵を 1 文字にしてあるのは、貼り付ける長さを削るため */
interface Payload {
  v: 1;
  d: string;
  s: number;
  ...
  /** 行動を畳んだ文字列 */
  a: string;
  /** できごと。`ターン|階|色|本文` を改行で並べたもの */
  l: string;
  /** 途中で記録をやめたか */
  x?: 1;
}

const encodeLines = (lines: readonly LogLine[]): string => lines
  .map((l) => `${l.turn}|${l.depth}|${Math.max(0, STYLES.indexOf(l.style))}|${l.text}`)
  .join('\n');

function decodeLines(s: string): LogLine[] {
...
export async function encodeReplay(replay: Replay): Promise<string> {
  ...
  if (replay.truncated) payload.x = 1;
...
export interface PlayLogHead {
  /** ダンジョンの表示名 */
  dungeonName: string;
  /** 終わり方。冒険の途中なら null */
  ending: string | null;
  /** いまのプレイヤー。冒険が終わっていれば最後の姿 */
  player: PlayerActor | null;
  depth: number;
  turn: number;
  /** 書き出した時刻（ミリ秒）。0 なら出さない */
  at: number;
}
 ...
 */
export async function buildPlayLog(replay: Replay, head: PlayLogHead): Promise<string> {
  const p = head.player;
  const out: string[] = [];
  ...
  for (const l of shown) out.push(`[${l.turn}] ${l.depth}F  ${l.text}`);
...
export async function parsePlayLog(text: string): Promise<Replay | null> {
  for (const line of text.split('\n')) {
    const t = line.trim();
    if (t.startsWith(TAG_RAW) || t.startsWith(TAG_GZIP)) return decodeReplay(t);
  }
  return null;
}
