 ...
 */
export function depositItem(town: TownState, item: ItemInstance): ItemInstance | null {
  ...
  if (storageFull(town)) return null;
 ...
 */
export function wearCharm(town: TownState, uid: number | null): boolean {
  if (uid === null) {
    town.activeCharm = null;
    return true;
  }
  if (!(town.charms ?? []).some((c) => c.uid === uid)) return false;
 ...
 */
export function meltCharm(town: TownState, uid: number): number {
  const list = town.charms ?? [];
  const i = list.findIndex((c) => c.uid === uid);
  if (i < 0) return 0;
  if (town.activeCharm === uid) return 0;
 ...
 */
export function smithReforge(
  town: TownState, rng: Rng, uid: number, keep: string | null,
): boolean {
  const list = town.charms ?? [];
  const i = list.findIndex((c) => c.uid === uid);
  if (i < 0) return false;
  if (town.gitan < SMITH_PRICE.reforge) return false;
...
export function finishRun(
  ...
  const bring = (item: ItemInstance): boolean => {
    ...
    if (wasKept && !keptAfter.includes(stored.uid)) keptAfter.push(stored.uid);
    return true;
  };
