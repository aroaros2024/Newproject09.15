...
function otherRoundsPerInput(world: World): number {
 ...
 */
function checkPlayerDeath(world: World, cause: string): boolean {
  if (world.finished) return true;
  const p = world.player;
  if (p.hp > 0 && p.alive) return false;
...
function endOfTurn(world: World): void {
  ...
  for (const a of [...world.livingActors()]) {
    if (!tickStatuses(world, a)) continue;
    if (a.kind === 'player') {
  ...
  if (checkPlayerDeath(world, 'ちからつきた')) return;
 ...
 */
function ensureBossChainNotStuck(world: World): void {
  const here = world.bossesHere();
  if (here.length === 0 || world.bossesCleared()) return;
...
export function restTurns(world: World, maxTurns: number): number {
  ...
  while (n < maxTurns) {
    ...
    if (p.hp >= p.maxHp) break; // 全快した
    if (restIsPointless(world)) break; // 休んでも回復しない
    if (p.inventory.length !== startItems) break; // 盗まれた
    if (statusKey(p) !== startStatuses) break; // 休んでいる間に状態が変わった
    if (hostileNear(world)) break; // 敵がいる

    const before = p.hp;
    stepTurn(world, { type: 'wait' });
    n++; // 進んだターンは必ず数える（表示が嘘をつかないように）
    if (p.hp < before) break; // 殴られた
  }
  return n;
}
 ...
 * 無害なものでも永久に休めなくなる。
 */
function restIsPointless(world: World): boolean {
  const p = world.player;
  return p.foodX10 <= 0
 ...
 */
function hostileNear(world: World): boolean {
  const p = world.player;
  for (const m of world.run.monsters) {
    if (!m.alive) continue;
    if (!world.isHostile(p, m)) continue;
    ...
    if (t?.visible && d <= 6) return true;
  }
  return false;
}

/** 休憩を中断すべき状態異常の顔ぶれ。疾風と永続のものは数えない */
const statusKey = (p: PlayerActor): string =>
  p.statuses.filter((st) => st.turns !== 0 && st.id !== 'quick')
 ...
 */
export function whyCannotRest(world: World): string | null {
  const p = world.player;
