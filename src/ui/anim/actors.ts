...
import type { Actor, ActorKind, FloorMap, RunState } from '../../core/types.js';
import { getMonster } from '../../data/registry.js';
import { type AnimId, type Rig, getRig, getSpecies } from '../art/rig.js';
import { hashFloat } from '../art/hash.js';
import { TILE_ART } from '../gfx/view.js';
import { ANIM } from '../theme.js';
import { type ActorDrawable, type AnimState, STATUS_BIT } from './types.js';

// ---------------------------------------------------------------------------
// 時間と大きさ（等速のミリ秒・ドット）
// ---------------------------------------------------------------------------

/** 動きのコマの進み方。リグの AnimDef もこの形を満たす */
...
export const MONSTER_TIMING: Readonly<Record<AnimId, AnimTiming>> = {
...
export const HERO_TIMING: Readonly<Record<AnimId, AnimTiming>> = {
  ...MONSTER_TIMING,
...
export function dirOfVec(dx: number, dy: number, fallback: number): number {
  const sx = Math.sign(dx);
  const sy = Math.sign(dy);
  for (let i = 0; i < 8; i++) if (DIR_X[i] === sx && DIR_Y[i] === sy) return i;
  return fallback;
}
...
export class ActorAnim implements ActorDrawable {
  // --- 描く側が読む値（ActorDrawable）
  id = -1;
  kind: ActorKind = 'monster';
  species = '';
  disguise: string | null = null;
  boss = false;
  anim: AnimId = 'idle';
  dir8 = 4;
  frame = 0;
...
/** 台帳で起きたことを受け取る（VisualWorld が実装し、土煙を出す） */
export interface RegistryListener {
  /** 新しいキャラが見えるマスに現れた（その階の最初の同期は除く） */
  actorAppeared(a: ActorAnim): void;
  /** 倒されずに居なくなった（見えるマスに居たときだけ呼ぶ） */
...
export class ActorRegistry {
  ...
  speedScale = 1;
  listener: RegistryListener | null = null;
  ...
  private readonly byId = new Map<number, ActorAnim>();
  private readonly list: ActorAnim[] = [];
  private listCount = 0;
  private readonly pool: ActorAnim[] = [];
  private poolCount = 0;
  private readonly order: ActorAnim[] = [];
  ...
  at(i: number): ActorAnim {
    return this.list[i];
  }

  get(id: number): ActorAnim | undefined {
    return this.byId.get(id);
  }

  /** 全部を返す（階の移動・画面に入り直したとき） */
  releaseAll(): void {
    for (let i = this.listCount - 1; i >= 0; i--) this.release(this.list[i]);
    this.synced = false;
  }

  /** 1 体を台帳から外して置き場へ返す */
  release(a: ActorAnim): void {
    if (a.id < 0 || a.slot < 0) return;
    ...
    this.listCount--;
    a.reset();
    if (this.poolCount < this.pool.length) this.pool[this.poolCount] = a;
   ...
   */
  sync(run: RunState, map: FloorMap): void {
    const stamp = ++this.stamp;
    this.syncActor(run.player, map, stamp);
    const allies = run.allies;
    for (let i = 0; i < allies.length; i++) this.syncActor(allies[i], map, stamp);
    const monsters = run.monsters;
    for (let i = 0; i < monsters.length; i++) this.syncActor(monsters[i], map, stamp);
    for (let i = this.listCount - 1; i >= 0; i--) {
      const a = this.list[i];
      if (a.stamp === stamp) continue;
      a.present = false;
      a.visible = tileVisible(map, a.tileX, a.tileY);
      if (a.pendingDefeat || a.dying || a.warpLock || a.vanishing) continue;
      if (!a.visible) {
        this.release(a);
        continue;
      }
   ...
   */
  ensure(id: number, run: RunState, map: FloorMap): ActorAnim | undefined {
  ...
  private syncActor(actor: Actor, map: FloorMap, stamp: number): void {
  ...
  private alloc(id: number): ActorAnim {
    let a: ActorAnim;
    if (this.poolCount > 0) {
    ...
    if (this.listCount < this.list.length) this.list[this.listCount] = a;
  ...
  private refresh(a: ActorAnim, actor: Actor): void {
    ...
    for (let i = 0; i < st.length; i++) {
      if (st[i].turns !== 0) mask |= STATUS_BIT[st[i].id] ?? 0;
    }
    a.statusMask = mask;
  ...
  private resolveSpecies(a: ActorAnim): void {
  ...
  moveTo(a: ActorAnim, x: number, y: number): void {
  ...
  timing(a: ActorAnim, id: AnimId): AnimTiming {
  ...
  actorAtTile(x: number, y: number): number {
    for (let i = 0; i < this.listCount; i++) {
      const a = this.list[i];
      if (a.present && !a.dying && a.tileX === x && a.tileY === y) return a.id;
    }
    return -1;
  }

  /** 攻撃：振りかぶり → 当たりのコマで踏み込む → 戻る。ux, uy は相手への向き */
  startAttack(a: ActorAnim, ux: number, uy: number): void {
    if (a.dying || a.downed) return;
  ...
  startBump(a: ActorAnim, dir8: number): void {
    if (a.dying || a.downed) return;
    const d = ((dir8 % 8) + 8) % 8;
    a.dir8 = d;
  ...
  /** 詠唱（杖・魔法・息） */
  startCast(a: ActorAnim): void {
    if (a.dying || a.downed) return;
    this.setState(a, 'cast', this.timing(a, 'cast'));
  }

  /** 道具を使う・投げる・掲げる */
  startUse(a: ActorAnim): void {
    if (a.dying || a.downed) return;
    this.setState(a, 'use', this.timing(a, 'use'));
  }

  /** 被弾：白く光って ux, uy の向きへ下がる */
  startHurt(a: ActorAnim, ux: number, uy: number, px: number): void {
    a.flash = 1;
    if (a.dying) return;
    if (!a.downed) this.setState(a, 'hurt', this.timing(a, 'hurt'));
    this.setKnock(a, ux, uy, px, ACTOR_TIMES.knock);
  }

  /** 空振りをかわす：攻撃の向きと直角へ少しずれて戻る */
  dodge(a: ActorAnim, ux: number, uy: number): void {
    if (a.dying) return;
  ...
  /** 倒されずに居なくなった：短く薄れて返す */
  startVanish(a: ActorAnim): void {
    a.vanishing = true;
    a.vanishT = 0;
  }

  /** モンスターハウス：その場で跳ねる */
  hop(a: ActorAnim): void {
    a.hopT = 0;
    a.hopDur = ACTOR_TIMES.hop;
  }

  /** ワープで縦に縮んで消える */
  warpOut(a: ActorAnim): void {
    a.warpDir = 1;
    a.warpT = 0;
  }

  /** ワープの着地：新しいマスへ置き、縦に伸びて現れる */
  warpIn(a: ActorAnim, x: number, y: number): void {
    a.place(x, y);
    a.warpLock = false;
    a.warpDir = -1;
    a.warpT = 0;
  ...
  freeze(a: ActorAnim, ms: number): void {
    if (ms > a.hitstop) a.hitstop = ms;
  }

  private setState(a: ActorAnim, s: AnimState, tm: AnimTiming): void {
    a.state = s;
    a.stateT = 0;
    a.stateDur = (tm.frames * 1000) / tm.fps;
  }

  private setLunge(a: ActorAnim, ux: number, uy: number, px: number, strike: number, dur: number): void {
  ...
  private setKnock(a: ActorAnim, ux: number, uy: number, px: number, dur: number): void {
   ...
   */
  private step(a: ActorAnim): boolean {
    const ms = this.stepMs;
    ...
    if (a.hitstop > 0) {
      ...
      a.stateT += act;
      if (a.lungeT < a.lungeDur) a.lungeT += act;
      if (a.knockT < a.knockDur) a.knockT += act;
      if (a.hopT < a.hopDur) a.hopT += act;
    ...
    switch (a.state) {
      ...
      case 'hurt':
        ...
        break;
      case 'death':
        if (a.stateT >= a.stateDur) return true;
        break;
      default:
        a.state = this.baseState(a);
        break;
    ...
    // --- コマ
    const animId: AnimId = a.state === 'death' ? 'hurt' : a.state;
    ...
    // --- 跳ねる
    let hopLift = 0;
    if (a.hopT < a.hopDur) hopLift = ACTOR_PX.hop * Math.sin((Math.PI * a.hopT) / a.hopDur);
    ...
    if (a.warpDir !== 0) {
      ...
      if (a.warpDir > 0) {
        ...
        if (a.warp <= 0) a.warpDir = 0;
      }
    }

    // --- 見えてくる・消える・灰色
    if (a.spawnT < 1) a.spawnT = Math.min(1, a.spawnT + act / ACTOR_TIMES.spawnFade);
    let fade = 1;
    if (a.vanishing) {
      a.vanishT += act;
      if (a.vanishT >= ACTOR_TIMES.vanish) return true;
      fade = 1 - a.vanishT / ACTOR_TIMES.vanish;
    }
    if (a.downed && a.grey < 1) a.grey = Math.min(1, a.grey + act / ACTOR_TIMES.downGrey);
  ...
  private baseState(a: ActorAnim): AnimState {
    if (a.moveT < a.moveDur || a.walkHold > 0) return 'walk';
  ...
  sortDrawables(): number {
    ...
    for (let i = 0; i < this.listCount; i++) {
      const a = this.list[i];
      if (!isDrawable(a)) continue;
      if (n < this.order.length) this.order[n] = a;
      else this.order.push(a);
      n++;
    }
    const o = this.order;
    for (let i = 1; i < n; i++) {
 ...
 */
export function isDrawable(a: ActorAnim): boolean {
  if (a.id < 0 || a.alpha <= 0) return false;
  return a.kind === 'player' || a.visible;
}

/** a を b より後に描くか（奥から手前へ） */
function drawsAfter(a: ActorAnim, b: ActorAnim): boolean {
  if (a.artY !== b.artY) return a.artY > b.artY;
  if (a.artX !== b.artX) return a.artX > b.artX;
  return a.id > b.id;
}

function tileVisible(map: FloorMap, x: number, y: number): boolean {
  if (x < 0 || y < 0 || x >= map.width || y >= map.height) return false;
  return map.tiles[y * map.width + x].visible;
}

/** 一覧を直接たどって id のアクターを探す（配列を作らない） */
function findActor(run: RunState, id: number): Actor | null {
  if (run.player.id === id) return run.player;
  for (let i = 0; i < run.allies.length; i++) if (run.allies[i].id === id) return run.allies[i];
  for (let i = 0; i < run.monsters.length; i++) if (run.monsters[i].id === id) return run.monsters[i];
  return null;
}

/** ボスか。知らない id（検証の場面の仮の種）は false */
function isBossDef(defId: string): boolean {
  try {
