...
import type { Action, FloorMap, GameEvent, StatusId } from '../../core/types.js';
import type { World } from '../../game/world.js';
import { PAL_HEX, type RampName, ci } from '../art/palette.js';
import { ANIM } from '../theme.js';
import { ACTOR_PX, type ActorAnim, ActorRegistry, type RegistryListener, dirOfVec } from './actors.js';
import { Camera } from './camera.js';
import { type BeatContext, CUE, type Cue, type CueHandler, Timeline } from './timeline.js';
import type {
  ActorDrawable, BurstOpts, ProjectileKind, VfxElement, VfxPresetId, VfxSink,
} from './types.js';
...
const hex = (r: RampName, step: number): string => PAL_HEX[ci(r, step)];
...
/** 状態異常が掛かった瞬間の粒の色 */
const STATUS_COLOR: Readonly<Record<StatusId, string>> = {
  confused: hex('rose', 4),
  blind: hex('ink', 3),
  asleep: hex('indigo', 4),
...
const TRAP_VFX: Readonly<Record<string, { preset: VfxPresetId; color: string; shake: number }>> = {
...
const ZAP_ELEMENT: Readonly<Record<string, VfxElement>> = {
  '#ff8030': 'fire',
  '#ffd040': 'fire',
  '#ffe060': 'thunder',
  '#e0c040': 'thunder',
...
export function elementOfColor(color: string): VfxElement {
  ...
  const d = max - min;
  if (d < 0.08) return 'light';
  let h: number;
  if (max === r) h = ((g - b) / d + 6) % 6;
  else if (max === g) h = (b - r) / d + 2;
  else h = (r - g) / d + 4;
  h *= 60;
  if (h < 40 || h >= 330) return 'fire';
  if (h < 70) return 'thunder';
  if (h < 165) return 'poison';
  if (h < 200) return 'ice';
  if (h < 245) return 'water';
  return 'magic';
}
...
const isFriend = (a: ActorAnim): boolean => a.kind === 'player' || a.kind === 'ally';

// ---------------------------------------------------------------------------
// 本体
// ---------------------------------------------------------------------------

export class VisualWorld implements CueHandler, BeatContext, RegistryListener {
  /** キャラの見た目の台帳 */
  ...
  private dashNext = false;
  /** 前の階の残りを出し切っている間（前の階の位置に粒や数字を出さない） */
  private muted = false;
  /** 倒れた・踏破で、組の終わりまで入力を止めたい長さ */
  private endHold = 0;
  private readonly immediate: GameEvent[] = [];
  private readonly opts: BurstOpts = { dir8: -1, power: 1, color: '', tag: '' };
  private readonly p1 = { x: 0, y: 0 };
  private readonly p2 = { x: 0, y: 0 };
  /** 地形の種類の控え（階ごと） */
  private kinds: Uint8Array | null = null;

  constructor(sink: VfxSink) {
    this.sink = sink;
   ...
   */
  ingest(events: readonly GameEvent[], world: World): readonly GameEvent[] {
    this.world = world;
    this.playerId = world.run.player.id;
    this.immediate.length = 0;
    if (events.length === 0) return this.immediate;
    this.timeline.flush(this);
    this.endHold = 0;
    let from = 0;
    for (let i = 0; i < events.length; i++) {
      const e = events[i];
      if (e.t === 'message' || e.t === 'bgm') this.immediate.push(e);
      if (e.t !== 'floorChange') continue;
    ...
    if (this.endHold > 0) this.holdMs = Math.max(this.holdMs, this.endHold + this.timeline.batchMs);
  ...
  tick(stepMs: number, world: World): void {
    ...
    this.world = world;
    this.playerId = world.run.player.id;
    if (this.holdMs > 0) this.holdMs = Math.max(0, this.holdMs - stepMs);
    this.registry.sync(world.run, world.map);
    if (this.kinds === null) this.checkDig(world.map);
    if (this.freezeMs > 0) {
   ...
   */
  notePlayerAction(action: Action): void {
  ...
  shownHp(actorId: number, hp: number, maxHp: number): number {
  ...
  sortDrawables(): number {
    return this.registry.sortDrawables();
  }

  drawableAt(i: number): ActorDrawable {
    return this.registry.drawableAt(i);
  }

  /** 描けるキャラを奥から手前へ 1 体ずつ渡す。cb は毎フレーム作らないこと */
  forEachDrawable(cb: (d: ActorDrawable) => void): void {
    const n = this.registry.sortDrawables();
    for (let i = 0; i < n; i++) cb(this.registry.drawableAt(i));
  }

  /** id のキャラの見た目（見えるかどうかに関係なく）。主人公のランタンの位置など */
  actor(id: number): ActorDrawable | undefined {
  ...
  actorAppeared(a: ActorAnim): void {
    if (this.muted) return;
    this.burst('dust', a.tileX, a.tileY, -1, 0.8, '', 'spawn');
  }

  actorVanished(a: ActorAnim): void {
    if (this.muted) return;
  ...
  private planSegment(events: readonly GameEvent[], from: number, to: number, world: World): void {
    for (let i = from; i < to; i++) this.applyNow(events[i], world);
    this.timeline.plan(events, from, to, this);
  }

  /** 拍を待たずにその場で済ませること（移動・倒された印・ワープの着地待ち・入力の足止め） */
  private applyNow(e: GameEvent, world: World): void {
    const reg = this.registry;
    switch (e.t) {
      case 'move': {
        const a = reg.ensure(e.actorId, world.run, world.map);
        if (!a || a.warpLock) return;
  ...
  private enterFloor(depth: number, world: World): void {
  ...
  fireCue(c: Readonly<Cue>): void {
    ...
    switch (c.kind) {
      ...
      case CUE.BUMP: {
        const a = reg.get(c.actor);
        if (a) reg.startBump(a, c.amount);
        return;
      }
      case CUE.BEAM: {
        const a = c.actor >= 0 ? reg.get(c.actor) : undefined;
        if (a) reg.startCast(a);
        if (this.muted) return;
        ...
        if ((a && isFriend(a)) || this.tileVisible(c.x, c.y) || this.tileVisible(c.x2, c.y2)) {
          ...
          if (this.tileVisible(c.x2, c.y2)) this.light(c.x2, c.y2, c.str, 1.5, 0.7, 250);
        }
        return;
      }
      case CUE.THROW: {
        ...
        if (this.muted) return;
        if ((a && isFriend(a)) || this.tileVisible(c.x, c.y) || this.tileVisible(c.x2, c.y2)) {
          this.sink.projectile(this.pt1(c.x, c.y), this.pt2(c.x2, c.y2), c.str === '' ? null : c.str,
            projectileKind(c.str2), c.ms);
        }
        return;
      }
      case CUE.EXPLODE: {
        ...
        if (seen) {
          ...
          this.sink.flash(LIGHT.explosionFlash,
            this.reduceMotion ? VISUAL_FX.reducedFlashAlpha : VISUAL_FX.explosionFlashAlpha,
            Math.max(1, VISUAL_FX.explosionFlashMs * s));
        }
      ...
      case CUE.TRAP: {
        if (this.muted || !this.tileVisible(c.x, c.y)) return;
        const v = TRAP_VFX[c.str] ?? TRAP_VFX.spike;
        this.burst(v.preset, c.x, c.y, -1, 1, v.color, c.str);
        if (v.preset === 'hitFire') this.light(c.x, c.y, LIGHT.fire, 2, 0.8, 300);
        if (v.shake > 0 && !this.reduceMotion) this.sink.shake(v.shake, Math.max(1, 150 * s));
        return;
      }
      case CUE.WARP_OUT: {
        const a = reg.get(c.actor);
        if (!a) return;
        reg.warpOut(a);
        if (!this.muted && this.shows(a, a.tileX, a.tileY)) this.burst('warp', a.fx, a.fy, -1, 1, '', 'out');
        return;
      }
      case CUE.WARP_IN: {
        const a = reg.get(c.actor);
        ...
        if (a.kind === 'player') this.snapCamera = true;
        if (!this.muted && this.shows(a, c.x, c.y)) this.burst('warp', c.x, c.y, -1, 1, '', 'in');
        return;
      }
      case CUE.DAMAGE:
        this.fireDamage(c);
      ...
      case CUE.MISS: {
        ...
        if (this.muted || !this.shows(a, c.x, c.y)) return;
      ...
      case CUE.STATUS: {
        ...
        if (!c.flag || this.muted) return;
        const a = reg.get(c.actor);
        if (!this.shows(a, c.x, c.y)) return;
        this.burst('statusApplied', a ? a.fx : c.x, a ? a.fy : c.y, -1, 1,
          STATUS_COLOR[c.str as StatusId] ?? '', c.str);
        return;
      }
      case CUE.ITEM_GET: {
        const p = reg.get(this.playerId);
      ...
      case CUE.FLASH:
        if (this.muted) return;
        this.sink.flash(c.str, this.reduceMotion ? VISUAL_FX.reducedFlashAlpha : VISUAL_FX.eventFlashAlpha,
          Math.max(1, c.ms * s));
        return;
      case CUE.SHAKE:
        if (this.muted || this.reduceMotion) return;
        this.sink.shake(c.amount, Math.max(1, c.ms * s));
        return;
      case CUE.HOUSE: {
        if (this.muted) return;
        this.sink.banner('モンスターハウス！', houseLabel(c.str), 'house');
        if (!this.reduceMotion) this.sink.shake(VISUAL_FX.houseShake, Math.max(1, VISUAL_FX.houseShakeMs * s));
        for (let i = 0; i < reg.count; i++) {
          const a = reg.at(i);
          if (a.kind === 'monster' && a.present && a.visible) reg.hop(a);
        }
        return;
      }
      case CUE.BOSS: {
        const a = reg.get(c.actor);
        if (a) reg.startAttack(a, 0, 1);
        if (this.muted) return;
        this.sink.banner('ボスが 立ちはだかる', this.nameOf(c.actor), 'boss');
        if (!this.reduceMotion) this.sink.shake(VISUAL_FX.bossShake, Math.max(1, VISUAL_FX.bossShakeMs * s));
        return;
      }
      case CUE.GAME_OVER: {
        const p = reg.get(this.playerId);
        if (p) reg.setDowned(p);
  ...
  private fireDamage(c: Readonly<Cue>): void {
    const reg = this.registry;
    const s = this.speed;
    const a = reg.get(c.actor);
    ...
    if (a) {
      ...
      if (!this.reduceMotion) {
        ...
        reg.freeze(a, f * s);
        const src = c.target >= 0 ? reg.get(c.target) : undefined;
        if (src && src !== a) reg.freeze(src, f * s);
      }
    }
    if (this.muted || !this.shows(a, c.x, c.y)) return;
    const x = a ? a.fx : c.x;
    ...
    const preset: VfxPresetId = c.flag ? 'crit'
      : c.str === 'fire' ? 'hitFire'
        : c.str === 'magic' ? 'hitMagic' : 'hitPhysical';
    this.burst(preset, x, y, dirOfVec(ux, uy, -1), c.flag ? 1.6 : 1, '', c.str);
    if (c.str === 'fire') this.light(x, y, LIGHT.fire, 1.5, 0.6, 220);
    if (c.flag) {
      this.sink.flash(LIGHT.white, this.reduceMotion ? VISUAL_FX.reducedFlashAlpha : VISUAL_FX.critFlashAlpha,
        Math.max(1, VISUAL_FX.critFlashMs * s));
      if (!this.reduceMotion) {
   ...
   */
  private shows(a: ActorAnim | undefined, x: number, y: number): boolean {
    if (a) return isFriend(a) || this.tileVisible(a.tileX, a.tileY);
    return this.tileVisible(x, y);
  }

  private tileVisible(x: number, y: number): boolean {
    const w = this.world;
    if (!w || Number.isNaN(x) || Number.isNaN(y)) return false;
    const map = w.map;
  ...
  private tileOf(map: FloorMap, x: number, y: number): string {
    if (x < 0 || y < 0 || x >= map.width || y >= map.height) return '';
    return map.tiles[y * map.width + x].kind;
  }

  /** 吹き出しの補助の値を使い回しの物に書いて渡す */
  private burst(preset: VfxPresetId, x: number, y: number, dir8: number, power: number,
    color: string, tag: string): void {
    const o = this.opts;
    o.dir8 = dir8;
    o.power = power;
    o.color = color;
  ...
  private light(x: number, y: number, color: string, radius: number, intensity: number, ms: number): void {
    if (!this.tileVisible(x, y)) return;
    this.sink.light(x, y, color, radius, intensity, Math.max(1, ms * this.speed));
  }

  private pt1(x: number, y: number): { x: number; y: number } {
    this.p1.x = x;
    this.p1.y = y;
    return this.p1;
  }

  private pt2(x: number, y: number): { x: number; y: number } {
    this.p2.x = x;
    this.p2.y = y;
    return this.p2;
  }
  ...
  private nameOf(id: number): string | undefined {
    ...
    for (let i = 0; i < ms.length; i++) if (ms[i].id === id) return w.nameOf(ms[i]);
    return undefined;
  }
   ...
   */
  private checkDig(map: FloorMap): void {
    const n = map.width * map.height;
    const tiles = map.tiles;
    if (this.kinds === null || this.kinds.length !== n) {
      const k = new Uint8Array(n);
      for (let i = 0; i < n; i++) k[i] = kindCode(tiles[i].kind);
      this.kinds = k;
      return;
    }
    const k = this.kinds;
...
function projectileKind(s: string): ProjectileKind {
