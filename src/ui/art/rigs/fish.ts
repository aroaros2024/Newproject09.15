...
function drawSide(b: PixBuf, p: Readonly<Pose>, v: Readonly<Variant>): void {
  const s = scratch;
  s.clear();
  const cy = 17;
  ...
  for (let i = 0; i <= 12; i++) {
    ...
    for (let dy = -TOP[i]; dy <= BOT[i]; dy++) {
      let c: number;
      if (dy <= -2) c = mat(v, 'body', 4);
      else if (dy <= 0) c = mat(v, 'body', 3);
      else if (dy === 1) c = mat(v, 'belly', 4);
      else c = mat(v, 'belly', 3);
      s.set(x, cy + dy, c);
    }
  }
  ...
  if (eyes >= 1.5) runs(s, tail + 10, cy - 1, ['ee'], em);
  else if (eyes >= 0.5) runs(s, tail + 10, cy - 1, ['ee'], em);
  else runs(s, tail + 10, cy - 2, ['ge', 'ee'], em);
...
function drawFront(b: PixBuf, p: Readonly<Pose>, v: Readonly<Variant>): void {
  // 被弾で奥（上）へ押し戻される
  ...
  // 背びれ：背の真ん中の紅い筋
  for (let y = y0 + 4; y <= y0 + 8; y++) b.set(cx, y, mat(v, 'fin', y === y0 + 4 ? 4 : 3));
  // 目：頭の左右に張り出す
  const shut = p.eyes >= 0.5;
  runs(b, cx - 4, y0 + 8, shut ? ['.', 'e'] : ['g', 'e'], map);
  runs(b, cx + 4, y0 + 8, shut ? ['.', 'e'] : ['g', 'e'], map);
  // 口（鼻先）
  if (p.mouth >= 1.5) runs(b, cx - 1, y0 + 11, ['mtm', '.m.'], map);
  else if (p.mouth >= 0.5) runs(b, cx - 1, y0 + 11, ['mmm'], map);
  else b.set(cx, y0 + 11, mat(v, 'eye', 1));
  // 胸びれ（頭の後ろ、揺れで上下）
...
function drawBack(b: PixBuf, p: Readonly<Pose>, v: Readonly<Variant>): void {
  ...
  b.set(cx + 3, y0 + 1, mat(v, 'eye', 0));
  for (let y = y0 + 2; y <= y0 + 6; y++) b.set(cx, y, mat(v, 'fin', y === y0 + 2 ? 4 : 3));
  // 尾びれ：水面から跳ね上げる（広い ↔ 細い）
  const wide = !(p.sway > 0.3 || p.sway < -0.3);
...
const fish: Rig = {
  ...
  build(b: PixBuf, p: Readonly<Pose>, dir: DrawDir, v: Readonly<Variant>): void {
    if (dir === 'E') drawSide(b, p, v);
    else if (dir === 'N') drawBack(b, p, v);
    else drawFront(b, p, v);
  },
};
