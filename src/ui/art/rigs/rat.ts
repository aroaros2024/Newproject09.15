...
export type Tmpl = readonly string[];

/** 左半分（中央の列を含む）から左右対称の絵を作る */
export function sym(half: Tmpl): string[] {
  return half.map((row) => row + [...row.slice(0, -1)].reverse().join(''));
}

/** 左右反転 */
export function flipT(t: Tmpl): string[] {
  return t.map((row) => [...row].reverse().join(''));
}

/** 列 col を n 回増やす（n < 0 なら消す）。手描きの形のまま伸び縮みさせる */
export function widen(t: Tmpl, col: number, n: number): string[] {
  if (n === 0) return [...t];
  return t.map((row) => {
    if (col >= row.length) return row;
    if (n > 0) return row.slice(0, col) + row[col].repeat(n + 1) + row.slice(col + 1);
    return row.slice(0, col) + row.slice(col - n);
  });
}

/** 行 row を n 回増やす（n < 0 なら消す） */
export function heighten(t: Tmpl, row: number, n: number): string[] {
  if (n === 0 || row >= t.length) return [...t];
  if (n > 0) return [...t.slice(0, row), ...Array<string>(n + 1).fill(t[row]), ...t.slice(row + 1)];
  return [...t.slice(0, row), ...t.slice(row - n)];
}

/** 材質の 1〜5 段を文字に割り当てる。chars は「深い影 影 地 明 最明」の順、'_' は使わない */
export function shades(v: Readonly<Variant>, name: string, chars: string): Record<string, number> {
  const out: Record<string, number> = {};
  for (let i = 0; i < chars.length; i++) if (chars[i] !== '_') out[chars[i]] = mat(v, name, i + 1);
  return out;
}

/** 絵の幅 */
export const widthOf = (t: Tmpl): number => t.reduce((m, row) => Math.max(m, row.length), 0);

/** 塗ってある所にだけ載せる（泥・傷・マント・模様） */
export function over(b: PixBuf, x: number, y: number, rows: Tmpl, map: Readonly<Record<string, number>>): void {
  for (let j = 0; j < rows.length; j++) {
    const row = rows[j];
    for (let i = 0; i < row.length; i++) {
      const c = map[row[i]];
      if (c !== undefined) b.paint(x + i, y + j, c);
    }
  }
}
...
export function polyline(
  b: PixBuf, x: number, y: number, pts: readonly (readonly [number, number])[], c: number, thick = 0,
): void {
  for (let i = 1; i < pts.length; i++) {
    line(b, x + pts[i - 1][0], y + pts[i - 1][1], x + pts[i][0], y + pts[i][1], c);
    if (i <= thick) line(b, x + pts[i - 1][0], y + pts[i - 1][1] + 1, x + pts[i][0], y + pts[i][1] + 1, c);
  }
}
...
interface ViewKit {
  ...
  /** 耳（頭の左上から） */
  ears: readonly Part[];
  /** いつも見える歯（オオネズミ・長）。頭の左上から */
  teeth: Part;
  /** 冠（頭の左上から） */
  crown: Part;
  /** 長の白い眉（頭の左上から） */
  brows: Part;
  /** 長のマントの前（手前向きで顔の両脇に垂れる）。頭の左上から */
  robe: Part;
  /** 奥の脚・手前の脚。x は胴の左上から（y は使わず、足の裏を足元の行に置く）。横向きは [後ろ足, 前足] */
  legsFar: readonly Part[];
  legsNear: readonly Part[];
  /** 尾の折れ線（胴の左上から）。最後の点が揺れる */
  tail: readonly Pt[];
  /** 杖のように立てた尾（長） */
  staff: readonly Pt[];
}
...
interface RatKit {
  ...
  S: ViewKit;
  N: ViewKit;
  /** 脚の長さ（眠るときは脚をたたんで胴を下げる） */
  legLen: number;
  /** 尾の根元の太さ（折れ線の最初の何本を 2 列にするか） */
  tailThick: number;
  /** 横向きで前へ出られる量（胴のずれ ＋ 頭の伸び） */
  reach: number;
  /** 足元 */
  ax: number;
  ay: number;
...
function eye2(b: PixBuf, x: number, y: number, eyes: number, map: Record<string, number>): void {
  const e = rnd(eyes);
...
function put(b: PixBuf, x: number, y: number, part: Part, map: Record<string, number>): void {
  if (part.t.length) runs(b, x + part.x, y + part.y, part.t, map);
}

function buildRat(K: RatKit, b: PixBuf, p: Readonly<Pose>, dir: DrawDir, v: Readonly<Variant>): void {
  const map = palOf(v);
  ...
  const pad = (t: Tmpl): Tmpl => (t.length >= V.body.t.length ? t
    : [...t, ...Array<string>(V.body.t.length - t.length).fill('')]);
  const lay = (t: Tmpl): string[] => (t.length === 0 ? [] : curl
  ...
  if (dir !== 'N') hy = Math.min(hy, footY - headRows + 1);
  ...
  const drawTail = (): void => {
    ...
    const tip: Pt = staff ? pts[last] : dir === 'N'
  ...
  const stretchLeg = (t: Tmpl): Tmpl => (dir !== 'S' ? heighten(t, 0, Math.max(0, -bob - air)) : t);
  const legTop = (t: Tmpl): number => footY - air - t.length + 1;
  // 奥の脚
  if (!curl) {
  ...
  // 奥向き：耳と頭は胴の奥
  if (dir === 'N') {
    for (const ear of V.ears) put(b, hx, hy, ear, map);
    runs(b, hx, hy, V.head.t, map);
    if (flags & F_KING) put(b, hx, hy, V.crown, map);
  }
  // 胴
  runs(b, bx, by, bodyT, map);
  if (flags & F_MUD) over(b, bx, by, lay(V.mud), map);
  if (flags & F_SCAR) over(b, bx, by, lay(V.scar), map);
  if (flags & F_KING) over(b, bx, by, lay(V.cape), map);
  // 手前向き：耳は背中の上・顔の奥。噛みつく瞬間は耳を後ろへ倒す
  if (dir === 'S') {
    const flat = mouth >= 2 ? 1 : 0;
    V.ears.forEach((ear, i) => put(b, hx + (i === 0 ? -flat : flat), hy - flat, ear, map));
    if (flags & F_KING) put(b, hx, hy, V.robe, map);
  }
  // 手前の脚
  if (!curl) {
  ...
  if (flags & F_SCAR) over(b, hx, hy, V.headScar, map);
  for (const [ex, ey] of V.eyes) eye2(b, hx + ex, hy + ey, p.eyes, map);
  if (flags & F_KING) put(b, hx, hy, V.brows, map);
  if (mouth < 2 && (flags & (F_SCAR | F_KING))) put(b, hx, hy, V.teeth, map);
  if (dir === 'E') {
    const flat = mouth >= 2 ? 1 : 0;
    for (const ear of V.ears) put(b, hx - flat, hy + flat, ear, map);
  }
  if (flags & F_KING) put(b, hx, hy - (mouth >= 2 && dir === 'S' ? 1 : 0), V.crown, map);
}
