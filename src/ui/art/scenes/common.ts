...
export interface Diorama {
  ...
  lights: DioramaLight[];
  stars: DioramaStar[];
  /** 風車の羽根の軸（層 3 の座標）。風車が無ければ null */
  windmill: { x: number; y: number } | null;
  /** 環境の粒（カールノイズで舞う）。region 'torch' は灯籠のまわり、'view' は画面の上の方 */
  ambient: AmbientSpec[];
  /** 周辺減光の強さ（0〜1） */
  vignette: number;
...
export const NIGHT: Mood = { lift: 0, windowGlow: 0.7 };
export const DUSK: Mood = { lift: 1, windowGlow: 0.45 };
...
export function ridge(b: PixBuf, heightAt: (x: number) => number, base: number, rim: number): void {
  let prev = heightAt(-1);
  for (let x = 0; x < b.w; x++) {
    const h = heightAt(x);
    vline(b, x, h, b.h - h, base);
    if (prev >= h) b.set(x, h, rim);
    prev = h;
  }
}
 ...
 */
export function tower(b: PixBuf, tx: number, top: number, foot: number, lights: DioramaLight[],
  layer: number, mood: Mood, width = 1): void {
  ...
  for (let y = top; y < foot; y++) {
    ...
    if ((y - top) % 14 === 0) hline(b, tx - half - 1, y, half * 2 + 3, lit);
  }
  for (let y = top + 7; y < foot - 6; y += 14) {
    ...
    if (on) lights.push({ x: tx, y, radius: 10, color: '#ffcf7a', intensity: 0.5 * mood.windowGlow / 0.7, layer, flicker: true });
  }
  ...
  for (let x = tx - 1; x <= tx + 1; x++) b.set(x, top - 11, lit);
  lights.push({ x: tx, y: top - 8, radius: 38, color: '#ffe3a0', intensity: 0.8, layer, flicker: false });
}

/** 家 1 軒（切妻の茅葺き屋根・土壁・灯る窓） */
export function house(b: PixBuf, x: number, ground: number, w: number, lights: DioramaLight[], seed: number,
  mood: Mood = NIGHT): void {
  const wallH = 16;
  ...
  // 茅の筋
  for (let r = 2; r < roofH; r += 3) hline(b, peak - Math.round(((r + 1) / roofH) * (w / 2 + 4)) + 1, ground - wallH - roofH + r, 3, ci('gold', 2 + mood.lift));
  // 棟
  hline(b, peak - 3, ground - wallH - roofH - 1, 7, ci('earth', 0));
}

/** 灯籠 */
export function lantern(b: PixBuf, x: number, ground: number, lights: DioramaLight[], glow = 0.85): void {
...
export function windmillBody(b: PixBuf, wx: number, wg: number, lights: DioramaLight[], mood: Mood = NIGHT): { x: number; y: number } {
  for (let y = wg - 44; y < wg; y++) {
    ...
    if (y % 5 === 0) hline(b, wx - half + 1, y, half * 2 - 1, ci('stone', 1 + mood.lift));
  }
  for (let r = 0; r < 10; r++) hline(b, wx - r, wg - 54 + r, r * 2 + 1, ci('crimson', (r < 5 ? 2 : 1) + mood.lift));
...
export function trees(b: PixBuf, count: number, seed: number, groundAt: (x: number) => number,
  body: number, rim: number, skip?: (x: number) => boolean): void {
