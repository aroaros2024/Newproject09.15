...
import {
  DIORAMA_H, DIORAMA_W, type Diorama, type DioramaLight, type DioramaStar, NIGHT, house, lantern,
  noise1, ridge, tower, trees, windmillBody,
} from './common.js';
...
function far(b: PixBuf, lights: DioramaLight[]): void {
  ridge(b, (x) => Math.round(150 + 26 * noise1(x / 70, 11) - 18 * noise1(x / 23, 12) + 4 * noise1(x / 7, 13)),
    ci('ink', 1), ci('ink', 2));
...
function near(b: PixBuf): void {
  ...
  for (let x = 0; x < DIORAMA_W; x++) {
    const t = top(x);
    if (x > 190 && x < 260) continue; // 道の上は空ける
