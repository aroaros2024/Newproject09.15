...
export interface FinalizeOptions {
  ...
  /** 右下の縁を暗くする（既定 true） */
  shadowEdge?: boolean;
  /** この色の画素は明るさを変えない（目の光・宝石など、描いた色のまま見せたいもの） */
...
export function finalize(b: PixBuf, o: FinalizeOptions = {}): void {
  ...
  const at = (x: number, y: number): number =>
    x < 0 || y < 0 || x >= w || y >= h ? TRANSPARENT : src[y * w + x];
  ...
  if (rim || shadowEdge) {
    for (let y = 0; y < h; y++) {
      for (let x = 0; x < w; x++) {
        const c = src[y * w + x];
        if (c === TRANSPARENT || isEmissive(c) || (keep && keep.has(c))) continue;
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      if (src[y * w + x] !== TRANSPARENT) continue;
      ...
      if (c !== TRANSPARENT) px[y * w + x] = outlineOf(c, lightSide);
    }
  }
}
