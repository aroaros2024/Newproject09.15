...
import { PixBuf, ellipse, hline, line, rect, runs, vline } from './pixbuf.js';
...
function plate(b: PixBuf, ramp: RampName = 'stone'): void {
  ...
  // 角を落とす
  for (const [x, y] of [[2, 2], [13, 2], [2, 13], [13, 13]]) b.set(x, y, 0);
  // 内側の溝
...
/** 右上向きの矢 */
function arrowMark(b: PixBuf, tip: RampName, shaft: RampName): void {
...
function gas(b: PixBuf, ramp: RampName): void {
  plate(b);
  for (const x of [5, 8, 11]) b.set(x - 1, 11, ci('ink', 1));
  runs(b, 3, 4, [
    '....aab...',
    '.aa.abbbc.',
    'abbcbbbbbc',
...
const TRAPS: Record<string, Draw> = {
  arrow: (b) => {
    plate(b);
  ...
  bearTrap: (b) => {
    ...
    for (const x of [5, 7, 9, 11]) b.set(x, 6, ci('steel', 5));
    for (const x of [6, 8, 10]) b.set(x, 10, ci('steel', 4));
  ...
  sealTrap: (b) => {
    ...
    for (const y of [6, 8]) hline(b, 7, y, 2, ci('crimson', 3));
...
export function buildTrapIcon(id: string, out: PixBuf, finish = true): boolean {
  const draw = TRAPS[id];
...
export const hasTrapIcon = (id: string): boolean => id in TRAPS;
export const allTrapIconIds = (): string[] => Object.keys(TRAPS);
