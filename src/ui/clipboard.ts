...
import { type PlayLogHead, buildPlayLog } from '../game/playlog.js';

export type CopyResult = 'clipboard' | 'prompt' | 'failed';
 ...
 * 戻り値は画面に出す 1 行。
 */
export async function copyPlayLog(
  replay: Replay | null, head: PlayLogHead,
): Promise<string> {
  if (!replay) return 'まだ 記録が ありません。';
