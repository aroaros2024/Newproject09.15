...
import { type Rig, mat, registerRig, registerSpecies } from '../art/rig.js';
import { idleBreath, walkCycle } from '../art/anim.js';
import { type Canvas2D, type Ctx2D, makeCanvas, pixBufToCanvas } from '../gfx/canvas.js';
import { Compositor } from '../gfx/compositor.js';
import { FX_FLASH, FX_NORMAL, FX_OCCLUDER, SheetCache, animIndex } from '../gfx/sheets.js';
import {
  type ActorLighter, type FrameInfo, type WorldScene, worldToLogicalX, worldToLogicalY,
} from '../gfx/types.js';
import {
  TILE_EXPLORED, TILE_UNEXPLORED, TILE_VISIBLE_DARK, TILE_VISIBLE_LIT, type LightList,
  flickerSeed, parseHex,
...
function buildKinds(): Uint8Array {
  const k = new Uint8Array(MW * MH);
  for (let y = 0; y < MH; y++) {
    for (let x = 0; x < MW; x++) {
      let v = K_FLOOR;
      if (x === 0 || y === 0 || x === MW - 1 || y === MH - 1) v = K_WALL;
      // 仕切りの壁（x = 16）。y = 9〜10 が出入口
      if (x === 16 && y !== 9 && y !== 10) v = K_WALL;
      // 溶岩：左の部屋の帯と、右の部屋の池（池の右半分は未探索）
      if (y >= 14 && y <= 15 && x >= 3 && x <= 11) v = K_LAVA;
      if (y >= 12 && y <= 13 && x >= 20 && x <= 27) v = K_LAVA;
      k[y * MW + x] = v;
    }
  }
  return k;
...
const kindAt = (x: number, y: number): number =>
  x < 0 || y < 0 || x >= MW || y >= MH ? K_WALL : KINDS[y * MW + x];

/** 松明（上の壁の正面） */
const TORCH_X = [3, 8, 13];
/** 右の部屋で最初から未探索の所 */
const unexploredAt = (x: number, y: number): boolean => x >= 17 && ((x >= 25 && y >= 8) || y >= 14);

// ---------------------------------------------------------------------------
// 地形の絵（2 色の石・レンガの壁・溶岩 4 コマ）
// ---------------------------------------------------------------------------

interface TerrainCanvases {
  base: Canvas2D;
  lava: Canvas2D[];
  lavaGlow: Canvas2D[];
  flame: Canvas2D[];
...
function paintFloor(b: PixBuf, tx: number, ty: number): void {
  const ox = tx * T;
  const oy = ty * T;
  ...
  for (let y = 0; y < T; y++) b.set(ox, oy + y, ci('stone', 1));
  // 小石（座標のハッシュで 0〜2 個）
  ...
  if (kindAt(tx, ty - 1) === K_WALL) {
    for (let y = 0; y < 2; y++) {
      for (let x = 0; x < T; x++) if (((x + y) & 1) === 0 || y === 0) b.set(ox + x, oy + y, ci('stone', 1));
    }
  }
}

function paintWall(b: PixBuf, tx: number, ty: number): void {
  const ox = tx * T;
  const oy = ty * T;
  if (kindAt(tx, ty + 1) !== K_WALL) {
    ...
    for (let y = 4; y < 16; y++) {
      ...
      for (let x = 0; x < T; x++) {
        ...
        if ((y - 4) % 4 === 3 || joint) b.set(ox + x, oy + y, ci('stone', 1));
        else if ((y - 4) % 4 === 0) b.set(ox + x, oy + y, ci('stone', 4));
      }
    }
    ...
    for (let y = 0; y < T; y += 2) {
      for (let x = 0; x < T; x += 2) {
        if (hash2(ox + x, oy + y, 5) % 5 === 0) rect(b, ox + x, oy + y, 2, 2, ci('stone', 1));
      }
    }
  }
}

function paintLavaBase(b: PixBuf, tx: number, ty: number): void {
  const ox = tx * T;
  const oy = ty * T;
...
function paintLavaFrame(b: PixBuf, glow: PixBuf, f: number): void {
  for (let ty = 0; ty < MH; ty++) {
    for (let tx = 0; tx < MW; tx++) {
      if (kindAt(tx, ty) !== K_LAVA) continue;
      for (let y = 0; y < T; y++) {
        for (let x = 0; x < T; x++) {
          const wx = tx * T + x;
          const wy = ty * T + y;
...
/** 松明の炎（5×7、3 コマ） */
function flameFrame(f: number): PixBuf {
...
function buildTerrain(): TerrainCanvases {
  const base = new PixBuf(MW * T, MH * T);
  for (let ty = 0; ty < MH; ty++) {
    for (let tx = 0; tx < MW; tx++) {
      const k = kindAt(tx, ty);
      if (k === K_WALL) paintWall(base, tx, ty);
      else if (k === K_LAVA) paintLavaBase(base, tx, ty);
      else paintFloor(base, tx, ty);
    }
  }
  // 松明の燭台
  ...
  const flame: Canvas2D[] = [];
  const flameGlow: Canvas2D[] = [];
  for (let f = 0; f < 3; f++) {
...
function registerTestRigs(): void {
  if (rigsRegistered) return;
  rigsRegistered = true;
  registerRig(figure);
  registerRig(orb);
  registerSpecies({
...
class TestScene implements WorldScene {
  camX = 26;
  camY = 40;
  readonly look = LOOK;
  readonly mapW = MW;
  readonly mapH = MH;
  tileVersion = 0;
  fixedCam: [number, number] | null = null;

  private readonly art = buildTerrain();
  private readonly sheets = new SheetCache();
  private readonly actors: TestActor[] = [];
  private readonly order: TestActor[] = [];
  private readonly player: TestActor;
  private readonly walker: TestActor;
  private readonly orbActor: TestActor;
  private readonly slime: TestActor;
  private playerTile = -1;
  private readonly lanternRgb = parseHex(LOOK.lantern.color);
  private readonly shadowTex: Canvas2D;
  private readonly glowTex: Canvas2D;
  /** 床の物（旧来の絵）：[種, マス x, マス y] */
  private readonly ground: number[] = [];
  private lavaFrame = 0;
  private time = 0;
  /** 火の粉ごとの速さ・位相・横の位置・大きさ（最初に決めておく。毎フレームはハッシュを引かない） */
  private readonly ember = new Float32Array(EMBERS * 4);
  /** 松明ごとの揺らぎの種 */
  private readonly torchSeed = TORCH_X.map((tx) => flickerSeed(tx, 0));

  constructor() {
    const s = this.sheets;
    const mk = (species: number, tx: number, ty: number, shadow: number): TestActor => {
      const a: TestActor = {
  ...
  private stateOf(tx: number, ty: number): number {
    if (tx < 0 || ty < 0 || tx >= MW || ty >= MH) return TILE_UNEXPLORED;
    if (tx <= 16) return TILE_VISIBLE_LIT;
    if (unexploredAt(tx, ty)) return TILE_UNEXPLORED;
  ...
  paintTerrain(a: Ctx2D, _f: Readonly<FrameInfo>): void {
    ...
    for (let i = 0; i < TORCH_X.length; i++) a.drawImage(fl, TORCH_X[i] * T + 6, 3);
  }
  ...
  paintGround(a2: Ctx2D, _f: Readonly<FrameInfo>): void {
    ...
    for (let i = 0; i < g.length; i += 3) {
      ...
      if (this.stateOf(g[i + 1], g[i + 2]) === TILE_UNEXPLORED) continue;
      a2.drawImage(f.src, f.sx, f.sy, f.w, f.h,
        g[i + 1] * T + 8 - f.ax, g[i + 2] * T + 13 - f.ay, f.w, f.h);
    }
  }
  ...
  paintActors(b: Ctx2D, lighter: ActorLighter, _f: Readonly<FrameInfo>): void {
    ...
    for (let i = 0; i < o.length; i++) {
      const a = o[i];
      if (!this.visibleAt(a.x, a.y)) continue;
      // 影
      b.globalAlpha = 0.5;
      ...
      if (f) lighter.drawFrame(f, a.x, a.y, 1, !a.flash);
    }
  }
  ...
  paintEmissive(e: Ctx2D, _f: Readonly<FrameInfo>): void {
    ...
    for (let i = 0; i < TORCH_X.length; i++) e.drawImage(fl, TORCH_X[i] * T + 6, 3);
    ...
    for (let i = 0; i < o.length; i++) {
      ...
      if (f) e.drawImage(f.src, f.sx, f.sy, f.w, f.h, a.x - f.ax, a.y - f.ay, f.w, f.h);
    }
  }
  ...
  paintHD(g: Ctx2D, f: Readonly<FrameInfo>): void {
    ...
    for (let i = 0; i < EMBERS; i++) {
      ...
      if (!this.visibleAt(wx, 14 * T + 4)) continue;
...
/** HD の粒の柔らかい光の玉 */
function bakeGlow(rgb: number): Canvas2D {
  const s = 32;
...
function makeStage(w: number, h: number, id: string): HTMLCanvasElement {
...
function readParams(params: URLSearchParams): {
  q: number; scale: number; pp: boolean; perf: boolean; sync: 0 | 1 | 2; dof: boolean;
  t: number | null; cam: [number, number] | null; overlay: boolean;
} {
  ...
  const sync = Math.max(0, Math.min(2, Number(params.get('sync') ?? 1) | 0)) as 0 | 1 | 2;
  return {
...
function openTest(params: URLSearchParams): void {
  ...
  const g = canvas.getContext('2d', { alpha: false }) as CanvasRenderingContext2D | null;
  if (!g) return;
  ...
  const loop = (now: number): void => {
    ...
    if (o.perf && o.overlay) comp.perf.drawOverlay(g, title);
...
function openPasses(params: URLSearchParams): void {
  const o = readParams(params);
  const comp = new Compositor(o.q);
  comp.dofEnabled = o.dof;
  ...
  bufs.forEach((b, i) => {
    ...
    const src = b.c as unknown as { width: number; height: number };
    // 透明が分かるよう市松の地
...
export function open(spec: string, params: URLSearchParams, _canvas: HTMLCanvasElement): void {
  ...
  if (kind === 'passes') openPasses(params);
  else openTest(params);
}
