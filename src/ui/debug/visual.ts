...
import { DIRS, DIR_VEC, type Dir, dirTo } from '../../core/geom.js';
import type { Action, GameEvent, MonsterActor, TownState } from '../../core/types.js';
...
import { CUE, type CueKind, Timeline } from '../anim/timeline.js';
import type {
...
const CUE_NAME: Record<number, string> = {};
for (const [k, v] of Object.entries(CUE)) CUE_NAME[v] = k;

const CUE_COLOR: Partial<Record<CueKind, string>> = {
  [CUE.ATTACK]: '#e8d6a0',
  [CUE.BUMP]: '#a19cb4',
  [CUE.BEAM]: '#bd66d0',
  [CUE.THROW]: '#aacde0',
...
const PRESET_COLOR: Record<VfxPresetId, string> = {
...
interface DebugFx {
  kind: 'burst' | 'beam' | 'projectile' | 'light' | 'popup';
  label: string;
  color: string;
  x: number;
  y: number;
...
class DebugSink implements VfxSink {
  fx: DebugFx[] = [];
  calls: { at: number; what: string }[] = [];
  shakePx = 0;
  shakeLife = 0;
  flashColor = '';
  flashA = 0;
  ...
  transition(kind: TransitionKind): void {
    ...
    if (kind === 'floorOut') this.fade = 0.01;
  }
  ...
  update(ms: number): void {
    for (const f of this.fx) f.t += ms;
    this.fx = this.fx.filter((f) => f.t < f.life);
    if (this.shakeLife > 0) this.shakeLife -= ms;
    else this.shakePx = 0;
    if (this.flashLife > 0) this.flashT += ms;
    if (this.bannerLife > 0) this.bannerLife -= ms;
    if (this.fadeDir > 0) this.fade = Math.min(1, this.fade + ms / 200);
    if (this.fadeDir < 0) {
...
function makeSheet(w: number, h: number): CanvasRenderingContext2D {
...
function text(g: CanvasRenderingContext2D, s: string, x: number, y: number, color: string, size = 13,
...
function autoAction(world: World, r: Rng): Action {
  const p = world.player;
  let best: MonsterActor | null = null;
  let bestD = 99;
  for (const m of world.run.monsters) {
    if (!m.alive || m.kind === 'shopkeeper') continue;
    if (!at(world.map, m.pos.x, m.pos.y)?.visible) continue;
  ...
  if (best && bestD === 1) {
    ...
    if (d !== null) return { type: 'attack', dir: d };
  }
  if (best && bestD <= 6) {
    const d = dirTo(p.pos, best.pos);
    if (d !== null) {
      const q = { x: p.pos.x + DIR_VEC[d].x, y: p.pos.y + DIR_VEC[d].y };
      if (canEnter(world.map, q.x, q.y, 'ground')) return { type: 'move', dir: d };
    }
  }
  for (let tries = 0; tries < 8; tries++) {
    const d = DIRS[r.int(8)];
    const q = { x: p.pos.x + DIR_VEC[d].x, y: p.pos.y + DIR_VEC[d].y };
    if (canEnter(world.map, q.x, q.y, 'ground') && !world.actorAt(q)) return { type: 'move', dir: d };
  }
  return { type: 'wait' };
}
...
function drawMarks(g: CanvasRenderingContext2D, marks: readonly PlanMark[], xOf: (ms: number) => number,
  top: number, lanes: number): void {
  const ends: number[] = new Array(lanes).fill(-1e9);
...
function snapshotPlan(vw: VisualWorld, out: PlanMark[]): void {
  out.length = 0;
...
function play(dungeonId: string, params: URLSearchParams): void {
  loadAllSprites();
  ...
  vw.reset();
  const plan: PlanMark[] = [];
  let batchCalls: { at: number; what: string }[] = [];
...
function drawWorld(g: CanvasRenderingContext2D, world: World, vw: VisualWorld): void {
  ...
  for (let y = y0; y <= y1; y++) {
    for (let x = x0; x <= x1; x++) {
      ...
      let c: string = theme.floor;
      if (t.kind === 'wall') c = theme.wall;
      else if (t.kind === 'water' || t.kind === 'lava') c = theme.liquid;
      else if (t.kind === 'pit') c = '#05050a';
      else if (t.roomId < 0) c = theme.corridor;
      else if (((x + y) & 1) === 0) c = theme.floorAlt;
      g.fillStyle = c;
  ...
  vw.forEachDrawable((d: ActorDrawable) => {
...
function drawLabels(g: CanvasRenderingContext2D, vw: VisualWorld, sx: number): void {
  const cam = vw.camera;
...
function drawFx(g: CanvasRenderingContext2D, vw: VisualWorld, sink: DebugSink, sx: number): void {
  const cam = vw.camera;
  const toX = (tx: number): number => ((tx + 0.5) * TILE_ART - cam.camX) * K + sx;
  const toY = (ty: number): number => ((ty + 0.5) * TILE_ART - cam.camY) * K;
  g.save();
  for (const f of sink.fx) {
    ...
    if (f.kind === 'light') {
      ...
      text(g, f.label, toX(f.x), toY(f.y) - 44 - 20 * Math.min(1, u * 2.2), f.color, Math.round(20 * f.power),
        'center');
    }
  }
  g.restore();
  ...
  if (sink.bannerLife > 0) {
    ...
    if (sink.bannerSub) text(g, sink.bannerSub, VIEW_W / 2, VIEW_H / 2 + 6, '#f1ebdd', 16, 'center');
  }
}

/** 右：今の手で受け口が呼ばれた記録（組の中の時刻） */
function drawCalls(g: CanvasRenderingContext2D, calls: { at: number; what: string }[], vw: VisualWorld): void {
  const x0 = VIEW_W;
  g.fillStyle = '#0d1124';
...
/** 下の帯：今の手の合図 */
function drawStrip(g: CanvasRenderingContext2D, vw: VisualWorld, plan: PlanMark[], world: World): void {
  const y0 = VIEW_H;
  const left = 60;
  const w = SHEET_W - left - 30;
  const xOf = (ms: number): number => left + (Math.min(STRIP_MS, ms) / STRIP_MS) * w;
  g.fillStyle = '#10142a';
  ...
  text(g, `turn ${world.run.totalTurn}  beats ${vw.timeline.beats}  batch ${Math.round(vw.timeline.batchMs)}ms`
    + `  ×${vw.timeline.compression.toFixed(2)}  anims ${vw.registry.count} (made ${vw.registry.created})`,
  SHEET_W - 8, y0 + STRIP_H - 18, '#a19cb4', 11, 'right');
}
...
function beats(): void {
  ...
  world.drainEvents();
  const p = world.player;
  const ms: MonsterActor[] = [];
  for (const d of [2, 6, 4, 0] as Dir[]) {
    const q = { x: p.pos.x + DIR_VEC[d].x, y: p.pos.y + DIR_VEC[d].y };
    const m = world.spawnAt?.('ratField', q);
    if (m) ms.push(m);
  }
  while (ms.length < 3) ms.push(ms[0]);
  const [m1, m2, m3] = ms;
  const P = p.id;
  const cases: { name: string; events: GameEvent[] }[] = [
    {
      name: '殴って倒す → レベルアップ',
      events: [
    ...
    },
    {
      ...
      events: Array.from({ length: 6 }, (_v, i): GameEvent[] => [
  ...
  const xOf = (t: number): number => left + (t / STRIP_MS) * w;
  ...
  cases.forEach((c, row) => {
    ...
    text(g, `拍 ${tl.beats} / ${Math.round(tl.batchMs)}ms / ×${tl.compression.toFixed(2)}`,
      10, y + 28, '#a19cb4', 11);
