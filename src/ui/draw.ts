...
import { type FontFamily, SCREEN_H, SCREEN_W, UI, font } from './theme.js';
...
function frameLine(g: Ctx, x: number, y: number, w: number, h: number, t: number): void {
  g.fillRect(x, y, w, t);
...
export interface PanelOptions {
  /** 枠の色。UI.danger なら危ない確認用の赤い枠、それ以外は金 */
  frame?: string;
  /** 地の不透明度（0〜1）。省略時は既定 */
  alpha?: number;
  /** 使わない（角は四隅の飾りになった）。古い呼び出しのために残す */
  radius?: number;
  /** 影を落とすか */
  shadow?: boolean;
  /** 四隅の飾りを付けるか（小さな札では付けない） */
  ornaments?: boolean;
}
...
export interface TextOptions {
  ...
  maxWidth?: number;
  alpha?: number;
  /** 書体。見出し・名前・告知は 'serif'（明朝） */
  family?: FontFamily;
  /** 字間（px）。明朝の見出しを少し開けると読みやすい */
  spacing?: number;
}
...
export function drawText(g: Ctx, text: string, x: number, y: number, o: TextOptions = {}): void {
  ...
  if (o.spacing) (g as Ctx & { letterSpacing?: string }).letterSpacing = `${o.spacing}px`;
...
export function ellipsize(g: Ctx, text: string, maxWidth: number, size = 18): string {
  ...
  g.restore();
  if (ellipsizeCache.size > 2000) ellipsizeCache.clear();
...
export interface BarOptions {
  ...
  /** 目盛りの数。0 なら描かない */
  segments?: number;
  /** 使わない（角は四角になった）。古い呼び出しのために残す */
  radius?: number;
  /** 両端の菱形の飾り（既定：高さ 12px 以上なら付ける） */
  caps?: boolean;
}
 ...
 */
export function fitRect(r: Rect): Rect {
  let { x, y, w, h } = r;
  w = Math.min(w, SCREEN_W - SAFE * 2);
  h = Math.min(h, SCREEN_H - SAFE * 2);
  if (x + w > SCREEN_W - SAFE) x = SCREEN_W - SAFE - w;
  if (y + h > SCREEN_H - SAFE) y = SCREEN_H - SAFE - h;
  x = Math.max(SAFE, x);
  y = Math.max(SAFE, y);
  return { x, y, w, h };
}
