...
export class GachaAnim {
  ...
  private get bulk(): boolean {
    return this.results.length > 1;
  }

  /** まとめ引きのときの代表レア度（一番良いもの） */
  private get bestRarity(): Rarity | null {
    let best: Rarity | null = null;
    for (const r of this.results) {
      if (!r.rarity) continue;
      if (best === null || CHARGE_MS[r.rarity] > CHARGE_MS[best]) best = r.rarity;
    }
    return best;
  }
  ...
  private spriteOf(r: PullResult): string | null {
    if (r.charm) return 'charm';
    const p = r.prize;
    if (!p) return null;
    if (p.partnerId) return tryGetPartner(p.partnerId)?.baseId ?? null;
    if (p.boost?.t === 'knownItem') return tryGetItem(p.boost.itemId) ? iconKeyForCatalog(p.boost.itemId) : null;
    return null;
  }

  /** カードに出す名前 */
  private titleOf(r: PullResult): string {
    if (r.charm) return charmName(r.charm);
    if (r.prize) return r.prize.name;
    return `${r.gitan} ギタン`;
  }
  ...
  private noteOf(r: PullResult): string {
    if (r.charm) return '村で 1 つだけ 着けられる。冒険の 最初から 印が 効く。';
    if (!r.prize) return '出るものは もう 無い。石が ギタンに 変わる。';
