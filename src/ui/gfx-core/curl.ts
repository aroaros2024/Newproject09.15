...
/** 速度を書き込む先。[0] = vx、[1] = vy（ドット/秒） */
export type Vec2Out = Float32Array | Float64Array;

export interface CurlFieldOptions {
  /** 覆うマスの数（横）。格子点は cols + 1 個。ダンジョンは 48、タイトル・村のジオラマは 28 */
  cols?: number;
  /** 覆うマスの数（縦）。ダンジョンは 32、ジオラマは 16 */
  rows?: number;
  /** 格子の間隔（ドット）。既定 16 = 1 マス */
  cellSize?: number;
  /** 格子の左上の世界座標（ドット） */
  originX?: number;
  originY?: number;
...
export const CURL_GRID_DUNGEON: Readonly<CurlFieldOptions> = { cols: 48, rows: 32 };
/** タイトル・村のジオラマ（28×16 マス。画面 428×240 ドットを覆う） */
export const CURL_GRID_DIORAMA: Readonly<CurlFieldOptions> = { cols: 28, rows: 16 };
...
export class CurlField {
  ...
  private psiA: Float32Array;
  /** 後の格子（その 0.25 秒後） */
  private psiB: Float32Array;
  /** 作っている途中の次の格子（さらに 0.25 秒後） */
  private psiNext: Float32Array;
  ...
  /** 次の格子の、次に作る行 */
  private buildRow = 0;
  private readonly rowsPerTick: number;

  constructor(o: CurlFieldOptions = {}) {
  ...
  tick(): void {
    ...
    for (let j = this.buildRow; j < end; j++) this.fillRow(this.psiNext, j, tNext);
    this.buildRow = end;

    this.tickNo++;
    if (this.tickNo - this.keyIndex * CURL_KEY_TICKS >= CURL_KEY_TICKS) {
      // 作り残しがあれば仕上げる（行の数が 15 刻みで割り切れないときの保険）
      for (let j = this.buildRow; j < this.ny; j++) this.fillRow(this.psiNext, j, tNext);
      const old = this.psiA;
      this.psiA = this.psiB;
   ...
   */
  sample(ax: number, ay: number, out: Vec2Out): void {
  ...
  psiAt(ax: number, ay: number): number {
    let u = (ax - this.originX) * this.invCell;
    let v = (ay - this.originY) * this.invCell;
  ...
  private keyTime(k: number): number {
    return (k * CURL_KEY_TICKS) * CURL_TICK_S;
  }

  private fillRow(dst: Float32Array, j: number, t: number): void {
    const nx = this.nx;
  ...
  private fillAll(dst: Float32Array, t: number): void {
    for (let j = 0; j < this.ny; j++) this.fillRow(dst, j, t);
  }
  ...
  private blend(): void {
    ...
    const b = this.psiB;
    const c = this.cur;
    for (let i = 0; i < c.length; i++) c[i] = a[i] + (b[i] - a[i]) * phase;
  }
}
