 ...
 */
function hash24(x: number, y: number): number {
 ...
 */
export function flicker(seed: number, timeSec: number): number {
  const s = seed | 0;
  if (s === 0) return 1;
...
export const flickerSeed = (tx: number, ty: number, salt = 0): number =>
  hash24(tx, Math.imul(ty | 0, 0x9e3779b1) ^ (0x70c4 + salt)) | 1;
...
export class LightList {
  readonly cap = LIGHT_CAP;
  count = 0;
  /** 中心（世界のドット） */
  readonly x = new Float32Array(LIGHT_CAP);
  readonly y = new Float32Array(LIGHT_CAP);
  /** 半径（ドット） */
  readonly radius = new Float32Array(LIGHT_CAP);
  /** 色（0〜1） */
  readonly r = new Float32Array(LIGHT_CAP);
  readonly g = new Float32Array(LIGHT_CAP);
  readonly b = new Float32Array(LIGHT_CAP);
  /** 強さ（0〜1。1 を超えた分は描いても出ないので切り詰める） */
  readonly intensity = new Float32Array(LIGHT_CAP);
  /** 揺らぎの種（0 = 揺らがない） */
  readonly seed = new Float32Array(LIGHT_CAP);
  /** prepare() が書く：揺らぎを掛けたその瞬間の強さ（0〜1） */
  readonly strength = new Float32Array(LIGHT_CAP);
  ...
  add(
    ...
    if (!(radius > 0) || !(intensity > 0)) return -1;
  ...
  addRgb(x: number, y: number, radius: number, rgb: number, intensity: number, seed = 0): number {
    return this.add(x, y, radius, ((rgb >> 16) & 255) / 255, ((rgb >> 8) & 255) / 255,
      (rgb & 255) / 255, intensity, seed);
  }

  /** その瞬間の強さ（揺らぎ込み）を求める。描く前に 1 回だけ呼ぶ */
  prepare(timeSec: number): void {
...
export interface LightLevels {
  /** 地の色（r, g, b を 0〜1 で、状態 0〜3 の順に 12 個） */
  readonly base: Float32Array;
  /** 光を通す量（状態 0〜3） */
  readonly gate: Float32Array;
}

export function makeLevels(): LightLevels {
  return { base: new Float32Array(TILE_STATES * 3), gate: new Float32Array(TILE_STATES) };
}

/** テーマの空気のうち、明るさの表に要るもの（ThemeLook の一部） */
export interface LookLevels {
  ambient: string;
  memoryColor: string;
  litLevel: number;
  darkLevel: number;
...
export function parseHex(hex: string): number {
  ...
  if (h.length !== 6 && h.length !== 8) return 0x808080;
 ...
 */
export function levelsFromLook(look: LookLevels, out: LightLevels = makeLevels()): LightLevels {
  ...
  const put = (state: number, rgb: number, k: number, gate: number): void => {
 ...
 */
export function fillTileImages(
  states: Uint8Array, count: number, levels: LightLevels,
  base: Uint32Array, mask: Uint32Array, unexp: Uint32Array,
): void {
  // 状態ごとの値を先に 4 つ作っておく
  ...
  for (let s = 0; s < TILE_STATES; s++) {
    ...
    if (s === 0) { b0 = bv; m0 = mv; } else if (s === 1) { b1 = bv; m1 = mv; }
    else if (s === 2) { b2 = bv; m2 = mv; } else { b3 = bv; m3 = mv; }
  }
 ...
 */
export function bakeLightSprite(out: Uint8ClampedArray, size: number, r: number, g: number,
  b: number): void {
  const half = size / 2;
...
export const quantize16 = (v: number): number => (v <= 0 ? 0 : v >= 1 ? 15 : Math.round(v * 15));

/** 丸めた 3 色を 1 つの番号（0〜4095）に */
export const quantIndex = (r: number, g: number, b: number): number =>
  (quantize16(r) << 8) | (quantize16(g) << 4) | quantize16(b);
...
export class LightField {
  ...
  readonly levels: LightLevels = makeLevels();
  ...
  resize(w: number, h: number): void {
    ...
    if (n !== this.states.length) this.states = new Uint8Array(n);
    else this.states.fill(0);
    this.w = w | 0;
    this.h = h | 0;
  }

  /** 世界のドット座標のマスの状態（地図の外は未探索） */
  stateAt(ax: number, ay: number): number {
    const tx = Math.floor(ax / TILE);
    const ty = Math.floor(ay / TILE);
    if (tx < 0 || ty < 0 || tx >= this.w || ty >= this.h) return TILE_UNEXPLORED;
   ...
   */
  sampleLight(ax: number, ay: number, out: Float32Array | number[]): void {
   ...
   */
  sampleRim(ax: number, ay: number, out: Float32Array | number[]): void {
    out[0] = 0;
    const s = this.stateAt(ax, ay);
