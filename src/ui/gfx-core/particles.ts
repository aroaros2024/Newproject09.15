...
import { fxRng, type Rng } from '../../core/rng.js';
import {
  EMISSIVE, RAMP_DUST, RAMP_EMBER, RAMP_GOLD, RAMP_HEAL, RAMP_ICE, RAMP_MAGIC, RAMP_POISON,
  RAMP_SMOKE, RAMP_THUNDER, RAMP_WATER, TRANSPARENT, WHITE, ci, rampIndex, rampOf, rgbOf, stepOf,
} from '../art/palette.js';
import type { PixBuf } from '../art/pixbuf.js';
import type { AmbientRegion, AmbientSpec } from '../art/terrain/types.js';
import type { CurlField } from './curl.js';
...
/** 0 = ドットの粒（画用紙に 1 ドット）、1 = HD の粒（柔らかい光の玉を加算） */
export type ParticleKind = 0 | 1;
...
const rampTable: (readonly number[])[] = [];
/** 階調の番号 → HD の粒の色の組 */
...
function nearestTone(r: number, g: number, b: number): number {
  let best = 0;
  let bestD = Infinity;
...
/** パレットの色 → HD の粒の色の組 */
export function toneOfColor(c: number): number {
  const [r, g, b] = rgbOf(c);
...
export function toneOfHex(hex: string): number {
  ...
  if (Number.isNaN(r + g + b)) return TONE_WHITE;
  return nearestTone(r, g, b);
}
 ...
 */
export function rampIdOf(ramp: readonly number[]): number {
  for (let i = 0; i < rampTable.length; i++) if (rampTable[i] === ramp) return i;
  if (rampTable.length >= 255 || ramp.length === 0) return 0;
  rampTable.push(ramp);
...
/** 番号 → 階調の配列 */
export const rampById = (id: number): readonly number[] => rampTable[id] ?? RAMP_DUST;
...
/** 階調の番号 → HD の粒の色の組 */
export const toneOfRamp = (id: number): number => rampToneTable[id];

// ---------------------------------------------------------------------------
// 技の粒と環境の粒の指定
// ---------------------------------------------------------------------------

/** 一度に弾ける粒（技・爆発・回復）。演出の表（vfxPresets）に定数で並べる */
export interface BurstSpec {
  /** 粒の数 */
  count: number;
  /** 飛び出す速さの範囲（ドット/秒） */
  speed: readonly [number, number];
  /** 飛び出す向きの広がり（ラジアン、全幅）。Math.PI * 2 で全方向 */
  spread: number;
  /** 寿命の範囲（秒） */
  life: readonly [number, number];
  /** 大きさ（ドット）。ドットの粒は 1 辺、HD の粒は半径 */
  size: number;
  /** 色の進み方（palette.ts の RAMP_*）。HD の粒は色の組をここから選ぶ */
  ramp: readonly number[];
  /** 0 = ドットの粒、1 = HD の粒 */
  kind: ParticleKind;
  /** 流れ場に乗る強さ（1 = 塵と同じ） */
  gain: number;
  /** 自分の速さから流れの速さへ寄る速さ（1/秒）。0 なら流れに乗らず飛び続ける */
  drag: number;
  /** 重力（ドット/秒²。負で上へ昇る） */
  gravity: number;
  /**
   * 流れへ乗り換える速さ。寿命の 1/flowRamp が過ぎたところで流れに乗り切る。
   * 0 なら生まれたときから流れに乗る。
   */
  flowRamp: number;
  /** 生まれる位置のばらつき（ドット、半径）。既定 0 */
  radius?: number;
  /** 光る（ブルームの層にも描く） */
  emissive?: boolean;
  /** HD の粒の色の組（TONE_*）。省くと ramp から選ぶ */
  tone?: number;
}

/**
 * 環境の粒の生まれる場所を答える関数。region の中の点（世界のドット）を out[0], out[1] に書く。
 * 点が無ければ false（その回は生まれない）。毎刻み呼ばれるので、中で割り当てをしないこと。
 */
export type SpawnPointFn = (region: AmbientRegion, out: Float32Array, rng: Rng) => boolean;
...
export class ParticlePool {
  ...
  readonly flowIn: Float32Array;
  /** 0〜1 の位相（明滅をずらす） */
  readonly phase: Float32Array;
  /** KIND_PIXEL / KIND_HD */
  readonly kind: Uint8Array;
  /** 階調の番号。MODE_SPRITE のときは始まりの色（パレットの番号） */
  readonly rampId: Uint8Array;
  /** MODE_* */
  ...
  /** 生まれる点を受け取る場所（使い回す） */
  private readonly spawnPt = new Float32Array(2);

  // 環境の粒の出どころ（最初に MAX_EMITTERS ぶん確保）
  private readonly emSpec: (Readonly<AmbientSpec> | null)[];
  private readonly emSpawn: (SpawnPointFn | null)[];
  private readonly emAcc: Float64Array;
  private readonly emAlive: Uint16Array;
  private readonly emRamp: Uint8Array;
  private readonly emTone: Uint8Array;

  constructor(capacity = PARTICLE_CAPACITY, rng: Rng = fxRng) {
    ...
    this.rng = rng;
    this.emSpec = new Array<Readonly<AmbientSpec> | null>(MAX_EMITTERS).fill(null);
    this.emSpawn = new Array<SpawnPointFn | null>(MAX_EMITTERS).fill(null);
   ...
   */
  addAmbient(spec: Readonly<AmbientSpec>, spawn: SpawnPointFn): number {
    let e = -1;
  ...
  /** 出どころを 1 つ外し、その粒を消す */
  removeAmbient(e: number): void {
    if (e < 0 || e >= MAX_EMITTERS || this.emSpec[e] === null) return;
    for (let i = this.na - 1; i >= 0; i--) if (this.emitter[i] === e) this.kill(i);
    this.emSpec[e] = null;
    this.emSpawn[e] = null;
    this.emAlive[e] = 0;
  }
  ...
  clearAmbient(): void {
    ...
    for (let j = na; j < this.n; j++) this.move(j, j - na);
    this.n -= na;
    this.na = 0;
   ...
   * 流れ場は進めない。
   */
  prewarm(field: CurlField, seconds: number, windX = 0): void {
    const h = 1 / 30;
    for (let t = 0; t < seconds; t += h) this.step(h, field, windX);
  }
   ...
   */
  burst(spec: Readonly<BurstSpec>, ax: number, ay: number, dx = 0, dy = 0): number {
    const rng = this.rng;
    ...
    if (hd) fl |= PF_ADDITIVE;
    if (spec.flowRamp > 0) fl |= PF_FLOW_RAMP;
    let made = 0;
   ...
   */
  fromSprite(buf: PixBuf, ax: number, ay: number, every = 2): number {
    const { w, h, px } = buf;
    // 1 周目：数と重心
    let opaque = 0;
    let sx = 0;
    ...
    for (let yy = 0; yy < h; yy++) {
      for (let xx = 0; xx < w; xx++) {
        if (px[yy * w + xx] === TRANSPARENT) continue;
        opaque++;
        sx += xx;
        sy += yy;
      }
   ...
   */
  step(dtSeconds: number, field: CurlField, windX = 0): void {
    const h = dtSeconds;
   ...
   */
  rampStepOf(i: number): number {
    const m = this.mode[i];
    const t = this.age[i] / this.life[i];
    if (m === MODE_SPRITE) return Math.floor(t * (stepOf(this.rampId[i]) + 1));
    ...
    if (m === MODE_BLINK) b *= this.blinkOf(i);
  ...
  alphaOf(i: number): number {
    if (this.kind[i] !== KIND_HD) return 1;
    const t = this.age[i] / this.life[i];
    let a: number;
    ...
    if (this.mode[i] === MODE_BLINK) a *= 0.1 + 0.9 * this.blinkOf(i);
    return a < 0 ? 0 : a;
  }
  ...
  private blinkOf(i: number): number {
  ...
  private allocVfx(): number {
    if (this.n < this.capacity) return this.n++;
    if (this.na > 0) {
      // 環境の区画の最後の場所を、そのまま技の区画の先頭にする（移し替え無し）
      const e = --this.na;
      const em = this.emitter[e];
      if (em !== NO_EMITTER && this.emAlive[em] > 0) this.emAlive[em]--;
      return e;
    }
    return -1;
  }
  ...
  private allocAmbient(): number {
    if (this.n >= this.capacity) return -1;
    // 技の区画の先頭を末尾へ移し、空いた所を環境の区画に足す
    if (this.n > this.na) this.move(this.na, this.n);
    this.n++;
    return this.na++;
  }
  ...
  private kill(i: number): void {
    ...
    if (i < this.na) {
      ...
      if (i !== lastA) this.move(lastA, i);
      if (last !== lastA) this.move(last, lastA);
      this.na--;
    } else if (i !== last) {
      this.move(last, i);
    }
  ...
  private emitAmbient(h: number, field: CurlField): void {
    ...
    for (let e = 0; e < MAX_EMITTERS; e++) {
      ...
      while (acc >= 1) {
        acc -= 1;
        if (this.emAlive[e] >= cap) continue;
        if (!this.spawnAmbient(e, spec, field)) break;
      }
  ...
  private spawnAmbient(e: number, spec: Readonly<AmbientSpec>, field: CurlField): boolean {
    const spawn = this.emSpawn[e];
    if (spawn === null) return false;
    const pt = this.spawnPt;
    const rng = this.rng;
    if (!spawn(spec.region, pt, rng)) return true;
