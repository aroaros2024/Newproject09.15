...
import { type QualityPreset, qualityPreset } from './quality.js';
import type { FrameInfo, WorldScene } from './types.js';
...
export interface BackingSize {
  ...
  /** 使った画素密度（2 まで） */
  dpr: number;
  /** 画面の 1 ドットが何実画素か（3 × backH / 720） */
  artPx: number;
  /** 縦横とも 1 ドットが整数の実画素か（最近傍でそのまま拡大できる） */
  integer: boolean;
}
 ...
 */
export function computeBacking(availW: number, availH: number, dpr: number,
  pixelPerfect: boolean): BackingSize {
...
export function pixelInfo(backW: number, backH: number): { artPx: number; integer: boolean } {
  const px = (ART_SCALE * backH) / SCREEN_H;
  const pxW = (ART_SCALE * backW) / SCREEN_W;
  const isInt = (v: number): boolean => Math.abs(v - Math.round(v)) < 1e-6 && Math.round(v) >= 1;
  return { artPx: px, integer: isInt(px) && isInt(pxW) };
}

/** 整数でない倍率のときに作る中間の紙の倍率（2〜4） */
export const sharpFactor = (artPx: number): number =>
  Math.max(2, Math.min(4, Math.floor(artPx + 1e-9)));
...
export class Compositor {
  readonly lighting = new LightRenderer();
  readonly bloom = new Bloom();
  readonly post = new PostFx();
  readonly perf = new PerfMeter(PASS_NAMES);
  /** 被写界深度は使う画質になった時に作る */
  ...
  /** 地形（照らした後は「照らした地形」） */
  private readonly A = new Surface(ART_W, ART_H, true, false);
  /** 床の物（場面が描く） */
  private readonly A2 = new Surface(ART_W, ART_H, false, false);
  /** キャラ（場面が lighter で描く） */
  private readonly B = new Surface(ART_W, ART_H, false, false);
  /** 照らした床の物 ＋ キャラ */
  private readonly G = new Surface(ART_W, ART_H, false, false);
  /** 整数でない倍率のときの中間の紙（k 倍） */
  private up: Surface | null = null;
  private upK = 0;
  /** 前のフレームの画面の実画素と、そこから決めた拡大のしかた */
  private lastW = 0;
  private lastH = 0;
  private lastInteger = true;
  private lastK = 2;

  private q: QualityPreset;
  private look: ThemeLook | null = null;
  private size: BackingSize | null = null;
  ...
  private applyQuality(): void {
    ...
    if (this.q.dof && !this.dof) this.dof = new DepthOfField();
  }
   ...
   */
  resize(canvas: HTMLCanvasElement, availW: number, availH: number, dpr: number,
    pixelPerfect: boolean): BackingSize {
    const s = computeBacking(availW, availH, dpr, pixelPerfect);
    canvas.style.width = `${s.cssW}px`;
    canvas.style.height = `${s.cssH}px`;
    if (canvas.width !== s.backW) canvas.width = s.backW;
    if (canvas.height !== s.backH) canvas.height = s.backH;
    this.size = s;
    return s;
  }

  /** 最後に決めた大きさ */
  get backing(): BackingSize | null {
    return this.size;
  }
   ...
   */
  render(g: Ctx2D, scene: WorldScene, now: number): void {
    ...
    if (addOn) {
      ...
      if (q.shafts) this.post.drawShafts(h, f, 0.25, 0.25);
      this.bloom.finish();
    }
    if (q.fog) this.post.prepareFog(f);
    perf.mark(P_OFF_HD);
    ...
    if (sharp) this.ensureUp(this.lastK);
    ...
    g.setTransform(sx, 0, 0, sy, 0, 0);
    if (!merged) this.post.drawGrade(g, q, SCREEN_W, SCREEN_H);
    if (q.vignette) this.post.drawVignette(g);
    perf.mark(P_GRADE, g);
  ...
  private blit(g: Ctx2D, s: Surface, sharp: boolean): void {
    if (!sharp || !this.up) {
      g.imageSmoothingEnabled = false;
    ...
    g.imageSmoothingEnabled = true;
    g.drawImage(up.c, 0, 0, ART_W * k, ART_H * k,
      -CROP_X, -CROP_Y, ART_W * ART_SCALE, ART_H * ART_SCALE);
  }

  private ensureUp(k: number): void {
    if (this.up && this.upK === k) return;
    this.up?.release();
  ...
  debugBuffers(): { name: string; c: CanvasImageSource }[] {
    const out: { name: string; c: CanvasImageSource }[] = [
    ...
    if (this.up) out.push({ name: `sharp x${this.upK}`, c: this.up.c });
    return out;
  }
}
