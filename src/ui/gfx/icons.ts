...
import { type AnimId, TIER_BOX, animOf, getRig, getSpecies, renderFrame } from '../art/rig.js';
...
const frames = new Map<string, SpeciesFrame | null>();

/** 種の 1 コマを焼いて返す。リグが無ければ null */
export function speciesFrame(id: string, anim: AnimId = 'idle', dir8 = 4, frame = 0): SpeciesFrame | null {
  const key = `${id}|${anim}|${dir8}|${frame}`;
  const hit = frames.get(key);
  if (hit !== undefined) return hit;
  const def = getSpecies(id);
  const rig = def ? getRig(def.rig) : undefined;
  let out: SpeciesFrame | null = null;
  if (def && rig) {
...
export const hasSpeciesArt = (id: string): boolean => {
  const def = getSpecies(id);
  return !!def && !!getRig(def.rig);
};

/** 待機のコマ番号（時刻から。種ごとの fps） */
function idleFrame(id: string, now: number): number {
  const def = getSpecies(id);
  const rig = def ? getRig(def.rig) : undefined;
 ...
 */
export function drawSpeciesFit(g: G, id: string, cx: number, cy: number, size: number, now: number,
  alpha = 1): boolean {
  const f0 = speciesFrame(id, 'idle', 4, 0);
  if (!f0 || f0.bw === 0) return false;
...
export function drawSpeciesAt(g: G, id: string, feetX: number, feetY: number, scale: number, now: number,
  o: { anim?: AnimId; dir8?: number } = {}): boolean {
  const def = getSpecies(id);
  const rig = def ? getRig(def.rig) : undefined;
  if (!rig) return false;
  ...
  g.save();
  g.imageSmoothingEnabled = false;
  g.drawImage(f.canvas, Math.round(feetX - f.ax * scale), Math.round(feetY - f.ay * scale),
    f.canvas.width * scale, f.canvas.height * scale);
  g.restore();
  return true;
}
 ...
 */
export function drawIconKey(g: G, key: string, cx: number, cy: number, size: number, now = 0,
  o: { alpha?: number } = {}): boolean {
  const alpha = o.alpha ?? 1;
  if (!hasIcon(key) && !key.startsWith('trap:') && drawSpeciesFit(g, key, cx, cy, size, now, alpha)) return true;
