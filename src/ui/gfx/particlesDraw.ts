...
import {
  HD_TONE_CORE, HD_TONE_COUNT, HD_TONE_RIM, KIND_HD, KIND_PIXEL, PF_EMISSIVE, PF_LIT,
  type ParticlePool,
} from '../gfx-core/particles.js';
import { type Canvas2D, type Ctx2D, ctx2d, makeCanvas } from './canvas.js';

/** 光の玉 1 つの大きさ（atlas の 1 マス、px） */
export const HD_DOT = 16;

/** 先に焼いた光の玉（8 色を横に並べた 1 枚） */
export interface HDAtlas {
...
/**
 * 世界のドット → 描く先の座標。 画面 x = ドット x × scale + offX。
 * 画面（1280×720）なら scale = 3・off = −カメラ × 3 − 切り落とし、画用紙なら scale = 1。
 */
export interface ParticleXform {
  scale: number;
  offX: number;
  offY: number;
}
 ...
 */
export function makeHDAtlas(): HDAtlas {
  const cell = HD_DOT;
 ...
 */
export function drawPixelParticles(
  ctx: Ctx2D, pool: ParticlePool, camX: number, camY: number, lightStep = 0,
): void {
  const n = pool.count;
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (lightStep !== 0 && (flags[i] & PF_LIT) !== 0) c = shiftStep(c, lightStep);
 ...
 * 描き終わったら合成の設定を元に戻す。
 */
export function drawHDParticles(
  ctx: Ctx2D, pool: ParticlePool, xf: Readonly<ParticleXform>, atlas: HDAtlas,
): void {
  const n = pool.count;
...
export function drawEmissive(
  ...
  let last = -1;
  for (let i = 0; i < n; i++) {
    if ((flags[i] & PF_EMISSIVE) === 0) continue;
    if (kind[i] === KIND_HD) {
      const a = pool.alphaOf(i);
      if (a <= 0.01) continue;
      const r = size[i] * k;
      ctx.globalAlpha = a > 1 ? 1 : a;
      ctx.drawImage(atlas.canvas, tone[i] * cell, 0, cell, cell,
        (x[i] + 0.5) * k + xf.offX - r, (y[i] + 0.5) * k + xf.offY - r, r * 2, r * 2);
      continue;
    }
    const c = pool.colorIndexOf(i);
    if (EMISSIVE[c] !== 1) continue;
    ctx.globalAlpha = 1;
    if (c !== last) {
      ctx.fillStyle = PAL_CSS[c];
      last = c;
