...
import { type Ctx, drawCursor, drawText } from '../draw.js';
import { DioramaView } from '../world/diorama.js';
import { ConfirmDialog, ListMenu, type MenuEntry, liveValue } from '../menu.js';
...
export class TitleScreen implements Screen {
  ...
  constructor(
    private app: App,
    private onStart: (resume: boolean) => void,
    private onTown: () => void,
  ) {
  ...
  update(dt: number): void {
    ...
    if (this.confirm) {
      if (this.confirm.handleInput(input)) this.confirm = null;
      return;
    }
  ...
  draw(g: Ctx, now: number): void {
    ...
    if (!this.view) this.view = new DioramaView(buildTitleDiorama());
    ...
    rows.forEach((e, i) => {
      ...
      if (sel) drawCursor(g, { x: cx - 150, y, w: 300, h: 40 }, now);
