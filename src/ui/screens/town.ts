...
export class TownScreen implements Screen {
  ...
  private openGachaOdds(): void {
    ...
    this.menus.push(new ListMenu({
      title: () => {
        ...
        if (left > 0) return `出るもの　表の景品 残り ${left} / ${prizeTotal()}`;
        if (room > 0) return '出るもの　表の景品は 出きった。あとは 護石';
  ...
  private charmMenu(c: Charm): void {
    const town = this.app.town;
  ...
  /** どの印を残して打ち直すかを選ぶ */
  private reforgeMenu(c: Charm): void {
    const town = this.app.town;
    const entries: MenuEntry[] = charmRuneList(c).map(({ id, level }) => {
      const def = tryGetRune(id);
      return {
        label: level > 1 ? `${def?.name ?? id} Lv${level}` : (def?.name ?? id),
        desc: def?.desc ?? '',
        onSelect: () => {
          if (!smithReforge(town, new Rng(`reforge:${town.playerName}:${town.gitan}:${c.uid}`),
            c.uid, id)) return false;
          this.app.persist();
  ...
  draw(g: Ctx, now: number): void {
    // 背景：夕暮れの広場（ゆっくり横へ流す）
    if (!this.view) this.view = new DioramaView(buildTownDiorama());
