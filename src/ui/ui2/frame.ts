...
export interface FrameParts {
  /** 四隅（左上・右上・左下・右下） */
  corners: [Canvas2D, Canvas2D, Canvas2D, Canvas2D];
  /** 地の帯（1×64） */
  bg: Canvas2D;
  /** 選択の帯（64×1） */
  select: Canvas2D;
...
function fromRows(rows: readonly string[], flipX = false, flipY = false): Canvas2D {
...
export function frameParts(): FrameParts {
