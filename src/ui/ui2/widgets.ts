...
/** 数字の種類（anim/types.ts の popup の種類と同じ文字列） */
export type PopupStyleKind =
  | 'damage' | 'damageToPlayer' | 'crit' | 'heal' | 'miss' | 'exp' | 'levelUp' | 'info';
...
const POPUP_STYLE: Record<PopupStyleKind, PopupStyle> = {
...
export class PopupLayer {
  private readonly text: string[] = new Array(POPUP_CAP).fill('');
  private readonly kind: PopupStyleKind[] = new Array(POPUP_CAP).fill('info');
  private readonly tx = new Float32Array(POPUP_CAP);
  private readonly ty = new Float32Array(POPUP_CAP);
  private readonly age = new Float32Array(POPUP_CAP);
  /** 同じマスに重なった時の段（上へずらす） */
  private readonly stack = new Uint8Array(POPUP_CAP);
  private n = 0;
  ...
  spawn(text: string, kind: PopupStyleKind, tileX: number, tileY: number): void {
    if (this.n >= POPUP_CAP) this.removeAt(0);
    // 同じマスでまだ若い数字があれば、その上に積む
    let stack = 0;
    for (let i = 0; i < this.n; i++) {
      if (this.tx[i] === tileX && this.ty[i] === tileY && this.age[i] < 350) stack = Math.max(stack, this.stack[i] + 1);
    }
    const i = this.n++;
    this.text[i] = text;
    this.kind[i] = kind;
  ...
  update(dtMs: number): void {
    for (let i = 0; i < this.n; ) {
      this.age[i] += dtMs;
      if (this.age[i] >= POPUP_STYLE[this.kind[i]].life) this.removeAt(i);
      else i++;
    }
  }
   ...
   */
  draw(g: Ctx, originX: number, originY: number, tilePx: number, headPx = 40): void {
    if (this.n === 0) return;
    g.save();
    g.textAlign = 'center';
...
export function envelope(tMs: number, durMs: number, inMs = 200, outMs = 320): number {
  if (tMs <= 0 || tMs >= durMs) return 0;
  if (tMs < inMs) return tMs / inMs;
  if (tMs > durMs - outMs) return (durMs - tMs) / outMs;
  return 1;
}
 ...
 */
export function drawBanner(g: Ctx, text: string, sub: string | null, tMs: number, durMs = 1800,
  y = 250, color: string = PANEL.goldLight): void {
 ...
 */
export function drawFloorCard(g: Ctx, dungeonName: string, depthLabel: string, alpha: number,
  cx = 640, cy = 330): void {
  if (alpha <= 0) return;
  g.save();
...
export function drawLetterbox(g: Ctx, amount: number, maxPx = 72): void {
  if (amount <= 0) return;
...
/** ボスの名前の札（黒帯の上に出す） */
export function drawBossTitle(g: Ctx, name: string, epithet: string | null, tMs: number, durMs = 2400): void {
