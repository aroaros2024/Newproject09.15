...
import {
  type Diorama, WINDMILL_FRAMES, WINDMILL_SIZE, buildWindmillBlades,
} from '../art/scenes/common.js';
import type { AmbientRegion } from '../art/terrain/types.js';
...
import {
  type HDAtlas, type ParticleXform, drawHDParticles, drawPixelParticles, makeHDAtlas,
} from '../gfx/particlesDraw.js';
import { type Canvas2D, ctx2d, makeCanvas, pixBufToCanvas } from '../gfx/canvas.js';
...
function glowSprite(color: string): Canvas2D {
...
export class DioramaView {
  private readonly layers: BakedLayer[] = [];
  private readonly blades: Canvas2D[] = [];
  private readonly field = new CurlField(CURL_GRID_DIORAMA);
  private readonly pool: ParticlePool;
  private readonly atlas: HDAtlas;
  private readonly particleArt = makeCanvas(ART_W, ART_H);
  private readonly pg = ctx2d(this.particleArt);
  private vignette: Canvas2D | null = null;
  private skyCanvas: Canvas2D | null = null;
  private readonly xf: ParticleXform = { scale: ART_SCALE, offX: 0, offY: 0 };
  /** 画用紙の上でカメラがどれだけ横へずれているか（ドット） */
  camX = 0;

  constructor(private readonly d: Diorama) {
    for (const l of d.layers) {
      const sharp = pixBufToCanvas(l.buf);
      let soft: Canvas2D | null = null;
    ...
    for (const a of d.ambient) this.pool.addAmbient(a, spawn);
  ...
  /** 層の横のずれ（画用紙のドット、整数） */
  private offsetOf(parallax: number): number {
  ...
  draw(g: Ctx, now: number): void {
    ...
    this.layers.forEach((l, i) => {
      ...
      if (i === 0) this.drawStars(g, now, ox);
      if (i === 3) this.drawBlades(g, now, ox);
    });
    // 灯りの光（加算）
  ...
  private drawBlades(g: Ctx, now: number, ox: number): void {
    ...
    g.imageSmoothingEnabled = false;
    g.drawImage(c,
      (hub.x - half - ox) * ART_SCALE - CROP_X,
      (hub.y - half) * ART_SCALE - CROP_Y,
      WINDMILL_SIZE * ART_SCALE, WINDMILL_SIZE * ART_SCALE);
  }

  /** 空（なめらかなグラデーション。縦 1 列を焼いて横へ伸ばす） */
  private skyGradient(): Canvas2D {
    if (this.skyCanvas) return this.skyCanvas;
    const c = makeCanvas(1280, 720);
    const g = ctx2d(c);
    const grad = g.createLinearGradient(0, 0, 0, 720);
    for (const [t, col] of this.d.sky) grad.addColorStop(t, col);
    g.fillStyle = grad;
    g.fillRect(0, 0, 1280, 720);
    this.skyCanvas = c;
    return c;
  ...
  private vignetteCanvas(): Canvas2D {
