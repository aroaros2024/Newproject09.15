...
import type { DungeonTheme, FloorMap, TileKind } from '../../core/types.js';
import { PixBuf, hline, rect, vline } from '../art/pixbuf.js';
import { hash2 } from '../art/hash.js';
import type { DecorLight, DecorSprite, TerrainArt, TerrainMap, TerrainStyle, TerrainTile } from '../art/terrain/types.js';
import { ci } from '../art/palette.js';
import { type Canvas2D, ctx2d, makeCanvas, pixBufToImageData } from '../gfx/canvas.js';

const T = 16;
const KIND_CODE: Record<TileKind, number> = { wall: 0, floor: 1, water: 2, lava: 3, stairs: 4, pit: 5 };

/** ゲームの階 → 地形の描き手が読んでよい情報だけ（ワナ・探索状態・暗い部屋は渡さない） */
export function toTerrainMap(map: FloorMap): TerrainMap {
  const tiles: TerrainTile[] = map.tiles.map((t) => ({
...
export interface TerrainCanvases {
  base: Canvas2D;
  liquid: Canvas2D[];
  emissive: Canvas2D;
  lights: readonly DecorLight[];
  decor: readonly DecorSprite[];
}

export class TerrainView implements TerrainCanvases {
  base!: Canvas2D;
  liquid: Canvas2D[] = [];
  emissive!: Canvas2D;
  lights: readonly DecorLight[] = [];
  ...
  private tmap: TerrainMap | null = null;
  private kinds = new Uint8Array(0);
  private style: TerrainStyle;

  constructor(style: TerrainStyle | null, theme: DungeonTheme) {
    this.style = style ?? fallbackStyle(theme);
  }

  /** 階が変わった時：全部描いて焼く */
  build(map: FloorMap): void {
    const w = map.width * T;
    ...
    for (let i = 0; i < map.tiles.length; i++) this.kinds[i] = KIND_CODE[map.tiles[i].kind];
   ...
   * 変わった所があれば true
   */
  update(map: FloorMap): boolean {
    if (!this.art || !this.tmap || this.kinds.length !== map.tiles.length) return false;
    const changed: number[] = [];
    for (let i = 0; i < map.tiles.length; i++) {
      const k = KIND_CODE[map.tiles[i].kind];
      if (k === this.kinds[i]) continue;
      this.kinds[i] = k;
      const t = this.tmap.tiles[i] as { kind: TileKind; hard: boolean };
    ...
    x1 = Math.min(W - 1, x1); y1 = Math.min(map.height - 1, y1);
...
function bake(b: PixBuf): Canvas2D {
  const c = makeCanvas(b.w, b.h);
  ctx2d(c).putImageData(pixBufToImageData(b), 0, 0);
  return c;
}

function rebake(c: Canvas2D, b: PixBuf, x: number, y: number, w: number, h: number): void {
  const part = new PixBuf(w, h);
  for (let yy = 0; yy < h; yy++) {
    for (let xx = 0; xx < w; xx++) part.px[yy * w + xx] = b.get(x + xx, y + yy);
  }
  const g = ctx2d(c);
  g.clearRect(x, y, w, h);
...
function fallbackStyle(_theme: DungeonTheme): TerrainStyle {
  const paintOne = (m: TerrainMap, out: TerrainArt, i: number): void => {
    const x = i % m.width;
    ...
    for (const l of out.liquid) rect(l, ox, oy, T, T, 0);
    ...
    switch (t.kind) {
      case 'wall':
        if (below && below.kind !== 'wall') {
          ...
          for (let r = 0; r < 3; r++) hline(out.base, ox, oy + 7 + r * 3, T, ci('stone', 1));
        } else {
      ...
      case 'lava': {
        ...
        if (lava) rect(out.emissive, ox, oy, T, T, ci('ember', 3));
        break;
      }
      case 'pit':
      ...
      default: {
        ...
        if (t.kind === 'stairs') {
          for (let s = 0; s < 4; s++) rect(out.base, ox + 3 + s, oy + 3 + s * 3, 10 - s * 2, 2, ci('bone', 4 - s));
  ...
  return {
    theme: 'cave',
    paint(m, out) {
      for (let i = 0; i < m.tiles.length; i++) paintOne(m, out, i);
    },
      ...
      for (const c of cells) {
        ...
        for (let dy = -1; dy <= 1; dy++) {
          for (let dx = -1; dx <= 1; dx++) {
            const xx = x + dx;
            const yy = y + dy;
            if (xx >= 0 && yy >= 0 && xx < m.width && yy < m.height) set.add(yy * m.width + xx);
          }
        }
      }
      for (const i of set) paintOne(m, out, i);
    },
  };
}
