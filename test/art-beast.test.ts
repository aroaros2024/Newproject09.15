...
import { type AnimId, TIER_BOX, getRig, getSpecies, renderFrame } from '../src/ui/art/rig.js';
...
function frame(id: string, dir8 = 4, anim: AnimId = 'idle', f = 0): PixBuf {
...
const idle = (id: string, dir8 = 4): PixBuf => frame(id, dir8);

for (const id of RATS) test(`ネズミ ${id} の絵が約束を守る`, () => checkSpecies(id));

test('ネズミの段階は見分けられ、大きいネズミほど大きい', () => {
  checkDistinct(RATS);
  const area = (id: string): number => countPainted(idle(id, 2));
  ...
  const shape = (b: PixBuf): string => [...b.px].map((c) => (c ? 1 : 0)).join('');
...
test('ネズミの噛みつきは当たりのコマで前へ伸びる', () => {
  for (const id of RATS) {
    ...
    const right = (b: PixBuf): number => { const r = bbox(b); return r.x + r.w; };
    assert.ok(right(frame(id, 2, 'attack', strike)) > right(frame(id, 2, 'attack', 0)), `${id} の当たりが前へ出ない`);
  }
});

for (const id of BATS) test(`コウモリ ${id} の絵が約束を守る`, () => checkSpecies(id));
...
test('店の番犬は約束を守り、赤い前掛けで店の犬と分かる', () => {
  ...
  let red = 0;
  for (const c of b.px) if (c > 0 && rampOf(c) === rampIndex('crimson')) red++;
