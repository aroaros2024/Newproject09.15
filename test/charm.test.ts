...
import type { Charm, TownState } from '../src/core/types.js';
import { Rng } from '../src/core/rng.js';
import {
  CHARM_BOX_LIMIT, CHARM_MAX_SLOTS, CHARM_RUNES, CHARM_RUNE_IDS, validateCharmRunes,
} from '../src/data/charms.js';
import { RUNES } from '../src/data/runes.js';
import {
  addCharmRune, charmName, charmRuneLevel, charmRuneList, meltValue, reforgeCharm, rollCharm,
} from '../src/game/charm.js';
...
const charm = (runes: string[], slots = 0, uid = 1): Charm => ({ uid, runes, slots });
...
test('1 万個作っても、載せない印は出ず、上限も超えない', () => {
  ...
  const seenLevel = new Map<string, number>();
...
test('同時に付けられない印は同居しない', () => {
  ...
  for (let i = 0; i < 5_000; i++) {
    ...
    const has = (id: string) => charmRuneLevel(c, id) > 0;
...
test('護石は gachaOwned に入らない（セーブが無限に膨らまない）', () => {
  ...
  let charms = 0;
  for (let i = 0; i < 300; i++) {
    for (const r of pull(town, 1)) if (r.charm) charms++;
  }
  assert.ok(charms > 0, '護石が 1 つも出なかった');
  assert.ok(
    Object.keys(town.gachaOwned ?? {}).length <= prizeTotal(),
    'gachaOwned に護石が混ざっている',
  );
});
...
test('護石の箱が満杯なら護石は出ない', () => {
  const town = newTown({ stones: 100_000 });
  town.charms = [];
  for (let i = 0; i < CHARM_BOX_LIMIT; i++) town.charms.push(charm(['crit'], 0, i + 1));
...
test('引き続けても箱の上限を超えない', () => {
  ...
  for (let i = 0; i < 500; i++) pull(town, 1);
...
test('SR 以上の確定枠は護石に食われない', () => {
  ...
  for (let i = 0; i < 20; i++) {
    ...
    assert.ok(
      got.some((r) => r.rarity === 'ssr' || r.rarity === 'sr'),
      'SR 以上が出ていない',
    );
    for (const r of got) {
      if (r.charm) assert.ok(r.rarity === 'n' || r.rarity === 'r', '護石が SR 以上に出た');
    }
  }
});
...
test('護石の無い古いセーブを読んでも壊れない', () => {
  ...
  delete (old as Partial<TownState>).charms;
  delete (old as Partial<TownState>).activeCharm;
  saveTown(old);
  const back = loadTown();
...
test('箱の上限を超えたセーブは読み込みで切られる', () => {
  const town = newTown();
  town.charms = [];
  for (let i = 0; i < CHARM_BOX_LIMIT + 20; i++) town.charms.push(charm(['crit'], 0, i + 1));
  saveTown(town);
...
test('護石を入れても表の景品は二度と出ない', () => {
  ...
  assert.equal(drawable(town, 'n').length + drawable(town, 'r').length
    + drawable(town, 'sr').length + drawable(town, 'ssr').length,
  prizeTotal() - seen.size, '在庫の数が合わない');
});
