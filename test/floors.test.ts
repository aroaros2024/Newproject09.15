...
import {
  at, connectivityReport, renderAscii, walkDistance,
...
test('全ダンジョンの全フロアが生成でき、階段まで歩いて行ける', () => {
  ...
  for (const d of allDungeons()) {
    ...
    for (let depth = 1; depth <= d.depth; depth++) {
      ...
      if (tile.kind === 'wall') problems.push(`${where}: プレイヤーが壁の中`);
      ...
      for (const a of world.livingActors()) {
        ...
        if (spots.has(key)) problems.push(`${where}: アクターが重なっている (${key})`);
        spots.add(key);
      }
      const itemSpots = new Set<string>();
      for (const f of world.run.floorItems) {
        const key = `${f.pos.x},${f.pos.y}`;
        if (itemSpots.has(key)) problems.push(`${where}: 床アイテムが重なっている (${key})`);
        itemSpots.add(key);
      }
      // モンスターが上限を超えていない
...
test('どの階でもアイテムとモンスターが必ず湧く', () => {
  ...
  for (const d of allDungeons()) {
    ...
    for (let depth = 1; depth <= d.depth; depth += stepBy) {
      enterFloor(world, depth);
      world.drainEvents();
      if (world.run.monsters.length === 0) empty.push(`${d.id} ${depth}F: モンスターが 0`);
      if (world.run.floorItems.length === 0) empty.push(`${d.id} ${depth}F: アイテムが 0`);
    }
  }
...
test('もっと不思議のダンジョンは 99F まで作れる', () => {
  ...
  for (const depth of checks) {
    ...
    assert.ok(
      walkDistance(world.map, world.player.pos, world.map.stairs) >= 0,
      `ex ${depth}F で階段まで行けない`,
    );
