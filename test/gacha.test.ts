...
/** 倉庫の代わり。届いた数だけ数える */
function collector(): { got: string[]; deliver: (id: string, n: number) => boolean } {
  const got: string[] = [];
  return { got, deliver: (id, n) => { for (let i = 0; i < n; i++) got.push(id); return true; } };
}
...
test('景品はすべて実在するものを指す', () => {
  ...
  for (const p of PRIZES) {
    ...
    if (p.itemId) assert.ok(tryGetItem(p.itemId), `${p.id}: 道具 ${p.itemId} が無い`);
...
test('同じ相棒を引き直すと、強さではなく上限が伸びる', () => {
  const rec: PartnerRecord = { id: 'koro', level: 1, exp: 0, dupes: 0 };
...
test('相棒はプレイヤーのレベルより先へ行けない', () => {
  const rec: PartnerRecord = { id: 'koro', level: 40, exp: 0, dupes: 6 };
