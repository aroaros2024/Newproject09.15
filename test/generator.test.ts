...
import {
  at, connectivityReport, isOpen, allRoomFloors, walkDistance, renderAscii,
  reachabilityReport,
} from '../src/dungeon/tilemap.js';
import type { FloorGenParams } from '../src/core/types.js';

const BASE: FloorGenParams = {
  width: 48, height: 32,
...
test('300 シードすべてで全マスが連結している', () => {
  ...
  for (const seed of SEEDS) {
    ...
    if (!connectivityReport(map).connected) broken.push(seed);
  }
...
test('部屋どうしが互いに歩いて行き来できる', () => {
  for (const seed of SEEDS.slice(0, 80)) {
    const map = generateFloor(BASE, seed);
    if (map.rooms.length < 2) continue;
    const centers = map.rooms.map((r) => {
      ...
      for (let y = r.rect.y; y < r.rect.y + r.rect.h; y++) {
        for (let x = r.rect.x; x < r.rect.x + r.rect.w; x++) {
          if (at(map, x, y)?.kind === 'floor') return { x, y };
        }
      }
      return null;
...
test('水路を入れても、地上を歩いて全部の床へ行ける', () => {
  const params: FloorGenParams = { ...BASE, waterRate: 70, liquid: 'water' };
  let withWater = 0;
  for (const seed of SEEDS.slice(0, 120)) {
    ...
    assert.ok(
      reachabilityReport(map, 'ground').ok,
      `seed ${seed}: 水に囲まれて歩いて行けない床がある\n${renderAscii(map)}`,
    );
    if (map.tiles.some((t) => t.kind === 'water')) withWater++;
    // 水に囲まれて渡れない床が無いこと（部屋の外周は床のまま残る仕様）
    for (const room of map.rooms) {
      const r = room.rect;
      let dryEdge = 0;
      for (let x = r.x; x < r.x + r.w; x++) {
        if (at(map, x, r.y)?.kind === 'floor') dryEdge++;
        if (at(map, x, r.y + r.h - 1)?.kind === 'floor') dryEdge++;
      }
...
test('溶岩でも、地上を歩いて全部の床へ行ける', () => {
  const params: FloorGenParams = { ...BASE, waterRate: 80, liquid: 'lava' };
  for (const seed of SEEDS.slice(0, 60)) {
    const map = generateFloor(params, seed);
...
test('円形の部屋と水路が同時に出ても、歩いて行けない床が生まれない', () => {
  const params: FloorGenParams = { ...BASE, waterRate: 80, liquid: 'water' };
  for (const seed of SEEDS.slice(0, 150)) {
    ...
    assert.ok(
      report.ok,
      `seed ${seed}: ${report.unreachable.length} マスへ歩いて行けない\n${renderAscii(map)}`,
    );
  }
});

test('大部屋に水路が出ても歩いて渡れる', () => {
  const params: FloorGenParams = { ...BASE, waterRate: 90, liquid: 'water' };
  for (const seed of SEEDS.slice(0, 60)) {
...
test('極端に部屋を減らしても最低 3 部屋は残る', () => {
  const sparse: FloorGenParams = { ...BASE, emptyCellRate: 100, gridCols: [4, 4], gridRows: [3, 3] };
  for (const seed of SEEDS.slice(0, 40)) {
    ...
    for (const room of map.rooms) {
      for (let y = room.rect.y; y < room.rect.y + room.rect.h; y++) {
        for (let x = room.rect.x; x < room.rect.x + room.rect.w; x++) {
          ...
          assert.equal(
            t.roomId, room.id,
            `seed ${seed}: 部屋 ${room.id} の内側 (${x},${y}) が通路になっている`,
          );
        }
      }
    }
...
test('迷路フロアでも部屋が区画からはみ出さない', () => {
  const params: FloorGenParams = {
    ...BASE, gridCols: [3, 4], gridRows: [2, 3], emptyCellRate: 18,
  };
  for (let seed = 0; seed < 400; seed++) {
    const map = generateFloor(params, seed, { maze: true });
    for (const r of map.rooms) {
      assert.ok(r.rect.x >= 1 && r.rect.y >= 1,
        `seed ${seed}: 部屋 ${r.id} が外周に食い込んでいる ${JSON.stringify(r.rect)}`);
      assert.ok(r.rect.x + r.rect.w <= map.width - 1 && r.rect.y + r.rect.h <= map.height - 1,
        `seed ${seed}: 部屋 ${r.id} が外周に食い込んでいる ${JSON.stringify(r.rect)}`);
    ...
    for (const r of map.rooms) {
      for (const d of r.doors) {
        ...
        assert.equal(seen.has(k), false,
          `seed ${seed}: (${d.x},${d.y}) が複数の部屋の出入口になっている`);
        seen.add(k);
      }
    }
  }
