...
import {
  DIR_VEC, DIRS, ORTHO_DIRS, DIAGONAL_DIRS, isDiagonal, rotateDir, oppositeDir,
  dirDistance, vecToDir, dirTo, step, chebyshev, manhattan, isAdjacent,
  isOnRay, rayPoints, rectContains, rectCenter, rectOverlaps, rectPoints,
  clamp, samePoint, type Dir,
...
test('ORTHO/DIAGONAL の分類が正しい', () => {
  ...
  for (const d of ORTHO_DIRS) assert.equal(isDiagonal(d), false);
  for (const d of DIAGONAL_DIRS) assert.equal(isDiagonal(d), true);
});
...
test('isAdjacent は自分自身を含まない', () => {
  ...
  for (const d of DIRS) assert.equal(isAdjacent(c, step(c, d)), true);
