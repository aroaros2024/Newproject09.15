...
function takeAll(town: TownState): ItemInstance[] {
  const uids = town.storage.map((i) => i.uid);
  return uids.map((uid) => withdrawItem(town, uid)).filter((i): i is ItemInstance => i !== null);
}
...
test('村で名前を見て買った物は、ダンジョンでも名前のまま', () => {
  ...
  for (const item of town.storage) {
    ...
    assert.ok(
      !itemName(item, tid).includes('？'),
      `村で ${itemName(item, tid)} と出ている`,
    );
  }
  ...
  for (const defId of bought) {
    ...
    assert.equal(
      world.run.identify.known[defId], true,
      `${defId} がダンジョンで未識別に戻っている（${itemName(carried!, world.run.identify)}）`,
    );
  }
});
