...
import {
  EXPLORED_GATE, EXPLORED_LEVEL, FLICKER_AMP, LIGHT_CAP, LightField, LightList,
  MIN_VISIBLE_LEVEL, TILE_EXPLORED, TILE_UNEXPLORED, TILE_VISIBLE_DARK, TILE_VISIBLE_LIT,
  bakeLightSprite, falloff, fillTileImages, flicker, flickerSeed, levelsFromLook, parseHex,
  quantIndex, quantize16,
} from '../src/ui/gfx-core/lightModel.js';
...
/** 全部のマスが同じ状態の場 */
function uniformField(w: number, h: number, state: number): LightField {
...
test('未探索のマスは、強い光のすぐそばでも真っ黒（地の色も光を通す量も 0）', () => {
  ...
  for (let i = 1; i < 4; i++) assert.equal(unexp[i], 0, `状態 ${i} は黒く塗らない`);
...
test('探索済みのマスは 0.30（記憶の色）+ 0.35 × 光 より明るくならない', () => {
  ...
  const L = f.lights;
  for (let i = 0; i < 12; i++) {
    L.add(hashFloat(i, 1) * 160, hashFloat(i, 2) * 128, 20 + hashFloat(i, 3) * 80,
      hashFloat(i, 4), hashFloat(i, 5), hashFloat(i, 6), 0.3 + hashFloat(i, 7) * 0.7, i % 2 ? flickerSeed(i, 0) : 0);
  }
  L.prepare(3.7);
  const mem = rgb01(LOOK.memoryColor);
  const out = new Float32Array(3);
...
test('明るさは光から離れるほど弱まり（単調）、いつも 0〜1 に収まる', () => {
  for (const state of [TILE_EXPLORED, TILE_VISIBLE_DARK, TILE_VISIBLE_LIT]) {
    ...
    for (let c = 0; c < 3; c++) assert.ok(near(out[c], quantize16(f.levels.base[state * 3 + c]) / 15));
  }
});
...
test('光の絵は減衰と同じ形（中心が最も濃く、四隅は透明、色はそのまま）', () => {
  ...
  const alpha = (x: number, y: number): number => px[(y * size + x) * 4 + 3];
  assert.equal(alpha(0, 0), 0);
  assert.equal(alpha(size - 1, size - 1), 0);
  assert.ok(alpha(32, 32) >= 250);
  for (let x = 33; x < size; x++) assert.ok(alpha(x, 32) <= alpha(x - 1, 32));
...
test('光の一覧は 16 個まで。あふれたら弱い光と入れ替わり、強い光（ランタン）は残る', () => {
  ...
  for (let i = 0; i < 40; i++) L.add(i * 10, 0, 8 + (i % 5), 1, 1, 1, 0.2 + (i % 3) * 0.1);
...
test('明るさは 16 段に丸める（キャラの色の文字列を 4096 通りで済ませるため）', () => {
  ...
  for (let c = 0; c < 3; c++) assert.ok(near(out[c] * 15, Math.round(out[c] * 15), 1e-5));
});
