...
import type { AmbientSpec } from '../src/ui/art/terrain/types.js';
import { CurlField } from '../src/ui/gfx-core/curl.js';
import {
  type BurstSpec, KIND_HD, KIND_PIXEL, MODE_AGE, MODE_SPRITE, PF_AMBIENT, ParticlePool,
  SPRITE_PARTICLE_MAX, type SpawnPointFn,
} from '../src/ui/gfx-core/particles.js';

const H = 1 / 60;
const TAU = Math.PI * 2;

const anywhere: SpawnPointFn = (_region, out, rng) => {
  out[0] = rng.float() * 428;
  out[1] = rng.float() * 240;
  return true;
};
...
/** 丸い仮の絵 */
function blob(w: number, h: number, r: number): PixBuf {
...
function arraysOf(p: ParticlePool): (Float32Array | Uint8Array)[] {
...
test('1 万刻み回しても上限を超えず、配列は増えも作り直されもしない', () => {
  ...
  for (let t = 0; t < 10000; t++) {
    if (t % 10 === 0) pool.burst(MAGIC, 50 + rng.float() * 300, 40 + rng.float() * 160);
    if (t % 45 === 0) pool.burst(GLOW, 200, 120, 0, -1);
    if (t % 300 === 0) pool.fromSprite(sprite, 100, 100, 1);
    // 途中で画質を落とし、また戻す
    if (t === 4000) pool.setAmbientScale(0.25);
    if (t === 7000) pool.setAmbientScale(1);
    field.tick();
    ...
    if (n === pool.capacity) sawFull = true;
    if (t % 997 === 0) {
      for (let i = 0; i < n; i++) {
...
test('消えた粒の場所を詰めて使い回し、粒の中身は崩れない', () => {
  ...
  const a: BurstSpec = { ...MAGIC, count: 40, life: [0.3, 1.2], size: 1, gravity: 5 };
  const b: BurstSpec = { ...MAGIC, count: 40, life: [0.3, 1.2], size: 2, gravity: -7 };
...
test('MODE_AGE：年齢とともに階調を進むだけで戻らない', () => {
  ...
  const one: BurstSpec = { ...MAGIC, count: 1, life: [1.2, 1.2] };
  ...
  while (pool.count > 0) {
    ...
    if (seen.length) assert.ok(s >= seen[seen.length - 1], `${seen.join(',')} → ${s}`);
    seen.push(s);
    pool.step(H, field);
  }
...
test('満杯なら技の粒は環境の粒を追い出して入り、環境の粒は生まれない', () => {
  ...
  const field = new CurlField({ cols: 28, rows: 16 });
  const flood: AmbientSpec = { ...DUST, cap: 1000, rate: 6000, life: [100, 100] };
  ...
  const long: BurstSpec = { ...MAGIC, life: [100, 100] };
  ...
  // 満杯の間は環境の粒は生まれない
  for (let t = 0; t < 30; t++) pool.step(H, field);
  assert.equal(pool.ambientCount, 0);
  assert.equal(pool.count, 100);
});
...
test('出どころを外すと、その粒だけ消えて技の粒は残る', () => {
  ...
  let dust = 0;
  for (let i = 0; i < pool.ambientCount; i++) if (pool.emitter[i] === a) dust++;
  ...
  for (let i = 0; i < pool.ambientCount; i++) assert.notEqual(pool.emitter[i], a);
});
...
test('同じ種・同じ手順なら同じ粒（表示用の乱数だけで決まる）', () => {
  const run = (): Float32Array => {
    ...
    pool.addAmbient(DUST, anywhere);
    for (let t = 0; t < 300; t++) {
      if (t % 30 === 0) pool.burst(MAGIC, 100, 100, 1, 0);
      field.tick();
      pool.step(H, field);
    }
