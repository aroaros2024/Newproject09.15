...
import type { Action, ItemInstance, Replay, TownState } from '../src/core/types.js';
import { Rng } from '../src/core/rng.js';
import { DIRS } from '../src/core/geom.js';
import {
  MAX_ACTIONS, Recorder, decodeAction, decodeActions, encodeAction, encodeActions, replayRun,
...
/** ランダムに遊んで、押した行動を全部返す */
function playAndRecord(dungeonId: string, town: TownState, turns: number, seed: number): {
  actions: Action[]; final: string;
} {
  ...
  for (let i = 0; i < turns; i++) {
    ...
    const r = rng.float();
    const a: Action = r < 0.70 ? { type: 'move', dir: rng.pick(DIRS) }
...
test('Action の型がすべて書き出せる（型を足したら落ちる）', () => {
  ...
  const tags = new Set(types.map((t) => encodeAction({ type: t } as Action).slice(0, 1)));
...
test('記録した冒険を再生すると、最後の状態が完全に一致する', () => {
  for (const dungeonId of ['d1', 'd3', 'ex']) {
    ...
    assert.equal(
      JSON.stringify(out.world.syncForSave()), final,
      `${dungeonId}: 再生した結果が元とずれた`,
    );
  }
});
...
test('持ち込んだ道具も込みで再生できる', () => {
  ...
  const snapshot = JSON.parse(JSON.stringify(town)) as TownState;
  const bringCopy = JSON.parse(JSON.stringify(bring)) as ItemInstance[];
  const world = startRun('d1', town, { bring, seed: 99 });
  const r2 = new Rng(31);
  const actions: Action[] = [];
  for (let i = 0; i < 120; i++) {
    if (world.finished) break;
    const a: Action = { type: 'move', dir: r2.pick(DIRS) };
    actions.push(a);
    stepTurn(world, a);
    world.drainEvents();
  }
...
test('上限を超えたら記録をやめる', () => {
  ...
  rec.begin('d1', 1);
  for (let i = 0; i < MAX_ACTIONS + 10; i++) rec.action({ type: 'wait' });
...
function sampleReplay(actions: Action[]): Replay {
  return {
