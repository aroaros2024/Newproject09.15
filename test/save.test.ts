...
import type { Dir, RunState, TownState } from '../src/core/types.js';
...
test('中断データから再開すると、同じ続きが再現される', () => {
  ...
  const moves: Dir[] = Array.from({ length: 60 }, () => contRng.pick(DIRS) as Dir);
...
test('死ぬと持ち物を失う（ただし始まりの洞窟は例外）', () => {
  ...
  // d1 で死ぬ: 失わない
  {
    ...
    finishRun(world, town, 'death', 'やられた');
    assert.ok(
      town.storage.some((i) => i.defId === 'steelSword'),
      'd1 で死んだのに持ち物を失っている',
    );
  }
});
...
test('倉庫の壺に入れた物は、死んでも村に届く', () => {
  ...
  finishRun(world, town, 'death', 'やられた');
  assert.ok(
    town.storage.some((i) => i.defId === 'tenrinShield'),
    '倉庫の壺に入れた物が届いていない',
  );
});
...
test('冒険で出会ったモンスターと道具が図鑑に残る', async () => {
  ...
  refreshFov(world);
  assert.ok(
    world.run.encountered.monsters.includes(m.defId),
    '視界に入った敵が記録されていない',
  );
...
test('図鑑のデータもセーブに載る', () => {
  ...
  assert.equal(
    new Set(restored.encountered.monsters).size,
    restored.encountered.monsters.length,
    '同じ敵が重複して記録されている',
  );
});
