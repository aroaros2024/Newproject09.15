...
import type { Room, TownState } from '../src/core/types.js';
...
/** 店が出るフロアを探す */
function findShopFloor(): { world: World; room: Room } | null {
...
test('ギタンを払えば商品を買える', () => {
  ...
  assert.ok(
    world.player.inventory.some((i) => i.defId === good.item.defId && i.shopPrice === 0),
    '買った物が持ち物に入っていない',
  );
...
test('商品を持ったまま店を出ると泥棒になり、店主が怒る', () => {
  ...
  const outside = world.randomOpenTile(
    (p) => at(world.map, p.x, p.y)?.shop === false
