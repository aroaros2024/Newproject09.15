...
const uids = (town: TownState): number[] =>
  town.storage.flatMap((i) => [i.uid, ...i.contents.map((c) => c.uid)]);

test('倉庫に預けると uid が村の採番に振り直される', () => {
  const town = newTown();
  const rng = new Rng('t');
  // 冒険側の uid（1 から振り直される）をそのまま持ち帰ったつもりで預ける
  const a = makeItem('woodStick', rng, { uid: 1 } as never, () => 1);
  a.uid = 1;
  const b = makeItem('woodShield', rng, {} as never, () => 1);
  b.uid = 1;
...
test('倉庫から出すとき、狙った道具だけが消える', () => {
  ...
  for (const id of ['woodStick', 'woodShield', 'identifyScroll']) {
    const it = makeItem(id, rng, {} as never, () => 1);
    it.uid = 1; // わざと全部ぶつける
    depositItem(town, it);
  }
